...
static int cam_bu24618_ois_fw_download(struct cam_ois_ctrl_t *o_ctrl)
{
...
static int cam_ois_pkt_parse(struct cam_ois_ctrl_t *o_ctrl, void *arg)
{
	...
	case CAM_OIS_PACKET_OPCODE_INIT:
		...
		for (i = 0; i < csl_packet->num_cmd_buf; i++) {
			...
			} else if (o_ctrl->i2c_pre_init_data.is_settings_valid == 0) {
			...
			} else if (o_ctrl->i2c_post_init_data.is_settings_valid == 0) {
		...
		if (o_ctrl->ois_fw_flag) {
			rc = cam_bu24618_ois_fw_download(o_ctrl);
			if (rc) {
