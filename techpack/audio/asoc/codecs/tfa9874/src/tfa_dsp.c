...
int tfa987x_irq_enable(struct tfa_device *tfa, int bit, int status)
{
	...
	pr_debug("[slave=0x%x] bit=0x%x  status=%d\n", tfa->slave_address, bit,
		 status);
	if (tfa2_irq_all == bit) {
		err = reg_write(tfa, TFA98XX_INTERRUPT_ENABLE_REG1,
...
int tfa987x_irq_clear(struct tfa_device *tfa, int bit)
{
	...
	if (tfa2_irq_all == bit) {
		err = reg_write(tfa, TFA98XX_INTERRUPT_IN_REG1,
				0xffff); /* all bits */
	} else {
		uint16_t value = 0, new_value = 0;
		if (((tfa->rev & 0xff) == 0x74) && (bit >= tfa9874_irq_max)) {
...
void tfa_set_query_info(struct tfa_device *tfa)
{
	...
	tfa->sync_iv_delay = 0;
	tfa->dynamicTDMmode =
		-1; /**tracking dynamic TDM setting from alsa input stream*/
...
enum Tfa98xx_Error reg_read(struct tfa_device *tfa, unsigned char subaddress,
...
enum Tfa98xx_Error reg_write(struct tfa_device *tfa, unsigned char subaddress,
			     unsigned short value)
{
	enum Tfa98xx_Error error;
				 ...
				 int is_calibration)
{
	...
	case TFA_STATE_INIT_CF: /* coolflux HW access possible (~initcf) */
		...
		pr_debug(
			"after performed tfa98xx_dsp_system_stable()  err=%d ready=%d  loop=%d\n",
			err, ready, loop);
