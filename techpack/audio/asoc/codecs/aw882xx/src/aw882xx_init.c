...
int aw882xx_dev_i2c_write_bits(struct aw_device *aw_dev, unsigned char reg_addr,
...
int aw882xx_dev_i2c_write(struct aw_device *aw_dev, unsigned char reg_addr,
...
int aw882xx_dev_i2c_read(struct aw_device *aw_dev, unsigned char reg_addr,
			 unsigned int *reg_data)
{
	struct aw882xx *aw882xx = (struct aw882xx *)aw_dev->private_data;
...
{
	return (((value / AW_PID_1852_VOL_STEP_DB) << 4) +
		(value % AW_PID_1852_VOL_STEP_DB));
}
...
static int aw_pid_1852_set_volume(struct aw_device *aw_dev, unsigned int value)
{
	...
	real_value = aw_pid_1852_db_val_to_reg(
		AW_GET_MIN_VALUE(value, vol_desc->mute_volume));
...
static int aw_pid_1852_dev_init(struct aw882xx *aw882xx)
{
	...
	memcpy(aw_pa->monitor_name, AW_PID_1852_MONITOR_FILE,
	       strlen(AW_PID_1852_MONITOR_FILE));
	...
	aw_pa->work_mode.spk_val =
		AW_PID_1852_RCV_MODE_SPEAKER_MODE_VCOM13PVDD_VALUE;
	aw_pa->work_mode.rcv_val =
	...
	aw_pa->cco_mux_desc.divided_val =
		AW_PID_1852_I2S_CCO_MUX_8_16_32KHZ_VALUE;
	aw_pa->cco_mux_desc.bypass_val =
		AW_PID_1852_I2S_CCO_MUX_EXC_8_16_32KHZ_VALUE;
...
{
	return (((value / AW_PID_2013_VOL_STEP_DB) << 6) +
		(value % AW_PID_2013_VOL_STEP_DB));
}
...
static int aw_pid_2013_set_volume(struct aw_device *aw_dev, unsigned int value)
{
	...
	real_value = aw_pid_2013_db_val_to_reg(
		AW_GET_MIN_VALUE(value, vol_desc->mute_volume));
...
static void aw_pid_2013_efver_check(struct aw_device *aw_dev)
{
	...
	aw_dev->ops.aw_i2c_read(aw_dev, AW_PID_2013_EFRM1_REG, &reg_val);
...
static int aw_pid_2013_dev_init(struct aw882xx *aw882xx)
{
	...
	/*call aw device init func*/
	memset(aw_pa->monitor_name, 0, AW_NAME_MAX);
	memcpy(aw_pa->monitor_name, AW_PID_2013_MONITOR_FILE,
	       strlen(AW_PID_2013_MONITOR_FILE));
...
{
	return (((value / AW_PID_2032_VOL_STEP_DB) << 6) +
		(value % AW_PID_2032_VOL_STEP_DB));
}
...
static int aw_pid_2032_set_volume(struct aw_device *aw_dev, unsigned int value)
{
	...
	real_value = aw_pid_2032_db_val_to_reg(
		AW_GET_MIN_VALUE(value, vol_desc->mute_volume));
...
static int aw_pid_2032_dev_init(struct aw882xx *aw882xx)
{
	...
	/* call aw device init func */
	memset(aw_pa->monitor_name, 0, AW_NAME_MAX);
	memcpy(aw_pa->monitor_name, AW_PID_2032_MONITOR_FILE,
	       strlen(AW_PID_2032_MONITOR_FILE));
...
{
	return (((value / AW_PID_2055A_VOL_STEP_DB) << 6) +
		(value % AW_PID_2055A_VOL_STEP_DB));
}
...
static int aw_pid_2055a_set_volume(struct aw_device *aw_dev, unsigned int value)
{
	...
	real_value = aw_pid_2055a_db_val_to_reg(
		AW_GET_MIN_VALUE(value, vol_desc->mute_volume));
...
static int aw_pid_2055a_get_volume(struct aw_device *aw_dev,
				   unsigned int *value)
{
	unsigned int reg_value = 0;
	unsigned int real_value = 0;
...
static bool aw_pid_2055a_check_wr_access(int reg)
{
	...
	if (aw_pid_2055a_reg_access[reg] & AW_PID_2055A_REG_WR_ACCESS)
		return true;
	else
		return false;
}
...
static int aw_pid_2055a_dev_init(struct aw882xx *aw882xx)
{
	...
	/*call aw device init func*/
	memset(aw_pa->monitor_name, 0, AW_NAME_MAX);
	memcpy(aw_pa->monitor_name, AW_PID_2055A_MONITOR_FILE,
	       strlen(AW_PID_2055A_MONITOR_FILE));
...
{
	return (((value / AW_PID_2055_VOL_STEP_DB) << 6) +
		(value % AW_PID_2055_VOL_STEP_DB));
}
...
static int aw_pid_2055_set_volume(struct aw_device *aw_dev, unsigned int value)
{
	...
	real_value = aw_pid_2055_db_val_to_reg(
		AW_GET_MIN_VALUE(value, vol_desc->mute_volume));
...
static bool aw_pid_2055_check_wr_access(int reg)
{
	...
	if (aw_pid_2055_reg_access[reg] & AW_PID_2055_REG_WR_ACCESS)
		return true;
	else
		return false;
}
...
static int aw_pid_2055_dev_init(struct aw882xx *aw882xx)
{
	...
	/*call aw device init func*/
	memset(aw_pa->monitor_name, 0, AW_NAME_MAX);
	memcpy(aw_pa->monitor_name, AW_PID_2055_MONITOR_FILE,
	       strlen(AW_PID_2055_MONITOR_FILE));
...
static int aw_pid_2055_dev_check(struct aw882xx *aw882xx)
{
	...
	aw882xx_i2c_write(aw882xx, AW882XX_SOFT_RESET_REG,
	...
	if (reg_data == AW_PID_2055A_VERSION_VALUE)
		...
		aw_dev_err(aw882xx->dev, "unsupported 2055 verison, 0x%04x",
			   reg_data);
...
{
	return (((value / AW_PID_2071_VOL_STEP_DB) << 6) +
		(value % AW_PID_2071_VOL_STEP_DB));
}
...
static int aw_pid_2071_set_volume(struct aw_device *aw_dev, unsigned int value)
{
	...
	real_value = aw_pid_2071_db_val_to_reg(
		AW_GET_MIN_VALUE(value, vol_desc->mute_volume));
...
static int aw_pid_2071_dev_init(struct aw882xx *aw882xx)
{
	...
	/*call aw device init func*/
	memset(aw_pa->monitor_name, 0, AW_NAME_MAX);
	memcpy(aw_pa->monitor_name, AW_PID_2071_MONITOR_FILE,
	       strlen(AW_PID_2071_MONITOR_FILE));
...
{
	return (((value / AW_PID_2113_VOL_STEP_DB) << 6) +
		(value % AW_PID_2113_VOL_STEP_DB));
}
...
static int aw_pid_2113_set_volume(struct aw_device *aw_dev, unsigned int value)
{
	...
	real_value = aw_pid_2113_db_val_to_reg(
		AW_GET_MIN_VALUE(value, vol_desc->mute_volume));

	aw_dev_dbg(aw882xx->dev, "value:%d, min_val: %d, real_value: 0x%x",
		   value, AW_GET_MIN_VALUE(value, vol_desc->mute_volume),
		   real_value);
...
static int aw_pid_2113_frcset_check(struct aw_device *aw_dev)
{
	...
	aw_dev->ops.aw_i2c_read(aw_dev, AW_PID_2113_EFRH3_REG, &reg_val);
	temh = ((uint16_t)reg_val & (~AW_PID_2113_TEMH_MASK));

	aw_dev->ops.aw_i2c_read(aw_dev, AW_PID_2113_EFRL3_REG, &reg_val);
	...
	aw_dev_info(aw_dev->dev, "tem is 0x%04x, frcset_en is %d", tem,
		    aw_dev->frcset_en);
	return 0;
}
...
static void aw_pid_2113_reg_force_set(struct aw_device *aw_dev)
{
	...
	if (aw_dev->frcset_en == AW_FRCSET_ENABLE) {
		/*set FORCE_PWM*/
		aw_dev->ops.aw_i2c_write_bits(
			aw_dev, AW_PID_2113_BSTCTRL3_REG,
			AW_PID_2113_FORCE_PWM_MASK,
			AW_PID_2113_FORCE_PWM_FORCEMINUS_PWM_VALUE);
		/*set BOOST_OS_WIDTH*/
		aw_dev->ops.aw_i2c_write_bits(
			aw_dev, AW_PID_2113_BSTCTRL5_REG,
			AW_PID_2113_BST_OS_WIDTH_MASK,
			AW_PID_2113_BST_OS_WIDTH_50NS_VALUE);
		/*set BURST_LOOPR*/
		aw_dev->ops.aw_i2c_write_bits(aw_dev, AW_PID_2113_BSTCTRL6_REG,
					      AW_PID_2113_BST_LOOPR_MASK,
					      AW_PID_2113_BST_LOOPR_340K_VALUE);
		/*set RSQN_DLY*/
		aw_dev->ops.aw_i2c_write_bits(aw_dev, AW_PID_2113_BSTCTRL7_REG,
					      AW_PID_2113_RSQN_DLY_MASK,
					      AW_PID_2113_RSQN_DLY_35NS_VALUE);
		/*set BURST_SSMODE*/
		aw_dev->ops.aw_i2c_write_bits(
			aw_dev, AW_PID_2113_BSTCTRL8_REG,
			AW_PID_2113_BURST_SSMODE_MASK,
			AW_PID_2113_BURST_SSMODE_FAST_VALUE);
		/*set BST_BURST*/
		aw_dev->ops.aw_i2c_write_bits(aw_dev, AW_PID_2113_BSTCTRL9_REG,
					      AW_PID_2113_BST_BURST_MASK,
					      AW_PID_2113_BST_BURST_30MA_VALUE);
...
static int aw_pid_2113_dev_init(struct aw882xx *aw882xx)
{
	...
	/*call aw device init func*/
	memset(aw_pa->monitor_name, 0, AW_NAME_MAX);
	memcpy(aw_pa->monitor_name, AW_PID_2113_MONITOR_FILE,
	       strlen(AW_PID_2113_MONITOR_FILE));
...
int aw882xx_init(struct aw882xx *aw882xx)
{
	...
	default:
		aw_dev_err(aw882xx->dev, "unsupported chip id 0x%04x",
