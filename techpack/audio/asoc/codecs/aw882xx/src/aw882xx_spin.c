			    ...
			    struct aw_spin_desc spin_desc, uint32_t spin_val)
{
	...
	ret = aw_dev->ops.aw_i2c_write_bits(
		aw_dev, rx_desc->reg, rx_desc->mask,
...
static int aw_reg_write_spin(struct aw_device *aw_dev, uint32_t spin_val,
			     bool mixer_en)
{
	int ret;
	...
	list_for_each (pos, dev_list) {
		...
		if ((g_spin_mode == AW_REG_MIXER_SPIN_MODE) && (mixer_en)) {
			ret = aw882xx_dsp_set_mixer_en(aw_dev,
						       AW_AUDIO_MIX_DSIABLE);
			if (ret)
				return ret;
		}
	}
...
int aw882xx_spin_set_record_val(struct aw_device *aw_dev)
{
	...
	if (g_spin_mode == AW_ADSP_SPIN_MODE) {
		...
		ret = aw_dev_set_channal_mode(aw_dev, aw_dev->spin_desc,
					      g_spin_value);
		if (ret) {
			mutex_unlock(&g_spin_lock);
			return ret;
...
int aw882xx_spin_value_get(struct aw_device *aw_dev, uint32_t *spin_val,
			   bool pstream)
{
	int ret = 0;
...
int aw882xx_spin_value_set(struct aw_device *aw_dev, uint32_t spin_val,
			   bool pstream)
{
	int ret = 0;
	...
	if (pstream) {
		...
		if ((g_spin_mode == AW_REG_SPIN_MODE) ||
			...
			aw_dev_info(aw_dev->dev,
				    "stream no start only record spin angle");
	}
...
static int aw_parse_spin_table_dt(struct aw_device *aw_dev)
{
	...
	ret = of_property_read_string(aw_dev->dev->of_node, "spin-data",
				      &str_data);
	if (ret < 0) {
		aw_dev_err(aw_dev->dev,
			   "get spin_data failed, close spin function");
		return ret;
	}

	ret = sscanf(str_data, "%c %c %c %c", &spin_table_str[AW_SPIN_0],
		     &spin_table_str[AW_SPIN_90], &spin_table_str[AW_SPIN_180],
		     &spin_table_str[AW_SPIN_270]);
	if (ret != AW_SPIN_MAX) {
		aw_dev_err(aw_dev->dev,
			   "unsupported str:%s, close spin function", str_data);
		return -EINVAL;
	}

	for (i = 0; i < AW_SPIN_MAX; i++) {
		if (spin_table_str[i] == 'l' || spin_table_str[i] == 'L') {
			spin_desc->spin_table[i].rx_val =
				spin_desc->rx_desc.left_val;
		} else if (spin_table_str[i] == 'r' ||
			   spin_table_str[i] == 'R') {
			spin_desc->spin_table[i].rx_val =
				spin_desc->rx_desc.right_val;
		} else {
			aw_dev_err(aw_dev->dev,
				   "unsupported str:%s, close spin function",
				   str_data);
			return -EINVAL;
		}
	}
...
static int aw_parse_spin_dts(struct aw_device *aw_dev)
{
	...
	ret = of_property_read_string(aw_dev->dev->of_node, "spin-mode",
				      &spin_str);
	if (ret < 0) {
		g_spin_mode = AW_SPIN_OFF_MODE;
		aw_dev_info(
			aw_dev->dev,
			"spin-mode get failed, spin switch off, spin_mode:%d",
			g_spin_mode);
		return 0;
	}
...
int aw882xx_spin_init(struct aw_spin_desc *spin_desc)
{
	...
	aw_dev_info(aw_dev->dev, "aw_spin_kcontrol_st:%d",
		    spin_desc->aw_spin_kcontrol_st);
