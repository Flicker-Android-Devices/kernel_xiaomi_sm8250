...
#define DBG(fmt, arg...)                                                       \
	do {                                                                   \
		printk("AWINIC_BIN %s,line= %d," fmt, __func__, __LINE__,      \
		       ##arg);                                                 \
	} while (0)
#define DBG_ERR(fmt, arg...)                                                   \
	do {                                                                   \
		printk("AWINIC_BIN_ERR %s,line= %d," fmt, __func__, __LINE__,  \
		       ##arg);                                                 \
	} while (0)
#else
#define DBG(fmt, arg...)                                                       \
...
#define BigLittleSwap16(A)                                                     \
	((((aw_uint16)(A) & 0xff00) >> 8) | (((aw_uint16)(A) & 0x00ff) << 8))

#define BigLittleSwap32(A)                                                     \
	((((aw_uint32)(A) & 0xff000000) >> 24) |                               \
...
int aw_check_soc_app_num_v1(struct aw_bin *bin, int bin_num)
{
	...
	p_check_sum =
		&(bin->info.data[(bin->header_info[bin_num].valid_data_addr)]);
	DBG("aw_bin_parse p_check_sum = %p\n", p_check_sum);
	bin->header_info[bin_num].app_version =
		GET_32_DATA(*(p_check_sum + 3), *(p_check_sum + 2),
			    *(p_check_sum + 1), *(p_check_sum));
	parse_soc_app_num =
		GET_32_DATA(*(p_check_sum + 11), *(p_check_sum + 10),
				      ...
				      struct aw_bin *bin)
{
	...
	if (!bin_serial_num) {
		bin_start_addr =
			GET_32_DATA(*(bin->p_addr + 67), *(bin->p_addr + 66),
...
static int aw_dev_parse_check_acf_by_hdr(struct aw_container *aw_cfg)
{
	...
	/*check data size*/
	cfg_dde = (struct aw_cfg_dde *)((char *)aw_cfg->data +
	...
	if (act_data != aw_cfg->len) {
		aw_pr_err("act_data[%d] not equal to file size[%d]!", act_data,
			  aw_cfg->len);
		return -EINVAL;
	}
	...
	for (i = 0; i < cfg_hdr->a_ddt_num; i++) {
		...
		if (end_data_offset > aw_cfg->len) {
			aw_pr_err(
				"a_ddt_num[%d] end_data_offset[%d] overflow file size[%d]",
				i, end_data_offset, aw_cfg->len);
			return -EINVAL;
		}

		/* crc check */
		act_crc8 = aw_dev_parse_crc8_check(
			aw_cfg->data + cfg_dde[i].data_offset,
			cfg_dde[i].data_size);
		if (act_crc8 != cfg_dde[i].data_crc) {
			aw_pr_err(
				"a_ddt_num[%d] crc8 check failed, act_crc8:0x%x != data_crc 0x%x",
	...
	aw_pr_info("version name [%d.%d.%d.%d]", cfg_hdr->a_version[3],
		   cfg_hdr->a_version[2], cfg_hdr->a_version[1],
		   cfg_hdr->a_version[0]);
...
static int aw_dev_parse_check_acf_by_hdr_v_1_0_0_0(struct aw_container *aw_cfg)
{
	...
	/*check data size*/
	cfg_dde = (struct aw_cfg_dde_v_1_0_0_0 *)((char *)aw_cfg->data +
						  cfg_hdr->a_hdr_offset);
	act_data += hdr_ddt_len;
	for (i = 0; i < cfg_hdr->a_ddt_num; i++)
		act_data += cfg_dde[i].data_size;
...
static int aw_dev_parse_raw_reg(struct aw_device *aw_dev, uint8_t *data,
				uint32_t data_len,
...
static int aw_dev_parse_raw_dsp(struct aw_device *aw_dev, uint8_t *data,
				uint32_t data_len,
				struct aw_prof_desc *prof_desc)
{
	aw_dev_info(aw_dev->dev, "data_size:%d enter", data_len);
...
static int aw_dev_parse_data_by_sec_type(struct aw_device *aw_dev,
					 ...
					 struct aw_cfg_dde *prof_hdr,
					 struct aw_prof_desc *scene_prof_desc)
{
	switch (prof_hdr->data_type) {
	case ACF_SEC_TYPE_REG:
		return aw_dev_parse_raw_reg(
			aw_dev, (uint8_t *)cfg_hdr + prof_hdr->data_offset,
			prof_hdr->data_size, scene_prof_desc);
	case ACF_SEC_TYPE_HDR_REG:
		return aw_dev_parse_reg_bin_with_hdr(
			aw_dev, (uint8_t *)cfg_hdr + prof_hdr->data_offset,
			prof_hdr->data_size, scene_prof_desc);
	case ACF_SEC_TYPE_MONITOR:
		return aw882xx_monitor_parse_fw(&aw_dev->monitor_desc,
						(uint8_t *)cfg_hdr +
							prof_hdr->data_offset,
						prof_hdr->data_size);
	}
				 ...
				 struct aw_all_prof_info *all_prof_info)
{
	...
	for (i = 0; i < prof_hdr->a_ddt_num; i++) {
		if ((aw_dev->i2c->adapter->nr == cfg_dde[i].dev_bus) &&
			...
			if (cfg_dde[i].data_type != ACF_SEC_TYPE_MONITOR) {
				ret = aw_dev_parse_data_by_sec_type(
					aw_dev, prof_hdr, &cfg_dde[i],
					&all_prof_info->prof_desc
						 [cfg_dde[i].dev_profile]);
				if (ret < 0) {
					aw_dev_err(
						aw_dev->dev,
						"parse dev driver bin data failed");
					return ret;
				}
				sec_num++;
			} else {
				ret = aw_dev_parse_data_by_sec_type(
					aw_dev, prof_hdr, &cfg_dde[i], NULL);
				if (ret < 0) {
					aw_dev_err(
						aw_dev->dev,
						"parse monitor bin data failed");
					return ret;
				}
				sec_num++;
			}
	...
	if (sec_num == 0) {
		aw_dev_info(aw_dev->dev,
			    "get dev type num is %d, please use default",
			    sec_num);
				       ...
				       struct aw_all_prof_info all_prof_info)
{
	...
	aw_dev_info(aw_dev->dev, "get vaild profile:%d",
		    aw_dev->prof_info.count);
	...
	for (i = 0; i < AW_PROFILE_MAX; i++) {
		if (prof_desc[i].prof_st == AW_PROFILE_OK) {
			if (num >= prof_info->count) {
				aw_dev_err(
					aw_dev->dev,
					"get scene num[%d] overflow count[%d]",
					num, prof_info->count);
				return -ENOMEM;
			}
				   ...
				   struct aw_cfg_hdr *prof_hdr)
{
	...
	if (ret < 0) {
		...
		aw_dev_info(aw_dev->dev,
			    "get dev type num is0, parse default dev type");
		ret = aw_dev_parse_dev_default_type(aw_dev, prof_hdr,
						    &all_prof_info);
		if (ret < 0)
			return ret;
	}
...
static int
aw_dev_parse_create_prof_name_list_v_1_0_0_0(struct aw_device *aw_dev)
{
	struct aw_prof_info *prof_info = &aw_dev->prof_info;
	...
	prof_info->prof_name_list = devm_kzalloc(
		aw_dev->dev, prof_info->count * PROFILE_STR_MAX, GFP_KERNEL);
					   ...
					   int *cur_scene_id)
{
	...
	case ACF_SEC_TYPE_REG:
		ret = aw_dev_parse_raw_reg(
			aw_dev, (uint8_t *)prof_hdr + cfg_dde->data_offset,
			cfg_dde->data_size,
			&prof_info->prof_desc[*cur_scene_id]);
		if (ret < 0) {
			aw_dev_err(aw_dev->dev, "parse reg bin failed");
			return ret;
		}
		prof_info->prof_desc[*cur_scene_id].prf_str =
					   ...
					   struct aw_cfg_hdr *prof_hdr)
{
	...
	for (i = 0; i < prof_hdr->a_ddt_num; i++) {
		if ((cfg_dde[i].type == AW_DEV_TYPE_ID) &&
		    ...
		    (aw_dev->i2c->addr == cfg_dde[i].dev_addr) &&
		    (aw_dev->chip_id == cfg_dde[i].chip_id)) {
			ret = aw_dev_parse_drv_type_v_1_0_0_0(
				aw_dev, prof_hdr, &cfg_dde[i], &cur_scene_id);
			if (ret < 0)
				return ret;
		}
	}
	...
	if (cur_scene_id == 0) {
		aw_dev_info(aw_dev->dev, "get dev type num is %d",
			    cur_scene_id);
		return -EINVAL;
	}
...
static int aw_dev_parse_get_scene_id_v_1_0_0_0(struct aw_device *aw_dev,
					       char *scene_str, int *scene_id)
{
	struct aw_prof_info *prof_info = &aw_dev->prof_info;
	int i;
					   ...
					   struct aw_cfg_hdr *prof_hdr)
{
	...
	for (i = 0; i < prof_hdr->a_ddt_num; i++) {
		if ((cfg_dde[i].type == AW_SKT_TYPE_ID) &&
		    ...
		    (aw_dev->channel == cfg_dde[i].dev_index) &&
		    (aw_dev->chip_id == cfg_dde[i].chip_id)) {
			ret = aw_dev_parse_get_scene_id_v_1_0_0_0(
				aw_dev, cfg_dde[i].dev_profile_str, &scene_id);
			if (ret < 0) {
...
static int aw_dev_parse_acf_by_hdr_v_1_0_0_0(struct aw_device *aw_dev,
					     struct aw_container *aw_cfg)
{
	struct aw_prof_info *prof_info = &aw_dev->prof_info;
	struct aw_cfg_hdr *cfg_hdr = (struct aw_cfg_hdr *)aw_cfg->data;
	...
	ret = aw_dev_parse_get_scene_count_v1_0_0_0(
		aw_dev, aw_cfg, &prof_info->count, &is_default);
	if (ret < 0) {
...
char *aw882xx_dev_get_prof_name(struct aw_device *aw_dev, int index)
{
	...
	if (index >= prof_info->count) {
		aw_dev_err(aw_dev->dev, "index[%d] overflow count[%d]", index,
			   aw_dev->prof_info.count);
		return NULL;
	}
...
struct aw_sec_data_desc *aw882xx_dev_get_prof_data(struct aw_device *aw_dev,
						   int index, int data_type)
{
	...
	if (data_type >= AW_PROFILE_DATA_TYPE_MAX) {
		aw_dev_err(aw_dev->dev, "unsupport data type id [%d]",
			   data_type);
		return NULL;
	}
...
int aw882xx_dev_set_profile_index(struct aw_device *aw_dev, int index)
{
	...
	if (index >= prof_info->count || index < 0) {
		...
		aw_dev_info(
			aw_dev->dev, "set prof[%s]",
			prof_info->prof_name_list[prof_info->prof_desc[index]
							  .id]);
	}
