...
static const char *const aw882xx_spin[] = { "spin_0", "spin_90", "spin_180",
					    "spin_270" };
...
int aw882xx_get_version(char *buf, int size)
{
	if (size > strlen(AW882XX_DRIVER_VERSION)) {
		memcpy(buf, AW882XX_DRIVER_VERSION,
		       strlen(AW882XX_DRIVER_VERSION));
...
static int aw882xx_i2c_writes(struct aw882xx *aw882xx, unsigned char reg_addr,
...
static int aw882xx_i2c_reads(struct aw882xx *aw882xx, unsigned char reg_addr,
...
int aw882xx_i2c_write(struct aw882xx *aw882xx, unsigned char reg_addr,
	...
	while (cnt < AW_I2C_RETRIES) {
		...
		if (ret < 0) {
			...
			if (g_print_dbg)
				aw_dev_info(
					aw882xx->dev,
					"reg_addr: 0x%02x, reg_data :0x%04x",
...
int aw882xx_i2c_read(struct aw882xx *aw882xx, unsigned char reg_addr,
		     unsigned int *reg_data)
{
	int ret = -1;
	unsigned char cnt = 0;
...
int aw882xx_i2c_write_bits(struct aw882xx *aw882xx, unsigned char reg_addr,
...
static int aw882xx_set_dai_sysclk(struct snd_soc_dai *dai, int clk_id,
			     ...
			     struct snd_soc_dai *dai)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_CAPTURE) {
		aw_dev_info(
			aw882xx->dev,
			"stream capture requested rate: %d, sample size: %d",
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		aw_dev_info(
			aw882xx->dev,
			"stream playback requested rate: %d, sample size: %d",
			params_rate(params), params_width(params));
	}
...
static void aw882xx_start_pa(struct aw882xx *aw882xx)
{
	...
	if (aw882xx->fw_status == AW_DEV_FW_OK) {
		...
		for (i = 0; i < AW_START_RETRIES; i++) {
			...
			ret = aw882xx_dev_reg_update(aw882xx->aw_pa,
						     aw882xx->phase_sync);
			if (ret) {
				aw_dev_err(aw882xx->dev,
					   "fw update failed, cnt:%d", i);
				continue;
			}

			ret = aw882xx_device_start(aw882xx->aw_pa);
			if (ret) {
				aw_dev_err(aw882xx->dev, "start failed, cnt:%d",
					   i);
				continue;
			} else {
				if (aw882xx->dc_flag)
			       ...
			       struct snd_ctl_elem_value *ucontrol)
{
	...
	ucontrol->value.integer.value[0] =
		aw882xx_dev_get_profile_index(aw882xx->aw_pa);
			       ...
			       struct snd_ctl_elem_value *ucontrol)
{
	...
	mutex_unlock(&aw882xx->lock);
	aw_dev_info(codec->dev, "prof id %d",
		    (int)ucontrol->value.integer.value[0]);
	return 0;
}
...
static int aw882xx_dynamic_create_controls(struct aw882xx *aw882xx)
{
	...
	aw882xx_dev_control =
		devm_kzalloc(aw882xx->codec->dev,
			     sizeof(struct snd_kcontrol_new) * AW_KCTL_NUM,
			     GFP_KERNEL);
	...
	kctl_name =
		devm_kzalloc(aw882xx->codec->dev, AW_NAME_BUF_MAX, GFP_KERNEL);
	if (!kctl_name)
		return -ENOMEM;

	snprintf(kctl_name, AW_NAME_BUF_MAX, "aw_dev_%d_prof",
		 aw882xx->aw_pa->channel);

	aw882xx_dev_control[KCTL_TYPE_PROFILE].name = kctl_name;
	aw882xx_dev_control[KCTL_TYPE_PROFILE].iface =
	...
	snprintf(kctl_name, AW_NAME_BUF_MAX, "aw_dev_%d_switch",
		 aw882xx->aw_pa->channel);

	aw882xx_dev_control[KCTL_TYPE_SWITCH].name = kctl_name;
	aw882xx_dev_control[KCTL_TYPE_SWITCH].iface =
		SNDRV_CTL_ELEM_IFACE_MIXER;
	...
	snprintf(kctl_name, AW_NAME_BUF_MAX, "aw_dev_%d_monitor",
		 aw882xx->aw_pa->channel);

	aw882xx_dev_control[KCTL_TYPE_MONITOR].name = kctl_name;
	aw882xx_dev_control[KCTL_TYPE_MONITOR].iface =
		SNDRV_CTL_ELEM_IFACE_MIXER;
	...
	snprintf(kctl_name, AW_NAME_BUF_MAX, "aw_dev_%d_volume",
		 aw882xx->aw_pa->channel);

	aw882xx_dev_control[KCTL_TYPE_VOLUME].name = kctl_name;
	aw882xx_dev_control[KCTL_TYPE_VOLUME].iface =
		SNDRV_CTL_ELEM_IFACE_MIXER;
	...
	aw_componet_codec_ops.add_codec_controls(
		aw882xx->codec, aw882xx_dev_control, AW_KCTL_NUM);
...
static void aw882xx_request_firmware(struct work_struct *work)
{
	...
	aw_dev_info(aw882xx->dev, "load [%s] , file size: [%zu]", ACF_BIN_NAME,
		    cont ? cont->size : 0);
	...
	if (g_awinic_cfg == NULL) {
		...
		if (ret) {
			aw_dev_err(aw882xx->dev, "Load [%s] failed ....!",
				   ACF_BIN_NAME);
			vfree(aw_cfg);
			aw_cfg = NULL;
			mutex_unlock(&g_aw882xx_lock);
		...
		aw_cfg = g_awinic_cfg;
		release_firmware(cont);
		aw_dev_info(aw882xx->dev, "[%s] already loaded...",
			    ACF_BIN_NAME);
	}
	mutex_unlock(&g_aw882xx_lock);
...
{
	struct aw882xx *aw882xx =
		container_of(work, struct aw882xx, start_work.work);
...
{
	struct aw882xx *aw882xx =
		container_of(work, struct aw882xx, dc_work.work);
	int dc_status = -1;
	...
	if (aw882xx->dc_flag) {
		if (dev_status) {
			...
			if (dc_status > 0) {
				...
				queue_delayed_work(
					aw882xx->work_queue, &aw882xx->dc_work,
					msecs_to_jiffies(
						AW882XX_DC_DELAY_TIME));
			}
		}
...
{
	struct aw882xx *aw882xx =
		container_of(work, struct aw882xx, interrupt_work.work);
	struct aw_device *aw_dev = aw882xx->aw_pa;
			    ...
			    struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ctrl_value >= ARRAY_SIZE(aw882xx_spin)) {
		aw_dev_err(aw_dev->dev, "spin value %d is unsupport",
			   ctrl_value);
		return -EINVAL;
	}
...
static void aw882xx_add_codec_controls(struct aw882xx *aw882xx)
{
	...
	if (aw882xx->aw_pa->spin_desc.aw_spin_kcontrol_st ==
	    AW_SPIN_KCONTROL_ENABLE)
		aw_componet_codec_ops.add_codec_controls(
			aw882xx->codec, aw882xx_spin_control,
			ARRAY_SIZE(aw882xx_spin_control));
}

static int aw882xx_append_i2c_suffix(char *format, const char **name,
				     struct aw882xx *aw882xx)
{
	char buf[64];
...
static int aw882xx_append_channel_suffix(char *format, const char **name,
					 struct aw882xx *aw882xx)
{
	char buf[64];
...
static const struct snd_soc_dapm_widget aw882xx_dapm_widgets[] = {
	/* playback */
	SND_SOC_DAPM_AIF_IN("AIF_RX", "Speaker_Playback", 0, SND_SOC_NOPM, 0,
			    0),
...
static void aw882xx_add_widgets(struct aw882xx *aw882xx)
{
	...
	struct snd_soc_dapm_context *dapm =
		snd_soc_component_get_dapm(aw882xx->codec);
#else
	struct snd_soc_dapm_context *dapm =
		snd_soc_codec_get_dapm(aw882xx->codec);
#endif
	...
	for (i = 0; i < ARRAY_SIZE(aw882xx_dapm_widgets); i++) {
		if (aw882xx->rename_flag == AW_RENAME_ENABLE) {
			if (aw_widgets[i].name) {
				ret = aw882xx_append_channel_suffix(
					"%s_%d", &aw_widgets[i].name, aw882xx);
				if (ret) {
					aw_dev_err(
						aw882xx->dev,
						"append widget name channel suffix failed!");
					return;
				}
			}
			...
			if (aw_widgets[i].sname) {
				ret = aw882xx_append_channel_suffix(
					"%s_%d", &aw_widgets[i].sname, aw882xx);
				if (ret) {
					aw_dev_err(
						aw882xx->dev,
						"append widget sname channel suffix failed!");
					return;
				}
			}
		} else {
			if (aw_widgets[i].name) {
				ret = aw882xx_append_i2c_suffix(
					"%s_%d_%x", &aw_widgets[i].name,
					aw882xx);
				if (ret) {
					aw_dev_err(
						aw882xx->dev,
						"append widget name i2c suffix failed!");
					return;
				}
			}
			...
			if (aw_widgets[i].sname) {
				ret = aw882xx_append_i2c_suffix(
					"%s_%d_%x", &aw_widgets[i].sname,
					aw882xx);
				if (ret) {
					aw_dev_err(
						aw882xx->dev,
						"append widget sname i2c suffix failed!");
					return;
				}
			}
		}
	...
	for (i = 0; i < ARRAY_SIZE(aw882xx_audio_map); i++) {
		if (aw882xx->rename_flag == AW_RENAME_ENABLE) {
			if (aw_route[i].sink) {
				ret = aw882xx_append_channel_suffix(
					"%s_%d", &aw_route[i].sink, aw882xx);
				if (ret < 0) {
					aw_dev_err(
						aw882xx->dev,
						"append sink channel suffix failed!");
					return;
				}
			}
			...
			if (aw_route[i].source) {
				ret = aw882xx_append_channel_suffix(
					"%s_%d", &aw_route[i].source, aw882xx);
				if (ret < 0) {
					aw_dev_err(
						aw882xx->dev,
						"append source channel suffix failed!");
					return;
				}
			}
		} else {
			if (aw_route[i].sink) {
				ret = aw882xx_append_i2c_suffix(
					"%s_%d_%x", &aw_route[i].sink, aw882xx);
				if (ret < 0) {
					aw_dev_err(
						aw882xx->dev,
						"append sink i2c suffix failed!");
					return;
				}
			}
			...
			if (aw_route[i].source) {
				ret = aw882xx_append_i2c_suffix(
					"%s_%d_%x", &aw_route[i].source,
					aw882xx);
				if (ret < 0) {
					aw_dev_err(
						aw882xx->dev,
						"append source i2c suffix failed!");
					return;
				}
			}
		}
...
#else
	queue_delayed_work(aw882xx->work_queue, &aw882xx->fw_work,
					 ...
					 int num_dai)
{
	...
	if ((dai_drv != NULL) && (num_dai > 0))
		for (i = 0; i < num_dai; i++) {
			if (aw882xx->rename_flag == AW_RENAME_ENABLE) {
				ret = aw882xx_append_channel_suffix(
					"%s-%d", &dai_drv->name, aw882xx);
				if (ret < 0)
					return ret;
				ret = aw882xx_append_channel_suffix(
					"%s_%d", &dai_drv->playback.stream_name,
					aw882xx);
				if (ret < 0)
					return ret;
				ret = aw882xx_append_channel_suffix(
					"%s_%d", &dai_drv->capture.stream_name,
					aw882xx);
				if (ret < 0)
					return ret;
			...
			aw_dev_info(aw882xx->dev, "dai name [%s]",
				    dai_drv[i].name);
			aw_dev_info(aw882xx->dev, "pstream_name name [%s]",
				    dai_drv[i].playback.stream_name);
			aw_dev_info(aw882xx->dev, "cstream_name name [%s]",
...
int aw_componet_codec_register(struct aw882xx *aw882xx)
{
	...
	aw882xx_dai_drv_append_suffix(aw882xx, dai_drv,
				      ARRAY_SIZE(aw882xx_dai));
	...
	if (aw882xx->rename_flag == AW_RENAME_ENABLE) {
		dev_set_name(aw882xx->dev, "%d-00%x", aw882xx->i2c->adapter->nr,
			     aw882xx->i2c->addr);
...
{
	struct aw882xx *aw882xx =
		devm_kzalloc(&i2c->dev, sizeof(struct aw882xx), GFP_KERNEL);
	if (aw882xx == NULL) {
			    ...
			    struct device_node *np)
{
	...
	if (ret < 0) {
		...
		aw_dev_info(aw882xx->dev, "sync flag is %d", sync_enable);
	}
...
static int aw882xx_read_chipid(struct aw882xx *aw882xx)
{
	...
	while (cnt < AW_READ_CHIPID_RETRIES) {
		ret = aw882xx_i2c_read(aw882xx, AW882XX_CHIP_ID_REG,
				       &reg_value);
		if (ret < 0) {
			aw_dev_err(aw882xx->dev, "failed to read REG_ID: %d",
				   ret);
			return -EIO;
		}
		switch (reg_value) {
...
static int aw882xx_awrw_write(struct aw882xx *aw882xx, const char *buf,
			      size_t count)
{
	int i, ret;
	char *data_buf = NULL;
...
static int aw882xx_awrw_parse_buf(struct aw882xx *aw882xx, const char *buf,
				  size_t count)
{
	int ret;
				 ...
				 struct device_attribute *attr, char *buf)
{
	...
	ret = aw882xx_i2c_reads(aw882xx, packet->reg_addr, (char *)reg_data,
				data_len);
	if (ret < 0) {
		ret = -EFAULT;
		goto exit;
				       ...
				       const char *buf, size_t count)
{
	...
	if (1 == sscanf(buf, "%d", &databuf[0])) {
		if (databuf[0] > (aw882xx->aw_pa->volume_desc.mute_volume)) {
			aw_dev_info(aw882xx->dev, "step overflow %d Db",
				    databuf[0]);
			return count;
		}
				      ...
				      struct device_attribute *attr, char *buf)
{
	...
	len += snprintf(buf + len, PAGE_SIZE - len, "step: %d \n",
				      ...
				      struct device_attribute *attr, char *buf)
{
	...
	len += snprintf(buf + len, PAGE_SIZE - len, "g_print_dbg : %d\n",
			g_print_dbg);
...
static DEVICE_ATTR(reg, S_IWUSR | S_IRUGO, aw882xx_reg_show, aw882xx_reg_store);
static DEVICE_ATTR(rw, S_IWUSR | S_IRUGO, aw882xx_rw_show, aw882xx_rw_store);
static DEVICE_ATTR(awrw, S_IWUSR | S_IRUGO, aw882xx_awrw_show,
		   aw882xx_awrw_store);
static DEVICE_ATTR(drv_ver, S_IRUGO, aw882xx_drv_ver_show, NULL);
static DEVICE_ATTR(dsp_re, S_IRUGO, aw882xx_dsp_re_show, NULL);
static DEVICE_ATTR(fade_step, S_IWUSR | S_IRUGO, aw882xx_fade_step_show,
		   aw882xx_fade_step_store);
static DEVICE_ATTR(dbg_prof, S_IWUSR | S_IRUGO, aw882xx_dbg_prof_show,
		   aw882xx_dbg_prof_store);
static DEVICE_ATTR(phase_sync, S_IWUSR | S_IRUGO, aw882xx_sync_flag_show,
		   aw882xx_sync_flag_store);
static DEVICE_ATTR(print_dbg, S_IWUSR | S_IRUGO, aw882xx_print_dbg_show,
		   aw882xx_print_dbg_store);
static DEVICE_ATTR(algo_ver, S_IRUGO, aw882xx_algo_ver_show, NULL);

static struct attribute *aw882xx_attributes[] = { &dev_attr_reg.attr,
						  &dev_attr_rw.attr,
						  &dev_attr_awrw.attr,
			     ...
			     const struct i2c_device_id *id)
{
	...
	case 1:
		register_kobj_under_mmsysfs(MM_HW_PA_1,
					    MM_HARDWARE_SYSFS_PA_1_FOLDER);
		break;
	case 2:
		register_kobj_under_mmsysfs(MM_HW_PA_2,
					    MM_HARDWARE_SYSFS_PA_2_FOLDER);
		break;
	case 3:
		register_kobj_under_mmsysfs(MM_HW_PA_3,
					    MM_HARDWARE_SYSFS_PA_3_FOLDER);
		break;
	case 4:
		register_kobj_under_mmsysfs(MM_HW_PA_4,
					    MM_HARDWARE_SYSFS_PA_4_FOLDER);
		break;
	case 5:
		register_kobj_under_mmsysfs(MM_HW_PA_5,
					    MM_HARDWARE_SYSFS_PA_5_FOLDER);
		break;
	case 6:
		register_kobj_under_mmsysfs(MM_HW_PA_6,
					    MM_HARDWARE_SYSFS_PA_6_FOLDER);
		break;
	case 7:
		register_kobj_under_mmsysfs(MM_HW_PA_7,
					    MM_HARDWARE_SYSFS_PA_7_FOLDER);
		break;
	case 8:
		register_kobj_under_mmsysfs(MM_HW_PA_8,
					    MM_HARDWARE_SYSFS_PA_8_FOLDER);
		break;
	default:
		break;
	}
...
static int aw882xx_i2c_remove(struct i2c_client *i2c)
{
	...
	if (gpio_to_irq(aw882xx->irq_gpio))
		devm_free_irq(&i2c->dev, gpio_to_irq(aw882xx->irq_gpio),
			      aw882xx);
...
static const struct i2c_device_id aw882xx_i2c_id[] = { { AW882XX_I2C_NAME, 0 },
						       {} };
