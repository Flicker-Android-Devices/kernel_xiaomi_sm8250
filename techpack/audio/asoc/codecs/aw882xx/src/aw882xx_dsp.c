...
extern int mtk_spk_recv_ipi_buf_from_dsp(int8_t *buffer, int16_t size,
					 uint32_t *buf_len);
#elif defined AW_QCOM_PLATFORM
extern int afe_get_topology(int port_id);
extern int aw_send_afe_cal_apr(uint32_t param_id, void *buf, int cmd_size,
			       bool write);
extern int aw_send_afe_rx_module_enable(void *buf, int size);
extern int aw_send_afe_tx_module_enable(void *buf, int size);
#else
static int aw_send_afe_cal_apr(uint32_t param_id, void *buf, int cmd_size,
			       bool write)
{
	return 0;
}
...
static int aw_mtk_write_data_to_dsp(int param_id, void *data, int size)
{
	...
	memcpy(((char *)dsp_data) + sizeof(aw_dsp_msg_t), data, size);
...
int aw882xx_get_algo_version(struct aw_device *aw_dev, char *algo_ver_buf)
{
	...
	if ((ret < 0) || (algo_ver == 0)) {
		...
		snprintf(algo_ver_buf, ALGO_VERSION_MAX, "aw_algo_v%d.%d.%d.%d",
			 algo_data[3], algo_data[2], algo_data[1],
			 algo_data[0]);
	}
...
void aw882xx_device_parse_topo_id_dt(struct aw_device *aw_dev)
{
	...
	if (ret < 0) {
		...
		aw_dev_info(aw_dev->dev,
			    "read aw-tx-topo-id failed,use default");
	}

	ret = of_property_read_u32(aw_dev->dev->of_node, "aw-rx-topo-id",
				   &g_rx_topo_id);
	if (ret < 0) {
...
void aw882xx_device_parse_port_id_dt(struct aw_device *aw_dev)
{
	...
	if (ret < 0) {
		...
		aw_dev_info(aw_dev->dev,
			    "read aw-tx-port-id failed,use default");
	}

	ret = of_property_read_u32(aw_dev->dev->of_node, "aw-rx-port-id",
				   &g_rx_port_id);
	if (ret < 0) {
