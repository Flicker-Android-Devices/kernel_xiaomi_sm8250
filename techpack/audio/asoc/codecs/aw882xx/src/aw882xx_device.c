...
static int aw_dev_get_icalk(struct aw_device *aw_dev, int16_t *icalk)
{
	...
	if (desc->icalkl_reg == AW_REG_NONE) {
		ret = aw_dev->ops.aw_i2c_read(aw_dev, desc->icalk_reg,
					      &reg_val);
		...
		ret = aw_dev->ops.aw_i2c_read(aw_dev, desc->icalk_reg,
					      &reg_val);
		reg_icalk = (uint16_t)reg_val & (~desc->icalk_reg_mask);
		ret = aw_dev->ops.aw_i2c_read(aw_dev, desc->icalkl_reg,
					      &reg_val);
		reg_icalkl = (uint16_t)reg_val & (~desc->icalkl_reg_mask);
		if (aw_dev->efuse_check == AW_EF_OR_CHECK)
			reg_icalk = (reg_icalk >> desc->icalk_shift) |
				    (reg_icalkl >> desc->icalkl_shift);
		else
			reg_icalk = (reg_icalk >> desc->icalk_shift) &
				    (reg_icalkl >> desc->icalkl_shift);
	}
...
static int aw_dev_get_vcalk(struct aw_device *aw_dev, int16_t *vcalk)
{
	...
	if (desc->vcalkl_reg == AW_REG_NONE) {
		ret = aw_dev->ops.aw_i2c_read(aw_dev, desc->vcalk_reg,
					      &reg_val);
		...
		ret = aw_dev->ops.aw_i2c_read(aw_dev, desc->vcalk_reg,
					      &reg_val);
		reg_vcalk = (uint16_t)reg_val & (~desc->vcalk_reg_mask);
		ret = aw_dev->ops.aw_i2c_read(aw_dev, desc->vcalkl_reg,
					      &reg_val);
		reg_vcalkl = (uint16_t)reg_val & (~desc->vcalkl_reg_mask);
		if (aw_dev->efuse_check == AW_EF_OR_CHECK)
			reg_vcalk = (reg_vcalk >> desc->vcalk_shift) |
				    (reg_vcalkl >> desc->vcalkl_shift);
		else
			reg_vcalk = (reg_vcalk >> desc->vcalk_shift) &
				    (reg_vcalkl >> desc->vcalkl_shift);
	}
...
static int aw_dev_reg_fw_update(struct aw_device *aw_dev)
{
	...
	reg_data = aw882xx_dev_get_prof_data(aw_dev, aw_dev->set_prof,
					     AW_PROFILE_DATA_TYPE_REG);
	if (reg_data == NULL)
		return -EINVAL;
	...
	for (i = 0; i < data_len; i += 2) {
		...
		if (reg_addr == aw_dev->amppd_desc.reg) {
			...
			aw_dev_info(aw_dev->dev, "amppd_st=0x%04x",
				    aw_dev->amppd_st);
			aw_dev->ops.aw_i2c_read(aw_dev, (unsigned char)reg_addr,
		...
		if (reg_addr == aw_dev->efcheck_desc.reg) {
			...
			aw_dev_info(aw_dev->dev, "efuse check: %d",
				    aw_dev->efuse_check);
		}

		if (reg_addr == work_mode->reg) {
			if ((reg_val & (~work_mode->mask)) ==
			    work_mode->rcv_val)
		...
		if (reg_addr == aw_dev->mute_desc.reg) {
			...
			aw_dev_info(aw_dev->dev, "mute_st=0x%04x",
				    aw_dev->mute_st);
			aw_dev->ops.aw_i2c_read(aw_dev, (unsigned char)reg_addr,
						(unsigned int *)&read_val);
		...
		ret = aw_dev->ops.aw_i2c_write(aw_dev, (unsigned char)reg_addr,
					       (unsigned int)reg_val);
		if (ret < 0)
			break;
	}
...
void aw882xx_dev_mute(struct aw_device *aw_dev, bool mute)
{
	...
	aw_dev_dbg(aw_dev->dev, "enter, mute: %d, cali_result: %d", mute,
...
int aw882xx_dev_get_int_status(struct aw_device *aw_dev, uint16_t *int_status)
{
	...
	ret = aw_dev->ops.aw_i2c_read(aw_dev, aw_dev->int_desc.st_reg,
				      &reg_val);
	if (ret < 0)
...
static int aw_dev_mode2_pll_check(struct aw_device *aw_dev)
{
	...
	aw_dev->ops.aw_i2c_write_bits(aw_dev, cco_mux_desc->reg,
				      cco_mux_desc->mask,
				      cco_mux_desc->divided_val);
	ret = aw_dev_mode1_pll_check(aw_dev);

	/* change mode1 */
	aw_dev->ops.aw_i2c_write_bits(aw_dev, cco_mux_desc->reg,
				      cco_mux_desc->mask,
				      cco_mux_desc->bypass_val);
	if (ret == 0) {
		usleep_range(AW_2000_US, AW_2000_US + 10);
		ret = aw_dev_mode1_pll_check(aw_dev);
	}
...
static int aw_dev_sysst_check(struct aw_device *aw_dev)
{
	...
	for (i = 0; i < AW_DEV_SYSST_CHECK_MAX; i++) {
		aw_dev->ops.aw_i2c_read(aw_dev, desc->reg, &reg_val);
		if (((reg_val & (~desc->mask)) & desc->st_check) ==
		    desc->st_check) {
			ret = 0;
			break;
		} else {
	...
	if (cali_desc->mode) {
		if (cali_desc->cali_re == AW_ERRO_CALI_VALUE) {
			ret = aw882xx_cali_read_re_from_nvram(
				&cali_desc->cali_re, aw_dev->channel);
			if (ret) {
				aw_dev_info(
					aw_dev->dev,
					"read nvram cali failed, use default Re");
...
void aw_dev_i2s_enable(struct aw_device *aw_dev, bool flag)
{
	...
	if (flag) {
		if (cali_desc->mode == AW_CALI_MODE_NONE)
			aw_dev->ops.aw_i2c_write_bits(aw_dev, txen_desc->reg,
						      txen_desc->mask,
						      txen_desc->reserve_val);
		else
			aw_dev->ops.aw_i2c_write_bits(aw_dev, txen_desc->reg,
						      txen_desc->mask,
						      txen_desc->enable);
...
static int aw_device_parse_sound_channel_dt(struct aw_device *aw_dev)
{
	...
	ret = of_property_read_u32(aw_dev->dev->of_node, "sound-channel",
				   &channel_value);
	if (ret < 0) {
		channel_value = AW_DEV_CH_PRI_L;
		aw_dev_info(aw_dev->dev,
			    "read sound-channel failed,use default");
	}
