...
static int aw_monitor_get_data_form_system(struct aw_device *aw_dev, int *data,
					   int data_type)
{
	int ret;
	char name[] = "battery";
	...
	if (psy) {
		...
		*data = prop.intval;
		...
		aw_dev_err(aw_dev->dev, "no struct power supply name : %s",
			   name);
		return -EINVAL;
	}
	return 0;
...
static int aw_monitor_get_voltage(struct aw_device *aw_dev, unsigned int *vol)
{
	...
	if (desc->reg == AW_REG_NONE) {
		ret = aw_monitor_get_data_form_system(
			aw_dev, (int *)vol, POWER_SUPPLY_PROP_VOLTAGE_NOW);
		if (ret) {
			aw_dev_err(aw_dev->dev,
				   "get voltage from system failed!");
			return ret;
		}
		*vol = (*vol) / 1000;
	} else {
...
static int aw_monitor_get_temperature(struct aw_device *aw_dev, int *temp)
{
	...
	if (desc->reg == AW_REG_NONE) {
		ret = aw_monitor_get_data_form_system(aw_dev, temp,
						      POWER_SUPPLY_PROP_TEMP);
		if (ret) {
			aw_dev_err(aw_dev->dev,
				   "get temperature from system failed!");
			return ret;
		}
		*temp = (*temp) / 10;
	} else {
...
static int
aw_monitor_first_get_data_form_table(struct aw_device *aw_dev,
					  ...
					  uint32_t aplha)
{
	...
	if (monitor->first_entry == AW_FIRST_ENTRY) {
		return aw_monitor_first_get_data_form_table(aw_dev, table_info,
							    data_trace);
	} else {
		data_trace->sum_val = data_trace->sum_val / monitor->samp_count;
		data_trace->sum_val =
			((int32_t)aplha * data_trace->sum_val +
			 (1000 - (int32_t)aplha) * data_trace->pre_val) /
			1000;
		return aw_monitor_trace_data_from_table(aw_dev, table_info,
							data_trace);
	}
...
static void aw_monitor_set_ipeak(struct aw_device *aw_dev, uint16_t ipeak)
{
	...
	aw_dev_info(aw_dev->dev, "set reg val = 0x%x, ipeak = 0x%x", reg_val,
		    ipeak);
}
...
static void aw_monitor_set_vmax(struct aw_device *aw_dev, uint32_t vmax)
{
	int ret;
	struct aw_monitor_cfg *monitor_cfg = &aw_dev->monitor_desc.monitor_cfg;
...
{
	struct aw_device *aw_dev = container_of(work, struct aw_device,
						monitor_desc.delay_work.work);
	struct aw882xx *aw882xx = (struct aw882xx *)aw_dev->private_data;
	struct aw_monitor_cfg *monitor_cfg = &aw_dev->monitor_desc.monitor_cfg;
	struct aw_monitor_desc *monitor = &aw_dev->monitor_desc;

	aw_dev_dbg(aw_dev->dev,
		   "monitor_start:%d, monitor_status:%d, monitor_switch:%d",
...
void aw882xx_monitor_start(struct aw_monitor_desc *monitor_desc)
{
	...
	if (aw_dev->bop_en == AW_BOP_ENABLE) {
		aw_dev_info(aw_dev->dev,
			    "bop status is enable, monitor can't start");
		return;
	}
...
static int aw_monitor_check_fw(struct aw_device *aw_dev, uint8_t *data,
			       uint32_t data_len)
{
	struct aw_monitor_hdr *monitor_hdr = (struct aw_monitor_hdr *)data;
	...
	if (monitor_hdr->temp_offset > data_len) {
		aw_dev_err(aw_dev->dev,
			   "temp_offset[%d] overflow file size[%d]!",
	...
	if (monitor_hdr->vol_offset > data_len) {
		aw_dev_err(aw_dev->dev,
			   "vol_offset[%d] overflow file size[%d]!",
	...
	if (temp_size > data_len) {
		aw_dev_err(aw_dev->dev,
			   "temp_size:[%d] overflow file size[%d]!", temp_size,
			   data_len);
		return -ENOMEM;
	}
...
static int aw_monitor_check_fw_v_0_1_1(struct aw_device *aw_dev, uint8_t *data,
				       uint32_t data_len)
{
...
static void aw_monitor_parse_hdr(struct aw_device *aw_dev, uint8_t *data)
{
	...
	aw_dev_info(aw_dev->dev, "chip name:%s", monitor_hdr->chip_type);
	aw_dev_info(aw_dev->dev, "ui ver:0x%x", monitor_hdr->ui_ver);
	...
	aw_dev_info(aw_dev->dev,
		    "ipeak_switch:%d, gain_switch:%d, vmax_switch:%d",
					 ...
					 uint8_t *data)
{
	...
	monitor_cfg->logic_switch =
		(monitor_hdr->enable_flag >> MONITOR_LOGIC_BIT) &
		MONITOR_EN_MASK;
	...
	monitor_cfg->vol_switch =
		(monitor_hdr->enable_flag >> MONITOR_VOL_EN_BIT) &
		MONITOR_EN_MASK;
	monitor_cfg->vol_aplha = monitor_hdr->vol_aplha;
...
static int aw_monitor_parse_temp_data_v_0_1_1(struct aw_device *aw_dev,
					      uint8_t *data)
{
	struct aw_monitor_hdr_v_0_1_1 *monitor_hdr =
		(struct aw_monitor_hdr_v_0_1_1 *)data;
...
static int aw_monitor_parse_vol_data_v_0_1_1(struct aw_device *aw_dev,
					     uint8_t *data)
{
	struct aw_monitor_hdr_v_0_1_1 *monitor_hdr =
		(struct aw_monitor_hdr_v_0_1_1 *)data;
...
static int aw_monitor_parse_data(struct aw_device *aw_dev, uint8_t *data,
				 uint32_t data_len)
{
	int ret;
	struct aw_monitor_cfg *monitor_cfg = &aw_dev->monitor_desc.monitor_cfg;
	...
	if (ret < 0) {
		if (monitor_cfg->temp_info.aw_table != NULL) {
			devm_kfree(aw_dev->dev,
				   monitor_cfg->temp_info.aw_table);
			     ...
			     uint8_t *data, uint32_t data_len)
{
	...
	aw_dev = container_of(monitor_desc, struct aw_device, monitor_desc);
...
static int aw_monitor_real_time_update_monitor(struct aw_device *aw_dev)
{
	...
	if (ret < 0) {
		aw_dev_err(aw_dev->dev, "failed to read %s",
			   aw_dev->monitor_name);
			   ...
			   char *buf)
{
	...
	len += snprintf(buf + len, PAGE_SIZE - len, "vol: %d\n",
			    ...
			    char *buf)
{
	...
	len += snprintf(buf + len, PAGE_SIZE - len, "aw882xx temp: %d\n",
			       ...
			       struct device_attribute *attr, char *buf)
{
	...
	len += snprintf(buf + len, PAGE_SIZE - len, "monitor enable: %d\n",
