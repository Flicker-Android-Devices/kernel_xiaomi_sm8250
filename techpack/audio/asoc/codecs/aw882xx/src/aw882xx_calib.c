...
static const char *cali_str[CALI_STR_MAX] = {
	...
	"show_re",   "show_r0",	    "show_cali_f0", "show_f0",	   "show_te",
	"show_st",   "dev_sel",	    "get_ver",	    "get_dev_num", "cali_q_f0",
	"show_q_f0", "get_re_range"
};
...
/* write cali to data for miui */
#ifndef CONFIG_FACTORY_BUILD
#define AWINIC_CALI_FILE_MIUI "/data/vendor/cit/aw_cali.bin"
#endif
...
static int aw_cali_write_cali_re_to_file(int32_t cali_re, int channel)
{
	...
	loff_t pos = 0;
	mm_segment_t fs;
	char cali_file[128] = { 0 };
#ifdef CONFIG_FACTORY_BUILD
	snprintf(cali_file, sizeof(cali_file), AWINIC_CALI_FILE);
#else
	snprintf(cali_file, sizeof(cali_file), AWINIC_CALI_FILE_MIUI);
#endif
	pr_debug("%s: cali file path is %s\n", __func__, cali_file);
...
int aw_cali_store_cali_re(struct aw_device *aw_dev, int32_t re)
{
	...
	if ((re < aw_dev->re_min) || (re > aw_dev->re_max)) {
		aw_dev_info(aw_dev->dev, "re[%d] out of range {%d, %d}", re,
...
static int aw_cali_svc_dev_get_re(struct aw_device *aw_dev)
{
	...
	for (i = 0; i < AW_CALI_READ_RE_TIMES; i++) {
		...
		if (ret) {
			...
			if (aw_dev->cali_desc.cali_check_st) {
				aw_dev->cali_desc.cali_result =
					CALI_RESULT_ERROR;
				aw_cali_svc_run_mute(
					aw_dev, aw_dev->cali_desc.cali_result);
			}
			return ret;
		}
		sum += re[i];
...
static int aw_cali_svc_dev_cali_mode_en(struct aw_device *aw_dev, int type,
					bool is_enable, unsigned int flag)
{
	int ret;
	...
	if (is_enable) {
		...
		if (type == CALI_TYPE_RE) {
			...
			ret = aw882xx_dsp_cali_en(aw_dev,
					       ...
					       unsigned int data_type)
{
	...
	list_for_each (pos, dev_list) {
		...
		if (data_type == CALI_DATA_RE) {
			...
			aw_dev_err(local_dev->dev,
				   "unsupport cali data type :%d", data_type);
			return -EINVAL;
		}
		if (ret < 0) {
			aw_dev_err(local_dev->dev,
				   "get cali data type[%d] failed", data_type);
			return ret;
		}
	}
					     ...
					     bool is_enable)
{
	...
	list_for_each (pos, dev_list) {
		...
		ret = aw_cali_svc_dev_cali_mode_en(local_dev, type, is_enable,
						   flag);
		if (ret < 0)
			return ret;
...
static int aw_cali_svc_devs_cali_re(struct aw_device *aw_dev, unsigned int flag)
{
	...
	ret = aw_cali_svc_devs_cali_mode_enable(dev_list, CALI_TYPE_RE, flag,
						true);
	if (ret < 0)
		goto exit;
...
static int aw_cali_svc_devs_cali_f0(struct aw_device *aw_dev, unsigned int flag)
{
	...
	ret = aw_cali_svc_devs_cali_mode_enable(dev_list, CALI_TYPE_F0, flag,
						true);
	if (ret < 0)
		goto exit;
...
static int aw_cali_svc_cali_f0(struct aw_device *aw_dev, bool is_single,
			       unsigned int flag)
{
	if (is_single)
...
static int aw_cali_svc_cali_re_f0(struct aw_device *aw_dev, bool is_single,
				  unsigned int flag)
{
	int ret;
...
int aw_cali_svc_cali_cmd(struct aw_device *aw_dev, int cali_cmd, bool is_single,
			 unsigned int flag)
{
	switch (cali_cmd) {
	case AW_CALI_CMD_RE:
...
int aw_cali_svc_get_devs_cali_re(struct aw_device *aw_dev, int32_t *re_buf,
				 int num)
{
	int ret, cnt = 0;
	struct list_head *dev_list = NULL;
...
int aw_cali_svc_get_devs_r0(struct aw_device *aw_dev, int32_t *re_buf, int num)
{
	...
	list_for_each (pos, dev_list) {
		local_dev = container_of(pos, struct aw_device, list_node);
		if (local_dev->channel < num) {
			ret = aw882xx_dsp_read_r0(local_dev,
						  &re_buf[local_dev->channel]);
			if (ret) {
...
int aw_cali_svc_get_devs_te(struct aw_device *aw_dev, int32_t *te_buf, int num)
{
	...
	list_for_each (pos, dev_list) {
		local_dev = container_of(pos, struct aw_device, list_node);
		if (local_dev->channel < num) {
			ret = aw882xx_dsp_read_te(local_dev,
...
int aw_cali_svc_get_devs_st(struct aw_device *aw_dev, int32_t *st_buf, int num)
{
	...
	list_for_each (pos, dev_list) {
		local_dev = container_of(pos, struct aw_device, list_node);
		if (local_dev->channel < num) {
			ret = aw882xx_dsp_read_st(
				local_dev, &st_buf[local_dev->channel << 1],
				&st_buf[(local_dev->channel << 1) + 1]);
			if (ret) {
				aw_dev_err(local_dev->dev, "get re failed!");
				return ret;
			}
...
int aw_cali_svc_get_devs_cali_f0(struct aw_device *aw_dev, int32_t *f0_buf,
				 int num)
{
	int ret, cnt = 0;
	struct list_head *dev_list = NULL;
...
int aw_cali_svc_get_devs_f0(struct aw_device *aw_dev, int32_t *f0_buf, int num)
{
	...
	list_for_each (pos, dev_list) {
		local_dev = container_of(pos, struct aw_device, list_node);
		if (local_dev->channel < num) {
			ret = aw882xx_dsp_read_f0(local_dev,
						  &f0_buf[local_dev->channel]);
			if (ret) {
				aw_dev_err(local_dev->dev, "get re failed!");
				return ret;
...
int aw_cali_svc_get_devs_cali_f0_q(struct aw_device *aw_dev, int32_t *f0_buf,
				   int32_t *q_buf, int num)
{
	int ret, cnt = 0;
	struct list_head *dev_list = NULL;
	struct list_head *pos = NULL;
				       ...
				       const char *re_str)
{
	...
	ret = sscanf(
		...
		&re_data[AW_DEV_CH_PRI_L], &re_data[AW_DEV_CH_PRI_R],
		&re_data[AW_DEV_CH_SEC_L], &re_data[AW_DEV_CH_SEC_R],
		&re_data[AW_DEV_CH_TERT_L], &re_data[AW_DEV_CH_TERT_R],
		&re_data[AW_DEV_CH_QUAT_L], &re_data[AW_DEV_CH_QUAT_R]);
	...
	list_for_each (pos, dev_list) {
		...
		if (local_dev->channel < AW_DEV_CH_MAX) {
			ret = aw_cali_store_cali_re(
				local_dev, re_data[local_dev->channel]);
			if (ret < 0)
				return ret;
			cnt++;
		}
				      ...
				      struct device_attribute *attr, char *buf)
{
	...
	len += snprintf(buf + len, PAGE_SIZE - len, "time: %d \n",
			g_cali_re_time);
				  ...
				  const char *buf, size_t count)
{
	...
	if (ret == CALI_STR_CALI_RE_F0) {
		aw_cali_svc_cali_cmd(aw_dev, AW_CALI_CMD_RE_F0, is_single_cali,
				     CALI_OPS_HMUTE | CALI_OPS_NOISE);
		return count;
	} else if (ret == CALI_STR_CALI_RE) {
		aw_cali_svc_cali_cmd(aw_dev, AW_CALI_CMD_RE, is_single_cali,
				     CALI_OPS_HMUTE);
		return count;
	} else if (ret == CALI_STR_CALI_F0) {
		aw_cali_svc_cali_cmd(aw_dev, AW_CALI_CMD_F0, is_single_cali,
				     CALI_OPS_NOISE);
		return count;
	} else {
		aw_dev_err(aw_dev->dev, "supported cmd [%s]!", buf);
				    ...
				    struct device_attribute *attr, char *buf)
{
	...
	ret = aw_cali_svc_cali_cmd(aw_dev, AW_CALI_CMD_RE, is_single_cali,
				   CALI_OPS_HMUTE);
	if (ret < 0) {
	...
	if (is_single_cali) {
		...
		ret = aw_cali_svc_get_devs_cali_re(aw_dev, cali_re,
						   AW_DEV_CH_MAX);
		if (ret <= 0) {
			aw_dev_err(aw_dev->dev, "get re failed ");
			len += snprintf(buf + len, PAGE_SIZE - len,
				    ...
				    struct device_attribute *attr, char *buf)
{
	...
	ret = aw_cali_svc_cali_cmd(aw_dev, AW_CALI_CMD_F0, is_single_cali,
				   CALI_OPS_NOISE);
	if (ret < 0) {
		len += snprintf(buf + len, PAGE_SIZE - len,
				"cali f0 cmd failed\n");
	...
	if (is_single_cali) {
		...
		ret = aw_cali_svc_get_devs_cali_f0(aw_dev, cali_f0,
						   AW_DEV_CH_MAX);
		if (ret <= 0) {
			aw_dev_err(aw_dev->dev, "get f0 failed ");
			len += snprintf(buf + len, PAGE_SIZE - len,
...
/*set cali time*/
static DEVICE_ATTR(cali_time, S_IWUSR | S_IRUGO, aw_cali_attr_time_show,
		   aw_cali_attr_time_store);
/*start cali*/
static DEVICE_ATTR(cali, S_IWUSR, NULL, aw_cali_attr_store);
/*cali re*/
static DEVICE_ATTR(cali_re, S_IRUGO | S_IWUSR, aw_cali_attr_re_show,
		   aw_cali_attr_re_store);
/*cali_f0*/
static DEVICE_ATTR(cali_f0, S_IRUGO | S_IWUSR, aw_cali_attr_f0_show,
		   aw_cali_attr_f0_store);
/*show cali_re*/
static DEVICE_ATTR(re_show, S_IRUGO, aw_cali_attr_show_re, NULL);
/*show cali_f0*/
static DEVICE_ATTR(f0_show, S_IRUGO, aw_cali_attr_show_f0, NULL);
/*show re_range*/
static DEVICE_ATTR(re_range, S_IRUGO, aw_re_range_show, NULL);

static struct attribute *aw_cali_attr[] = {
	&dev_attr_cali_time.attr, &dev_attr_cali.attr,
	&dev_attr_cali_re.attr,	  &dev_attr_cali_f0.attr,
	&dev_attr_re_show.attr,	  &dev_attr_f0_show.attr,
	&dev_attr_re_range.attr,  NULL
};

static struct attribute_group aw_cali_attr_group = { .attrs = aw_cali_attr };
...
/*****************************class node******************************************************/
static ssize_t aw_cali_class_time_show(struct class *class,
					  ...
					  char *buf)
{
	...
	ret = aw_cali_svc_cali_cmd(local_dev, AW_CALI_CMD_RE, false,
				   CALI_OPS_HMUTE);
	if (ret < 0) {
		len += snprintf(buf + len, PAGE_SIZE - len,
				"cali Re cmd failed\n");
					  ...
					  char *buf)
{
	...
	ret = aw_cali_svc_cali_cmd(local_dev, AW_CALI_CMD_F0, false,
				   CALI_OPS_NOISE);
	if (ret < 0) {
		len += snprintf(buf + len, PAGE_SIZE - len,
				"cali f0 cmd failed\n");
...
static ssize_t aw_cali_class_f0_show(struct class *class,
				     struct class_attribute *attr, char *buf)
{
	int ret, i;
	struct list_head *dev_list = NULL;
...
static ssize_t aw_cali_class_re_show(struct class *class,
				     struct class_attribute *attr, char *buf)
{
	int ret, i;
	struct list_head *dev_list = NULL;
...
static ssize_t aw_class_re_range_show(struct class *class,
				      struct class_attribute *attr, char *buf)
{
	int ret, i;
	ssize_t len = 0;
	...
	for (i = 0; i < ret; i++) {
		len += snprintf(buf + len, PAGE_SIZE - len,
				"%s:re_min:%d mOhms re_max:%d mOhms ",
				ch_name[i],
...
static struct class_attribute class_attr_f0_calib =
	__ATTR(f0_calib, S_IWUSR | S_IRUGO, aw_cali_class_cali_f0_show,
	       aw_cali_class_cali_f0_store);

static struct class_attribute class_attr_re_show =
	__ATTR(re_show, S_IRUGO, aw_cali_class_re_show, NULL);

static struct class_attribute class_attr_f0_show =
	__ATTR(f0_show, S_IRUGO, aw_cali_class_f0_show, NULL);
...
static int aw_cali_misc_open(struct inode *inode, struct file *file)
{
	...
	if (local_dev->channel != g_dev_select) {
		local_dev =
			list_first_entry(dev_list, struct aw_device, list_node);
		aw_dev_dbg(local_dev->dev,
			   "can not find dev num %d, use default",
			   g_dev_select);
	}
...
static int aw_cali_misc_params_ptr(struct aw_device *aw_dev,
				  ...
				  unsigned long arg)
{
	...
	case AW_IOCTL_SET_PTR_PARAM_NUM: {
		ret = aw_cali_misc_params_ptr(
			aw_dev, (struct ptr_params_data *)data_ptr);
	} break;
				 ...
				 unsigned long arg)
{
	...
	case AW_IOCTL_GET_F0_Q: {
		data_32_ptr = (int32_t *)data_ptr;
		ret = aw882xx_dsp_read_f0_q(aw_dev, &data_32_ptr[0],
					    &data_32_ptr[1]);
	} break;
...
static int aw_cali_misc_read_dsp(struct aw_device *aw_dev, aw_ioctl_msg_t *msg)
{
	...
	if (copy_to_user((void __user *)user_data, data_ptr, data_len))
		ret = -EFAULT;
exit:
	kfree(data_ptr);
	return ret;
...
static int aw_cali_misc_ops_msg(struct aw_device *aw_dev, unsigned long arg)
{
	...
	if (copy_from_user(&ioctl_msg, (void __user *)arg,
			   sizeof(aw_ioctl_msg_t)))
		return -EFAULT;

	if (ioctl_msg.version != AW_IOCTL_MSG_VERSION) {
		aw_dev_err(aw_dev->dev, "unsupported msg version %d",
			   ioctl_msg.version);
		return -EINVAL;
	}
	...
	if (ioctl_msg.type == AW_IOCTL_MSG_RD_DSP) {
		...
		return aw_cali_misc_ops(aw_dev, ioctl_msg.opcode_id,
					(unsigned long)ioctl_msg.data_buf);
	} else {
		aw_dev_err(aw_dev->dev, "unsupported msg type %d",
			   ioctl_msg.type);
		return -EINVAL;
	}
}
				 ...
				 size_t size, loff_t *pos)
{
	...
	case CALI_STR_SHOW_RE: {
		ret = aw_cali_svc_get_devs_cali_re(aw_dev, temp_data,
						   AW_DEV_CH_MAX);
		if (ret <= 0) {
	...
	case CALI_STR_SHOW_CALI_F0: {
		ret = aw_cali_svc_get_devs_cali_f0(aw_dev, temp_data,
						   AW_DEV_CH_MAX);
		if (ret <= 0) {
	...
	case CALI_STR_DEV_NUM: {
		if (aw_dev->ops.aw_get_dev_num) {
			...
			len += snprintf(local_buf + len,
					sizeof(local_buf) - len, "dev_num:%d\n",
					dev_num);
		} else {
	...
	case CALI_STR_SHOW_F0_Q: {
		ret = aw_cali_svc_get_devs_cali_f0_q(aw_dev, temp_data,
						     temp_data1, AW_DEV_CH_MAX);
		if (ret <= 0) {
			...
			for (i = 0; i < ret; i++)
				len += snprintf(local_buf + len,
						sizeof(local_buf) - len,
						"%s:f0:%d q:%d", ch_name[i],
						temp_data[i], temp_data1[i]);
		...
		if (ret <= 0) {
			...
			for (i = 0; i < ret; i++)
				len += snprintf(
					local_buf + len,
					sizeof(local_buf) - len,
					"%s:re_min:%d re_max:%d ", ch_name[i],
					re_value[RE_MIN_FLAG + i * RE_RANGE_NUM],
					re_value[RE_MAX_FLAG +
						 i * RE_RANGE_NUM]);
				   ...
				   char *cmd_buf)
{
	...
	list_for_each (pos, dev_list) {
		...
		if (local_dev->channel == i) {
			...
			g_dev_select = i;
			aw_dev_info(local_dev->dev, "switch dev to [%s]",
				    ch_name[i]);
			return 0;
		}
	}
				  ...
				  size_t size, loff_t *pos)
{
	...
	case CALI_STR_SET_RE: {
		/*skip store_re*/
		kernel_buf[size] = '\0';
		ret = aw_cali_svc_set_devs_re_str(
			aw_dev,
...
int aw_cali_parse_re_dt(struct aw_device *aw_dev)
{
	...
	ret = of_property_read_u32(aw_dev->dev->of_node, "aw-re-min",
				   &aw_dev->re_min);
	if (ret < 0) {
	...
	ret = of_property_read_u32(aw_dev->dev->of_node, "aw-re-max",
				   &aw_dev->re_max);
	if (ret < 0) {
	...
	aw_dev_info(aw_dev->dev, "re min: %d, re max: %d", aw_dev->re_min,
		    aw_dev->re_max);
	return 0;
}
