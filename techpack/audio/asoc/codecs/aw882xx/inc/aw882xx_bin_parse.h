...
struct aw_bin {
	unsigned char *
		p_addr; /* Offset pointer (backward offset pointer to obtain frame header information and important information) */
...
int aw882xx_dev_parse_acf(struct aw_device *aw_dev,
			  struct aw_container *aw_cfg);

int aw882xx_dev_get_profile_count(struct aw_device *aw_dev);
int aw88xx_dev_get_profile_name(struct aw_device *aw_dev, char *name,
				int index);
