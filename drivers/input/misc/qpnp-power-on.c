...
static void collect_d_work_func(struct work_struct *work)
{
	...
	volp_scan = !pmic_gpio_get_external(
		"c440000.qcom,spmi:qcom,pm8150@0:pinctrl@c000", 2);
	...
	if (cfg != NULL) {
		if (volp_scan == 1) {
			...
			qpnp_pon_masked_write(pon, cfg->s2_cntl2_addr,
					      ...
					      QPNP_PON_S2_CNTL_EN);
			dev_info(pon->dev,
				 "3-combo-keys detected, enable s2 reset\n");
		} else {
			// disable
			qpnp_pon_masked_write(pon, cfg->s2_cntl2_addr,
					      QPNP_PON_S2_CNTL_EN, 0);
			dev_info(pon->dev,
				 "volp key not detected, disable s2 reset\n");
			goto err_return;
		}
	}
	...
	if ((pon_rt_sts & QPNP_PON_KPDPWR_RESIN_N_SET) ==
	    QPNP_PON_KPDPWR_RESIN_N_SET) {
		console_verbose();
		dev_info(
			pon->dev,
			"------ collect D&R-state processes info before long comb key ------\n");
		show_state_filter_single(TASK_UNINTERRUPTIBLE);
		show_state_filter_single(TASK_RUNNING);
		dev_info(
			pon->dev,
			"------ end collecting D&R-state processes info ------\n");
...
static irqreturn_t qpnp_kpdpwr_bark_irq(int irq, void *_pon)
{
	...
	dev_info(pon->dev, "Enter in kpdpwr irq !");
