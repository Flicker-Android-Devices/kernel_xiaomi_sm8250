...
static void ln8282_enter_switching(struct ln8282_info *info)
{
	...
	if (info->reverse_power) {
		...
		//   when VIN droops
		msleep(100);
		regmap_write(info->regmap, LN8282_REG_FAULT_CTRL,
			     0x0C); //+DISABLE_VIN_SWITCH_OK=1
	}
}
...
int ln8282_hw_init(struct ln8282_info *info)
{
	...
	regmap_update_bits(info->regmap, LN8282_REG_HV_SC_CTRL_1, 0x10,
			   (1 << 4)); //SC_OUT_MAX_OV_CFG=1 (20V threshold)
	regmap_update_bits(
		info->regmap, LN8282_REG_TRACK_CTRL, 0x04,
		(1
		 << 2)); //DISABLE_VIN_UV_TRACK=1 (side-effect of raising 11V->20V)
#endif
