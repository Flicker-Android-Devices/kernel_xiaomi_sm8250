			     ...
			     int *slave_icl_ua, int *total_settled_icl_ua)
{
	...
	if (!get_effective_result_locked(chip->pl_disable_votable)) {
		...
		rc = power_supply_get_property(
			chip->main_psy, POWER_SUPPLY_PROP_INPUT_CURRENT_SETTLED,
			&pval);
		if (rc < 0) {
	...
	if (total_current_ua < 0) {
		...
		rc = power_supply_get_property(
			chip->usb_psy, POWER_SUPPLY_PROP_HW_CURRENT_MAX, &pval);
		if (rc < 0) {
...
static void split_settled(struct pl_data *chip)
{
	...
	if (slave_ua > chip->pl_settled_ua) {
		...
		rc = power_supply_set_property(
			chip->pl_psy, POWER_SUPPLY_PROP_CURRENT_MAX, &pval);
		if (rc < 0) {
...
static ssize_t passthrough_limit_show(struct class *c,
...
static ssize_t passthrough_limit_store(struct class *c,
...
static struct class_attribute class_attr_passthrough_limit =
	__ATTR(passthrough_limit, 0644, passthrough_limit_show,
	       passthrough_limit_store);

static ssize_t passthrough_dis_show(struct class *c,
				    struct class_attribute *attr, char *ubuf)
{
	struct pl_data *chip = container_of(c, struct pl_data, qcom_batt_class);
...
static ssize_t passthrough_dis_store(struct class *c,
				     struct class_attribute *attr,
				     const char *ubuf, size_t count)
{
	struct pl_data *chip = container_of(c, struct pl_data, qcom_batt_class);
...
static struct class_attribute class_attr_passthrough_dis = __ATTR(
	passthrough_dis, 0644, passthrough_dis_show, passthrough_dis_store);
				  ...
				  const char *ubuf, size_t count)
{
	...
	vote(chip->fcc_votable, RESTRICT_CHG_VOTER,
	     chip->restricted_charging_enabled, chip->restricted_current);
 ...
 * CELL BATT VOLT *
 ****************************/
static ssize_t cell1_volt_show(struct class *c, struct class_attribute *attr,
			       char *ubuf)
{
	struct pl_data *chip = container_of(c, struct pl_data, qcom_batt_class);
...
static ssize_t cell2_volt_show(struct class *c, struct class_attribute *attr,
			       char *ubuf)
{
	struct pl_data *chip = container_of(c, struct pl_data, qcom_batt_class);
...
static struct attribute *batt_class_attrs[] = {
	...
	[FCC_STEPPING_IN_PROGRESS] = &class_attr_fcc_stepping_in_progress.attr,
	[CELL1_VOLT] = &class_attr_cell1_volt.attr,
	[CELL2_VOLT] = &class_attr_cell2_volt.attr,
	[PASSTHROUGH_LIMIT] = &class_attr_passthrough_limit.attr,
	[PASSTHROUGH_DIS] = &class_attr_passthrough_dis.attr,
	NULL,
};
...
#define STEP_UP 1
#define STEP_DOWN -1
static void get_fcc_split(struct pl_data *chip, int total_ua, int *master_ua,
			  int *slave_ua)
{
	int rc, effective_total_ua, slave_limited_ua,
		hw_cc_delta_ua = 0, icl_ua, adapter_uv, bcl_ua;
	union power_supply_propval pval = {
		0,
	};
	...
	if (chip->pl_mode == POWER_SUPPLY_PL_USBMID_USBMID) {
		...
		rc = power_supply_get_property(
			chip->main_psy, POWER_SUPPLY_PROP_INPUT_VOLTAGE_SETTLED,
			&pval);
		if (rc < 0) {
	...
	if (chip->pl_batfet_mode == POWER_SUPPLY_PL_STACKED_BATFET) {
		*master_ua = max(0, total_ua);
		if (chip->main_fcc_max)
			*master_ua =
				min(*master_ua, chip->main_fcc_max + *slave_ua);
	} else {
				   ...
				   int parallel_fcc_ua)
{
	...
	if (is_override_vote_enabled_locked(chip->fcc_main_votable)) {
		...
		if ((main_set_fcc_ua != chip->override_main_fcc_ua) ||
			...
			parallel_fcc_ua =
				(total_fcc_ua - chip->override_main_fcc_ua);
		} else {
	...
	chip->main_step_fcc_dir =
		(main_fcc_ua > chip->main_fcc_ua) ? STEP_UP : STEP_DOWN;
	...
	chip->parallel_step_fcc_dir =
		(parallel_fcc_ua > chip->slave_fcc_ua) ? STEP_UP : STEP_DOWN;
...
skip_fcc_step_update:
	...
	pr_debug(
		"Main FCC Stepper parameters: main_step_direction: %d, main_step_count: %d, main_residual_fcc: %d\n",
		chip->main_step_fcc_dir, chip->main_step_fcc_count,
		chip->main_step_fcc_residual);
	pr_debug(
		"Parallel FCC Stepper parameters: parallel_step_direction: %d, parallel_step_count: %d, parallel_residual_fcc: %d\n",
...
{
	struct pl_data *chip =
		container_of(work, struct pl_data, pl_taper_work);
	union power_supply_propval pval = {
		0,
	};
	int rc;
	...
	while (true) {
		if (get_effective_result(chip->pl_disable_votable)) {
			...
			if (slave_fcc_ua <= MINIMUM_PARALLEL_FCC_UA) {
				pl_dbg(chip, PR_PARALLEL,
				       "terminating: parallel's share is low\n");
				vote(chip->pl_disable_votable, TAPER_END_VOTER,
				     true, 0);
			} else {
				pl_dbg(chip, PR_PARALLEL,
				       "terminating: parallel disabled\n");
			}
			goto done;
		}
		...
		if (get_effective_result(chip->fv_votable) >
		    chip->taper_entry_fv) {
			pl_dbg(chip, PR_PARALLEL,
			       "Float voltage increased. Exiting taper\n");
			goto done;
		} else {
			chip->taper_entry_fv =
		...
		if (pval.intval == POWER_SUPPLY_CHARGE_TYPE_TAPER) {
			...
			pl_dbg(chip, PR_PARALLEL,
			       ...
			       fcc_ua);
			vote(chip->fcc_votable, TAPER_STEPPER_VOTER, true,
			     fcc_ua);
		} else {
			pl_dbg(chip, PR_PARALLEL,
				     ...
				     int fcc_main_ua, const char *client)
{
	...
	return power_supply_set_property(
		chip->main_psy, POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT_MAX,
		&pval);
}
				...
				int total_fcc_ua, const char *client)
{
	...
	if (chip->usb_psy) {
		rc = power_supply_get_property(
			chip->usb_psy, POWER_SUPPLY_PROP_PASSTHROUGH_CURR_MAX,
			&pval);
		if (rc < 0)
			...
			passthrough_limit =
				min(chip->passthrough_limit, pval.intval);
			if (!passthrough_limit)
				passthrough_limit =
					PASSTHROUGH_FCC_THRESHOLD_UA;
		}
	}

	if (chip->cp_master_psy) {
		rc = power_supply_get_property(
			chip->cp_master_psy, POWER_SUPPLY_PROP_MIN_ICL, &pval);
		if (rc < 0)
	...
	pl_dbg(chip, PR_DEBUG,
	       "cp_fcc_ua:%d, total_fcc_ua:%d, main_fcc_ua:%d, cp_reason:%d\n",
	...
	if (chip->pl_mode != POWER_SUPPLY_PL_NONE) {
		...
		if (slave_fcc_ua > MINIMUM_PARALLEL_FCC_UA) {
			vote(chip->pl_disable_votable, PL_FCC_LOW_VOTER, false,
			     0);
		} else {
			vote(chip->pl_disable_votable, PL_FCC_LOW_VOTER, true,
			     0);
		}
	}
	...
	if (total_fcc_ua <= passthrough_limit &&
	    (total_fcc_ua > fcc_thr_ua * 5) && !chip->passthrough_dis) {
		/*
		 * Enter Passthrough if FCC
		 * falls below 5.5A threshold.
		 */
		if (chip->passthrough_mode_disable_votable) {
			vote(chip->passthrough_mode_disable_votable, FCC_VOTER,
			     false, 0);
		}
	} else {
		if (chip->passthrough_mode_disable_votable)
...
{
	struct pl_data *chip =
		container_of(work, struct pl_data, fcc_stepper_work.work);
	union power_supply_propval pval = {
		0,
	};
	...
	if (!charger_present) {
		...
		if (chip->pl_psy) {
			pval.intval = 1;
			rc = power_supply_set_property(
				chip->pl_psy, POWER_SUPPLY_PROP_INPUT_SUSPEND,
				&pval);
			if (rc < 0) {
				pr_err("Couldn't change slave suspend state rc=%d\n",
				       rc);
	...
	if (parallel_fcc < chip->slave_fcc_ua) {
		...
		if (chip->pl_psy) {
			 ...
			 */
			if (chip->pl_disable &&
			    parallel_fcc >= MINIMUM_PARALLEL_FCC_UA) {
				pval.intval = 0;
				rc = power_supply_set_property(
					chip->pl_psy,
					POWER_SUPPLY_PROP_INPUT_SUSPEND, &pval);
...
static int dynamic_recharge_vbat(struct pl_data *chip, int fv_uv)
{
	...
	rc = power_supply_get_property(chip->batt_psy, POWER_SUPPLY_PROP_TEMP,
				       &val);
	if (rc < 0) {
...
static int pl_fv_vote_callback(struct votable *votable, void *data, int fv_uv,
			       const char *client)
{
	struct pl_data *chip = data;
	union power_supply_propval pval = {
	...
	if (chip->pl_mode != POWER_SUPPLY_PL_NONE) {
		...
		rc = power_supply_set_property(
			chip->pl_psy, POWER_SUPPLY_PROP_VOLTAGE_MAX, &pval);
		if (rc < 0) {
				 ...
				 int icl_ua, const char *client)
{
	...
	rc = power_supply_get_property(
		chip->main_psy, POWER_SUPPLY_PROP_INPUT_CURRENT_SETTLED, &pval);
	if (rc < 0) {
		pr_err("Couldn't get aicl settled value rc=%d\n", rc);
		return rc;
	}
	...
	pval.intval = icl_ua;
	power_supply_set_property(chip->main_psy, POWER_SUPPLY_PROP_CURRENT_MAX,
				  &pval);
...
{
	struct pl_data *chip =
		container_of(work, struct pl_data, pl_disable_forever_work);
...
static int pl_disable_vote_callback(struct votable *votable, void *data,
				    int pl_disable, const char *client)
{
	...
	rc = power_supply_get_property(
		chip->batt_psy, POWER_SUPPLY_PROP_FCC_STEPPER_ENABLE, &pval);
	if (rc < 0) {
	...
	if (chip->pl_mode != POWER_SUPPLY_PL_NONE && !pl_disable) {
		...
		rc = power_supply_get_property(
			chip->pl_psy, POWER_SUPPLY_PROP_CHARGE_TYPE, &pval);
		if (rc == -ENODEV) {
			/*
		...
		pl_dbg(chip, PR_PARALLEL,
		       "master_fcc=%d slave_fcc=%d distribution=(%d/%d)\n",
		...
		if (!chip->fcc_stepper_enable) {
			...
			if (cp_ilim > 0) {
				...
				if (chip->passthrough_mode_disable_votable) {
					pl_dbg(chip, PR_PARALLEL,
					       "cp_ilim:%d, chip->passthrough_limit:%d\n",
					       cp_ilim,
					       chip->passthrough_limit);
					cp_configure_ilim(
						chip, FCC_VOTER,
...
static int pl_enable_indirect_vote_callback(struct votable *votable, void *data,
					    int pl_enable, const char *client)
{
	struct pl_data *chip = data;
...
static int pl_awake_vote_callback(struct votable *votable, void *data,
...
static bool is_parallel_available(struct pl_data *chip)
{
	...
	if (IS_USBIN(chip->pl_mode) &&
		...
		if (chip->hvdcp_hw_inov_dis_votable)
			...
			vote(chip->hvdcp_hw_inov_dis_votable, PL_VOTER, true,
			     0);
		else
			return false;
	}
	...
	rc = power_supply_get_property(
		chip->pl_psy, POWER_SUPPLY_PROP_PARALLEL_BATFET_MODE, &pval);
	if (rc < 0) {
	...
	rc = power_supply_get_property(
		chip->pl_psy, POWER_SUPPLY_PROP_PARALLEL_FCC_MAX, &pval);
	if (!rc)
	...
	*/
	...
	if (pval.intval == POWER_SUPPLY_CHARGE_TYPE_TAPER ||
		...
		if (pval.intval == POWER_SUPPLY_CHARGE_TYPE_FAST &&
		    (chip->taper_entry_fv <
		     get_effective_result(chip->fv_votable))) {
			vote(chip->pl_disable_votable, TAPER_END_VOTER, false,
			     0);
		}
...
static void handle_settled_icl_change(struct pl_data *chip)
{
	...
	rc = power_supply_get_property(
		chip->batt_psy, POWER_SUPPLY_PROP_INPUT_CURRENT_LIMITED, &pval);
	if (rc < 0) {
		pr_err("Couldn't get aicl settled value rc=%d\n", rc);
		return;
	}
	...
	if ((main_limited &&
	     (main_settled_ua + chip->pl_settled_ua) < 1400000) ||
	    (main_settled_ua == 0) ||
	    ((total_current_ua >= 0) && (total_current_ua <= 1400000)))
		vote(chip->pl_enable_votable_indirect, USBIN_I_VOTER, false, 0);
	else
	...
	if (IS_USBIN(chip->pl_mode)) {
		...
		if (abs(new_total_settled_ua - chip->total_settled_ua) >
			...
			vote(chip->pl_disable_votable, ICL_LIMIT_VOTER, disable,
			     0);
...
static void handle_parallel_in_taper(struct pl_data *chip)
{
	 ...
	 */
	if (pval.intval == POWER_SUPPLY_CHARGE_TYPE_TAPER) {
		vote(chip->pl_disable_votable, PL_TAPER_EARLY_BAD_VOTER, true,
		     0);
		return;
	}
}
...
static void handle_usb_change(struct pl_data *chip)
{
	...
	if (!pval.intval) {
		...
		vote(chip->pl_disable_votable, PL_TAPER_EARLY_BAD_VOTER, false,
		     0);
...
{
	struct pl_data *chip =
		container_of(work, struct pl_data, status_change_work.work);
...
static int pl_notifier_call(struct notifier_block *nb, unsigned long ev,
			    void *v)
{
...
int qcom_batt_init(struct charger_param *chg_param)
{
	...
	chip->fcc_main_votable = create_votable(
		"FCC_MAIN", VOTE_MIN, pl_fcc_main_vote_callback, chip);
	...
	chip->fcc_votable =
		create_votable("FCC", VOTE_MIN, pl_fcc_vote_callback, chip);
	...
	chip->fv_votable =
		create_votable("FV", VOTE_MIN, pl_fv_vote_callback, chip);
	...
	chip->pl_disable_votable = create_votable(
		"PL_DISABLE", VOTE_SET_ANY, pl_disable_vote_callback, chip);
	...
	chip->pl_enable_votable_indirect =
		create_votable("PL_ENABLE_INDIRECT", VOTE_SET_ANY,
			       pl_enable_indirect_vote_callback, chip);
