...
static int fg_mac_read_block(struct bq_fg_chip *bq, u16 cmd, u8 *buf, u8 len)
{
	...
	t_buf[0] = (u8)cmd;
	t_buf[1] = (u8)(cmd >> 8);
	ret = __fg_write_block(bq->client, bq->regs[BQ_FG_REG_ALT_MAC], t_buf,
			       2);
	if (ret < 0) {
		mutex_unlock(&bq->i2c_rw_lock);
		return ret;
	...
	ret = __fg_read_block(bq->client, bq->regs[BQ_FG_REG_ALT_MAC], t_buf,
			      36);
	if (ret < 0) {
		mutex_unlock(&bq->i2c_rw_lock);
		return ret;
...
static int fg_disable_sleep_mode(struct bq_fg_chip *bq)
{
	...
	if (static_df == BQ28Z610_NEED_UPDATE_FW1 ||
	    static_df == BQ28Z610_NEED_UPDATE_FW2) {
	...
	if (raw_soc > 9600) {
		...
		if (raw_soc == 0 && bq->last_soc > 1) {
			...
			for (i = SMOOTH_VOLT_LEN; i > 0; i--) {
				if (volt >
				    lowsoc_highvolt_smooth[i - 1].volt_value) {
					unit_time =
						lowsoc_highvolt_smooth[i - 1]
							.unit_time;
					break;
				}
			}
...
static const u8 fg_dump_regs[] = {
	0x00, 0x02, 0x04, 0x06, 0x08, 0x0A, 0x0C, 0x0E, 0x10, 0x16,
	0x18, 0x1A, 0x1C, 0x1E, 0x20, 0x28, 0x2A, 0x2C, 0x2E, 0x30,
};
...
update:
	...
	bq_dbg(PR_OEM, "cell1:%d,cell2:%d,delta:%d,temp:%d,curr:%d\n",
	       bq->cell1, bq->cell2, bq->cell_delta, bq->batt_temp,
	       bq->batt_curr);
				      ...
				      struct device_attribute *attr, char *buf)
{
	...
	ret = fg_mac_read_block(bq, FG_MAC_CMD_ITSTATUS1, t_buf,
				ITSTATUS1_DATA_LEN);
	if (ret < 0)
		return 0;
				      ...
				      struct device_attribute *attr, char *buf)
{
	...
	ret = fg_mac_read_block(bq, FG_MAC_CMD_ITSTATUS2, t_buf,
				ITSTATUS2_DATA_LEN);
	if (ret < 0)
		return 0;
				      ...
				      struct device_attribute *attr, char *buf)
{
	...
	ret = fg_mac_read_block(bq, FG_MAC_CMD_ITSTATUS3, t_buf,
				ITSTATUS3_DATA_LEN);
	if (ret < 0)
		return 0;
				     ...
				     struct device_attribute *attr, char *buf)
{
	...
	ret = fg_mac_read_block(bq, FG_MAC_CMD_CBSTATUS, t_buf,
				CBSTATUS_DATA_LEN);
	if (ret < 0)
		return 0;
...
static struct attribute *fg_attributes[] = {
	&dev_attr_Qmax.attr,	  &dev_attr_fcc_soh.attr,
	&dev_attr_rm.attr,	  &dev_attr_fcc.attr,
	&dev_attr_soh.attr,	  &dev_attr_verify_digest.attr,
	&dev_attr_batt_volt.attr, &dev_attr_itstatus1.attr,
	&dev_attr_itstatus2.attr, &dev_attr_itstatus3.attr,
	&dev_attr_cbstatus.attr,  NULL,
};
...
static int fg_check_full_status(struct bq_fg_chip *bq)
{
	...
	if (bq->usb_present && bq->raw_soc == BQ_RAW_SOC_FULL &&
	    bq->batt_volt > full_volt && bq->batt_curr < 0 &&
	    (bq->batt_curr > term_curr * (-1)) && !bq->batt_sw_fc) {
		full_check++;
		...
		if (full_check > BAT_FULL_CHECK_TIME) {
			...
			bq_dbg(PR_OEM,
			       "detect charge termination bq->batt_sw_fc:%d\n",
			       bq->batt_sw_fc);
		}
	...
	if (bq->bbc_psy) {
		...
		rc = power_supply_set_property(
			bq->bbc_psy, POWER_SUPPLY_PROP_TERMINATION_CURRENT,
			&prop);
		if (rc < 0) {
...
static void fg_update_status(struct bq_fg_chip *bq)
{
	...
	bq_dbg(PR_OEM,
	       "SOC:%d,Volt:%d,Cur:%d,Temp:%d,RM:%d,FC:%d, RAW_SOC:%d,SOH:%d,"
	       "FAST:%d,DR1:%d,Cell1:%d,Cell2:%d,Delta:%d",
	       bq->batt_soc, bq->batt_volt, bq->batt_curr, bq->batt_temp,
	       bq->batt_rm, bq->batt_fcc, bq->raw_soc, bq->soh, bq->fast_mode,
	       bq->soc_decimal_rate, bq->cell1, bq->cell2, bq->cell_delta);
...
#define HW_ONE_PERCENT_RAW_SOC                                                 \
	((HW_REPORT_FULL_SOC - FG_REPORT_FULL_SOC) * 100 /                     \
					  ...
					  int batt_ma)
{
	...
	if (bq->ffc_smooth) {
		...
		if (batt_soc == last_batt_soc) {
			bq_dbg(PR_DEBUG,
			       "batt_soc:%d, last_batt_soc:%d, is samed set ffc smooth to false\n",
		...
		if (status != last_status) {
			if (last_status == POWER_SUPPLY_STATUS_CHARGING &&
				...
				last_change_time = ktime_get();
			bq_dbg(PR_DEBUG,
			       "update last_statu:%d, last_change_time:%ld\n",
			       status, last_change_time);
			last_status = status;
		}
	}
	rc = power_supply_get_property(
		bq->usb_psy, POWER_SUPPLY_PROP_QUICK_CHARGE_TYPE, &pval);
	if (rc < 0) {
		bq_dbg(PR_OEM, "could not get quick charge type\n");
		return -EINVAL;
	}

	if (pval.intval == QUICK_CHARGE_SUPER &&
	    raw_soc >= FG_REPORT_FULL_SOC && raw_soc != FG_RAW_SOC_FULL) {
		bq_dbg(PR_DEBUG,
		       "raw_soc:%d, last_raw_soc:%d HW_ONE_PERCENT_RAW_SOC:%d optimiz_soc:%d\n",
		       raw_soc, last_raw_soc, HW_ONE_PERCENT_RAW_SOC,
		       optimiz_soc);
	...
	if (last_batt_soc < batt_soc && batt_ma < 0) {
		 ...
		 */
		last_batt_soc =
			bq->update_now ? batt_soc : last_batt_soc + soc_changed;
	...
	bq_dbg(PR_OEM,
	       "avg:%d ma:%d, changed:%d update:%d smooth:%d "
	       "optimiz:%d soc:%d system:%d last:%d time:%d",
	       batt_ma_avg, batt_ma, soc_changed, bq->update_now,
	       bq->ffc_smooth, optimiz_soc, batt_soc, system_soc, last_batt_soc,
	       unit_time);
...
static int bq_parse_dt(struct bq_fg_chip *bq)
{
	...
	ret = of_property_read_u32(node, "bq,cold-term", &bq->cold_term);
	if (ret < 0) {
		bq_dbg(PR_OEM, "failed to get cold term curr\n");
		bq->cold_term = DEFUALT_TERM_CURR;
	}

	ret = of_property_read_u32(node, "bq,normal-term", &bq->normal_term);
	if (ret < 0) {
...
static int bq27z561_notifier_call(struct notifier_block *nb, unsigned long ev,
				  void *v)
{
	...
	if (bq->usb_psy) {
		rc = power_supply_get_property(
			bq->usb_psy, POWER_SUPPLY_PROP_PRESENT, &pval);
		if (rc < 0) {
