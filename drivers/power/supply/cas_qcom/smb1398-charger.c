...
static const struct ovp_configs int_ovp_levels_uv[] = {
	[0] = { .voltage_uv = 7200000, .reg_config = 0xA0 },
	[1] = { .voltage_uv = 13900000, .reg_config = 0x80 },
	[2] = { .voltage_uv = 17100000, .reg_config = 0x20 },
	[3] = { .voltage_uv = 22150000, .reg_config = 0x00 },
};

static const struct ovp_configs ext_ovp_levels_uv[] = {
	[0] = { .voltage_uv = 7300000, .reg_config = 0xC0 },
	[1] = { .voltage_uv = 14000000, .reg_config = 0x40 },
	[2] = { .voltage_uv = 17700000, .reg_config = 0x00 },
	[3] = { .voltage_uv = 22200000, .reg_config = 0x80 },
};
...
static int smb1398_masked_write(struct smb1398_chip *chip, u16 reg, u8 mask,
				u8 val)
{
	int rc = 0;
	...
	if (rc < 0)
		dev_err(chip->dev,
			"Update register 0x%x to 0x%x with mask 0x%x failed, rc=%d\n",
			reg, val, mask, rc);
...
static int smb1398_div2_cp_get_status1(struct smb1398_chip *chip, u8 *status)
{
	int rc = 0;
	u8 val;
...
static int smb1398_div2_cp_get_status2(struct smb1398_chip *chip, u8 *status)
{
	int rc = 0;
	u8 val;
	...
	*status = smb_en << 7 | vin_ov << 6 | vin_uv << 5 | irev << 3 |
		  tsd << 2 | switcher_off;
...
static int smb1398_div2_cp_get_irq_status(struct smb1398_chip *chip, u8 *status)
{
	int rc = 0;
	u8 val;
	...
	rc = power_supply_get_property(
		chip->usb_psy, POWER_SUPPLY_PROP_ADAPTER_CC_MODE, &pval);
	if (rc < 0) {
...
static int smb1398_set_ovlo(struct smb1398_chip *chip, int ovlo)
{
	...
	rc = smb1398_masked_write(chip, USBIN_OVP_THRESHOLD_REG, USBIN_OVP_MASK,
				  ovlo);
	if (rc < 0) {
		dev_err(chip->dev,
			"write USBIN_OVP_THRESHOLD_REG failed, rc=%d\n", rc);
		return rc;
	}
	...
	if (rc < 0) {
		dev_err(chip->dev,
			"write USBIN_OVLO_THRESHOLD_REG failed, rc=%d\n", rc);
		return rc;
	}
	rc = smb1398_masked_write(chip, USBIN_OVLO_THRESHOLD_REG,
...
static int smb1398_div2_cp_switcher_en(struct smb1398_chip *chip, bool en)
{
	...
	rc = smb1398_masked_write(chip, MISC_SL_SWITCH_EN_REG, EN_SWITCHER,
				  en ? EN_SWITCHER : 0);
	if (rc < 0) {
...
static int smb1398_get_1s_vout_mv(struct smb1398_chip *chip, int *vout_mv)
{
	...
	*vout_mv =
		(val & VOUT_DAC_VALUE_MASK) * VOUT_1S_STEP_MV + VOUT_1S_MIN_MV;
...
static int smb1398_div2_cp_isns_mode_control(struct smb1398_chip *chip,
					     enum isns_mode mode)
{
	int rc = 0;
	u8 mux_sel;
...
static int smb1398_div2_cp_get_master_isns(struct smb1398_chip *chip,
					   int *isns_ua)
{
	union power_supply_propval pval = { 0 };
	int rc = 0, temp;
	...
	if (is_cps_available(chip)) {
		...
		rc = power_supply_set_property(
			chip->div2_cp_slave_psy,
			POWER_SUPPLY_PROP_CURRENT_CAPABILITY, &pval);
		if (rc < 0) {
			dev_err(chip->dev,
				"Couldn't set slave ISNS_MODE_OFF, rc=%d\n",
				rc);
			goto unlock;
		}
	}
	...
	if (rc < 0) {
		dev_err(chip->dev,
			"Couldn't set master ISNS_MODE_ACTIVE, rc=%d\n", rc);
		goto unlock;
	}
	...
	if (rc < 0) {
		dev_err(chip->dev,
			"Couldn't set master ISNS_MODE_STANDBY, rc=%d\n", rc);
		goto unlock;
	}
...
static int smb1398_div2_cp_get_slave_isns(struct smb1398_chip *chip,
					  int *isns_ua)
{
	union power_supply_propval pval = { 0 };
	int temp = 0, rc;
	...
	if (rc < 0) {
		dev_err(chip->dev,
			"Set master ISNS_MODE_STANDBY failed, rc=%d\n", rc);
		goto unlock;
	}
unlock:
	mutex_unlock(&chip->die_chan_lock);
...
static int smb1398_div2_cp_config_ovp(struct smb1398_chip *chip)
{
	...
	for (i = 0; i < 4; i++) {
		if (int_ovp_levels_uv[i].voltage_uv > target_ovp_uv) {
			rc = smb1398_masked_write(
				chip, SSUPPLY_CFG0,
				EN_HV_OV_OPTION2 | EN_MV_OV_OPTION,
				int_ovp_levels_uv[i].reg_config);
			if (rc < 0) {
				dev_err(chip->dev,
					"Write SSUPPLY_CFG0 failed, rc=%d\n",
					rc);
				return rc;
			}
			dev_dbg(chip->dev,
				"Master OVP Configured: internal OVP=%d mV\n",
	...
	for (i = 0; i < 4; i++) {
		if (ext_ovp_levels_uv[i].voltage_uv > target_ovp_uv) {
			rc = smb1398_masked_write(
				chip, PERPH1_LOCK_SPARE, EXT_OVPFET_MASK,
				ext_ovp_levels_uv[i].reg_config);
			if (rc < 0) {
				dev_err(chip->dev,
					"Write PERPH1_LOCK_SPARE failed, rc=%d\n",
					rc);
				return rc;
			}
			dev_dbg(chip->dev,
				"Master OVP Configured: external OVP=%d mV\n",
	...
	if (is_cps_available(chip)) {
		...
		if (rc < 0) {
			dev_err(chip->dev,
				"Failed to configure slave OVP, rc=%d\n", rc);
			return rc;
		}
	}
...
static int smb1398_div2_cp_slave_config_ovp(struct smb1398_chip *chip)
{
	...
	if (rc < 0) {
		dev_err(chip->dev,
			"Read Slave passthrough mode bit failed, rc=%d\n", rc);
		return rc;
	}
	in_passthrough = in_passthrough & CFG_ITLGS_2P_BIT;
	...
	for (i = 0; i < 4; i++) {
		if (int_ovp_levels_uv[i].voltage_uv > target_ovp_uv) {
			...
			if (rc < 0) {
				dev_err(chip->dev,
					"Write Slave SSUPPLY_CFG0 failed, rc=%d\n",
					rc);
				return rc;
			}
			dev_dbg(chip->dev,
				"Slave OVP Configured: internal OVP=%d mV\n",
				int_ovp_levels_uv[i].voltage_uv / 1000);
			break;
		}
	}
...
static int smb1398_set_ocp(struct smb1398_chip *chip, bool enable)
{
	...
	rc = smb1398_masked_write(chip, CFG_3LVL_BK_CURRENT, CFG_HSON_SEL,
				  enable ? CFG_HSON_SEL : 0);
...
static int smb1398_passthrough_exit(struct smb1398_chip *chip)
{
	...
	rc = smb1398_masked_write(chip, SDCDC_1, EN_CHECK_CFLY_SSDONE_BIT,
				  EN_CHECK_CFLY_SSDONE_BIT);
	rc = smb1398_masked_write(chip, MISC_CFG0_REG, CFG_ITLGS_2P_BIT,
				  0); //Disable Passthrough
	if (is_cps_available(chip)) {
		pval.intval = 0;
		rc = power_supply_set_property(
			chip->div2_cp_slave_psy,
...
static int smb1398_charge_get_wireless_adapter_type(struct smb1398_chip *chip,
						    int *adpater_type);
				   ...
				   union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CP_WIN_OV:
		...
		if (rc < 0) {
			dev_err(chip->dev,
				"Read passthrough mode bit failed, rc=%d\n",
				rc);
			return rc;
		}
		in_passthrough = in_passthrough & CFG_ITLGS_2P_BIT;
				   ...
				   const union power_supply_propval *val)
{
	...
	switch (prop) {
	case POWER_SUPPLY_PROP_CP_ENABLE:
		vote(chip->div2_cp_disable_votable, USER_VOTER, !val->intval,
		     0);
		break;
	case POWER_SUPPLY_PROP_CP_TOGGLE_SWITCHER:
		if (!!val->intval)
	...
	case POWER_SUPPLY_PROP_RESET_DIV_2_MODE:
		...
		break;
	case POWER_SUPPLY_PROP_CP_PASSTHROUGH_MODE:
		vote(chip->passthrough_mode_disable_votable, USER_VOTER,
		     !val->intval, 0);
		break;
	case POWER_SUPPLY_PROP_CP_PASSTHROUGH_CONFIG:
		dev_info(
			chip->dev,
			"set POWER_SUPPLY_PROP_CP_PASSTHROUGH_CONFIG to val->intval:%d\n",
			val->intval);
		if (chip->passthrough_mode_allowed &&
		    (val->intval > 0 ? (smb1398_is_rev3(chip) ||
					is_vbus_ok_for_passthrough(chip)) :
				       val->intval == 0)) {
			if (val->intval == 1) {
				rc = smb1398_div2_cp_config_ovp(chip);
				rc = smb1398_masked_write(
					chip, MISC_CFG0_REG, CFG_ITLGS_2P_BIT,
					0); //Disable Passthrough bit before enabling SMB
			}
			vote(chip->smb_override_votable, PASSTHROUGH_VOTER,
			     !val->intval, 0);
			vote(chip->div2_cp_disable_votable, PASSTHROUGH_VOTER,
			     !val->intval, 0);
			if (val->intval == 0) {
				smb1398_masked_write(chip, PERPH0_MISC_CFG0,
						     CFG_PRE_REG_2S, 0);
				vote(chip->div2_cp_slave_disable_votable,
				     DELAY_OPEN_VOTER, true, 0);
				...
				rc = smb1398_masked_write(chip,
							  CFG_3LVL_BK_CURRENT,
							  CFG_HSON_SEL,
							  0); //Disable OCP
				rc = smb1398_masked_write(
					chip, SDCDC_1, EN_CHECK_CFLY_SSDONE_BIT,
					0);
				rc = smb1398_masked_write(
					chip, MISC_CFG0_REG, CFG_ITLGS_2P_BIT,
					CFG_ITLGS_2P_BIT); //Enable Passthrough
				if (is_cps_available(chip)) {
					pval.intval = 1;
					rc = power_supply_set_property(
						chip->div2_cp_slave_psy,
				...
				dev_info(
					chip->dev,
					"DEBUG: Passthrough Entry Config, rc=%d\n",
					rc);
			}
		} else
...
static int smb1398_init_div2_cp_master_psy(struct smb1398_chip *chip)
{
	...
	chip->div2_cp_master_psy = devm_power_supply_register(
		chip->dev, &div2_cp_master_desc, &div2_cp_master_psy_cfg);
	if (IS_ERR(chip->div2_cp_master_psy)) {
		rc = PTR_ERR(chip->div2_cp_master_psy);
		dev_err(chip->dev,
			"Register div2_cp_master power supply failed, rc=%d\n",
			rc);
		return rc;
	}
...
static bool is_psy_voter_available(struct smb1398_chip *chip)
{
	...
	if (!chip->smb_override_votable) {
		...
		if (!chip->smb_override_votable) {
			dev_dbg(chip->dev,
				"Couldn't find SMB_EN_OVERRIDE votable\n");
			return false;
		}
	}
...
static int smb1398_awake_vote_cb(struct votable *votable, void *data, int awake,
				 const char *client)
{
...
static int smb1398_div2_cp_disable_vote_cb(struct votable *votable, void *data,
					   int disable, const char *client)
{
	...
	if (is_for_wireless && disable && client &&
	    strcmp(client, SWITCHER_TOGGLE_VOTER) != 0) {
		schedule_work(&chip->wireless_close_work);
	}

	/* if cp is disable by toggle voter, do not clean wireless cp status */
	if (chip->cp_for_wireless && disable && client &&
	    strcmp(client, SWITCHER_TOGGLE_VOTER) != 0) {
						 ...
						 const char *client)
{
	...
	if (disable && (chip->div2_cp_ilim_votable)) {
		ilim_ua =
			get_effective_result_locked(chip->div2_cp_ilim_votable);
		...
		dev_dbg(chip->dev,
			"slave disabled, restore master CP ilim to %duA\n",
			ilim_ua);
	}
...
static int smb1398_div2_cp_ilim_vote_cb(struct votable *votable, void *data,
					int ilim_ua, const char *client)
{
	...
	dev_info(chip->dev,
		 "ilim_ua :%d, pass_dis:%d, cp_disable:%d, cp_salve_dis:%d\n",
		 ilim_ua,
	...
	is_for_wireless = is_for_wireless_charging(chip);
	max_ilim_ua = is_cps_available(chip) ? DIV2_MAX_ILIM_DUAL_CP_UA :
					       DIV2_MAX_ILIM_UA;
	...
	if (cp_dis) {
		dev_dbg(chip->dev,
			"ilim %duA is too low to config CP charging\n",
			ilim_ua);
		...
		if (is_cps_available(chip)) {
			split_ilim = true;
			slave_dis =
				ilim_ua < chip->ilim_ua_disable_div2_cp_slave;
			...
			if (rc < 0) {
				dev_err(chip->dev,
					"get CP slave CFLY SS failed, rc=%d\n",
					rc);
				return rc;
			}
...
static int passthrough_mode_disable_vote_cb(struct votable *votable, void *data,
					    int disable, const char *client)
{
	struct smb1398_chip *chip = (struct smb1398_chip *)data;
	int rc = 0;
...
static int usbin_suspend_cb(struct votable *votable, void *data, int disable,
			    const char *client)
{
	struct smb1398_chip *chip = (struct smb1398_chip *)data;
	smb1398_masked_write(chip, MISC_USB_WLS_SUSPEND_REG, USB_SUSPEND,
			     disable ? USB_SUSPEND : 0);
	return 0;
}
...
static int smb1398_div2_cp_create_votables(struct smb1398_chip *chip)
{
	...
	chip->awake_votable = create_votable("SMB1398_AWAKE", VOTE_SET_ANY,
					     smb1398_awake_vote_cb, chip);
	if (IS_ERR_OR_NULL(chip->awake_votable))
		return PTR_ERR_OR_ZERO(chip->awake_votable);

	chip->div2_cp_disable_votable =
		create_votable("CP_DISABLE", VOTE_SET_ANY,
			       smb1398_div2_cp_disable_vote_cb, chip);
	...
	chip->div2_cp_slave_disable_votable =
		create_votable("CP_SLAVE_DISABLE", VOTE_SET_ANY,
			       smb1398_div2_cp_slave_disable_vote_cb, chip);
	if (IS_ERR_OR_NULL(chip->div2_cp_slave_disable_votable)) {
		rc = PTR_ERR_OR_ZERO(chip->div2_cp_slave_disable_votable);
		goto destroy;
	}

	chip->div2_cp_ilim_votable = create_votable(
		"CP_ILIM", VOTE_MIN, smb1398_div2_cp_ilim_vote_cb, chip);
	...
	chip->passthrough_mode_disable_votable =
		create_votable("PASSTHROUGH", VOTE_SET_ANY,
			       passthrough_mode_disable_vote_cb, chip);
	if (IS_ERR_OR_NULL(chip->passthrough_mode_disable_votable)) {
		rc = PTR_ERR_OR_ZERO(chip->passthrough_mode_disable_votable);
		goto destroy;
	}

	chip->usbin_suspend_votable = create_votable(
		"USBIN_SUSPEND", VOTE_SET_ANY, usbin_suspend_cb, chip);
...
static int smb1398_get_wls_charging_icl(struct smb1398_chip *chip,
					int *effectiveIcl)
{
	int rc = 0;
	int tx_adapter_type;
	...
	if (tx_adapter_type == ADAPTER_XIAOMI_QC3 ||
		...
		*effectiveIcl = WLDC_XIAOMI_50W_IOUT_MAX_UA;
		...
		dev_err(chip->dev,
			"Get Main tx_adapter_type failed, tx_adapter_type=%d\n",
			tx_adapter_type);
		rc = -EINVAL;
	}
	return rc;
...
static void smb1398_status_change_work(struct work_struct *work)
{
	struct smb1398_chip *chip =
		container_of(work, struct smb1398_chip, status_change_work);
	union power_supply_propval pval = { 0 };
	...
	if (!is_adapter_in_cc_mode(chip) && !is_for_wireless) {
		...
		if (chip->passthrough_mode_allowed)
			vote(chip->passthrough_mode_disable_votable,
			     CUTOFF_SOC_VOTER, is_cutoff_soc_reached(chip), 0);
	}
	...
	if (pval.intval != POWER_SUPPLY_CHARGER_SEC_CP) {
		...
		vote(chip->div2_cp_slave_disable_votable, TAPER_VOTER, false,
		     0);
		vote(chip->div2_cp_slave_disable_votable, DELAY_OPEN_VOTER,
		     false, 0);
		vote(chip->div2_cp_disable_votable, TAPER_VOTER, false, 0);
		vote(chip->passthrough_mode_disable_votable, TAPER_VOTER, false,
		     0);
		...
		vote(chip->passthrough_mode_disable_votable, SRC_VOTER, true,
		     0);
		vote(chip->passthrough_mode_disable_votable, USER_VOTER, false,
		     0);
		smb1398_set_ocp(chip, false);
		smb1398_masked_write(
			chip, PERPH0_MISC_CFG0, CFG_PRE_REG_2S,
			0); //Allow FP FET enable on IREV for input removal
		smb1398_masked_write(chip, PERPH0_CFG_SDCDC_REG, EN_WIN_OV_BIT,
				     EN_WIN_OV_BIT); //Re-Enable WIN_OV
		goto out;
	}
	...
	if (chip->passthrough_mode_allowed &&
		...
		vote(chip->passthrough_mode_disable_votable, SRC_VOTER, false,
		     0);
	...
	if (pval.intval == POWER_SUPPLY_CP_WIRELESS) {
		...
		rc = power_supply_get_property(
			chip->wls_psy, POWER_SUPPLY_PROP_CURRENT_MAX, &pval);
		if (rc < 0) {
			dev_err(chip->dev,
				"Get Wireless CURRENT_MAX failed, rc=%d\n", rc);
		} else {
...
static int smb1398_notifier_cb(struct notifier_block *nb, unsigned long event,
			       void *data)
{
...
{
	struct smb1398_chip *chip =
		container_of(work, struct smb1398_chip, cp_close_work);
...
static void smb1398_wireless_close_work(struct work_struct *work)
{
	...
	int rc = 0;
	struct smb1398_chip *chip =
		container_of(work, struct smb1398_chip, wireless_close_work);
	rc = smb1398_get_wls_charging_icl(chip, &effectiveICL);
	if (!rc) {
		effectiveICL = min(effectiveICL, MAX_MAIN_PMIC_ICL_UA);
		vote(chip->usb_icl_votable, WIRELESS_ICL_VOTER, true,
		     effectiveICL);
	}
}
...
{
	struct smb1398_chip *chip =
		container_of(work, struct smb1398_chip, wireless_open_work);
	union power_supply_propval val = {
		0,
	};
	int wireless_vout = 0;
	chip->isns_cnt = 0;
	vote(chip->usb_icl_votable, WIRELESS_VOTER, true,
	     WLS_MAIN_CHAREGER_ICL_UA);
	vote(chip->div2_cp_ilim_votable, WIRELESS_CP_OPEN_VOTER, true,
	     WLS_MAIN_START_ILIM_UA);
	if (chip->batt_psy) {
...
static void smb1398_isns_process_work(struct work_struct *work)
{
	...
	union power_supply_propval pval = { 0 };
	struct smb1398_chip *chip =
		container_of(work, struct smb1398_chip, isns_process_work);
	...
	if (rc >= 0) {
		...
		if (is_for_wireless_charging(chip)) {
			...
			if (isns_total >= WLS_MAIN_START_ILIM_THRESHOLD_UA &&
			    chip->isns_cnt <= WLS_MAIN_START_ILIM_CNT) {
				if (chip->isns_cnt++ >=
				    WLS_MAIN_START_ILIM_CNT) {
					vote(chip->div2_cp_ilim_votable,
					     WIRELESS_CP_OPEN_VOTER, false, 0);
					smb1398_charge_get_wireless_adapter_type(
						chip, &tx_adapter_type);
					if (tx_adapter_type ==
						    ADAPTER_XIAOMI_PD_45W ||
					    tx_adapter_type ==
						...
						if (!chip->rx_ocp_disable_votable)
							chip->rx_ocp_disable_votable =
								find_votable(
									"IDTP_OCP_DISABLE");
						if (chip->rx_ocp_disable_votable)
							vote(chip->rx_ocp_disable_votable,
							     HIGH_POWER_VOTER,
							     true, 0);

						schedule_delayed_work(
							&chip->high_power_monitor_work,
							msecs_to_jiffies(0));
					}
				}
			}
			...
			if (pval.intval) {
				if (isns_total >=
				    2 * WLS_WIN_OV_EN_THRESHOLD_UA)
					smb1398_masked_write(chip,
							     PERPH0_MISC_CFG0,
							     CFG_PRE_REG_2S,
							     CFG_PRE_REG_2S);
				else
			...
			if (is_adapter_in_cc_mode(chip) &&
				...
				if (count > 10) {
					dev_err(chip->dev,
						"adapter removal set smb disable\n");
					vote(chip->smb_override_votable,
					     PL_SMB_EN_VOTER, true, 0);
				} else
					count++;
			} else {
				count = 0;
...
{
	struct smb1398_chip *chip =
		container_of(work, struct smb1398_chip, stop_rx_ocp_work.work);
...
{
	struct smb1398_chip *chip = container_of(work, struct smb1398_chip,
						 high_power_monitor_work.work);
	...
	if (chip->wls_psy) {
		power_supply_get_property(chip->wls_psy,
	...
	if (get_effective_result_locked(chip->div2_cp_ilim_votable) !=
	    (WLDC_XIAOMI_50W_IOUT_MAX_UA - WLS_MAIN_CHAREGER_ICL_UA)) {
		dev_err(chip->dev, "ilim changed, go out\n");
		ilim_changed = true;
	}

	if (chip->max_power_cnt++ >= MAX_MONITOR_CYCLE_CNT || temp_high ||
	    ilim_changed) {
		/* set 40w icl, break work */
		vote(chip->div2_cp_ilim_votable, HIGH_POWER_VOTER, true,
		     WLDC_XIAOMI_40W_IOUT_MAX_UA - WLS_MAIN_CHAREGER_ICL_UA);
		schedule_delayed_work(&chip->stop_rx_ocp_work,
				      msecs_to_jiffies(5000));
	} else {
		schedule_delayed_work(&chip->high_power_monitor_work,
...
{
	struct smb1398_chip *chip =
		container_of(work, struct smb1398_chip, irev_process_work.work);
	int rc;
	union power_supply_propval pval = { 0 };
	long vbus_uv, vbat_uv;
	int rx_off;
	...
	if (is_for_wireless_charging(chip)) {
		...
		rc = power_supply_get_property(
			chip->usb_psy, POWER_SUPPLY_PROP_VOLTAGE_NOW, &pval);
		if (rc < 0) {
			dev_err(chip->dev,
				"Get USB VOLTAGE_NOW failed, rc=%d\n");
			return;
		}
		...
		rc = power_supply_get_property(
			chip->batt_psy, POWER_SUPPLY_PROP_VOLTAGE_NOW, &pval);
		if (rc < 0) {
			dev_err(chip->dev,
				"Get BATT VOLTAGE_NOW failed, rc=%d\n");
			return;
		}
...
{
	struct smb1398_chip *chip =
		container_of(work, struct smb1398_chip, taper_work);
	union power_supply_propval pval = { 0 };
	...
	while (true) {
		...
		rc = power_supply_get_property(
			chip->bms_psy, POWER_SUPPLY_PROP_VOLTAGE_NOW, &pval);
		if (rc < 0) {
		...
		rc = power_supply_get_property(
			chip->batt_psy, POWER_SUPPLY_PROP_CHARGE_TYPE, &pval);
		if (rc < 0) {
			dev_err(chip->dev, "get CHARGE_TYPE failed, rc=%d\n",
				rc);
			goto out;
		...
		if (pval.intval == POWER_SUPPLY_CHARGE_TYPE_TAPER) {
			...
			if (fcc_ua <= (chip->div2_cp_min_ilim_ua * 4)) {
				vote(chip->smb_override_votable, TAPER_VOTER,
				     true, 0);
			...
			slave_en = !get_effective_result(
				chip->div2_cp_slave_disable_votable);
			if ((get_effective_result(chip->fcc_votable) <=
			     chip->ilim_ua_disable_div2_cp_slave) &&
			    slave_en && is_cps_available(chip)) {
				vote(chip->div2_cp_slave_disable_votable,
				     TAPER_VOTER, true, 0);
				dev_info(chip->dev,
					 "Disable slave CP in taper\n");
				vote_override(
					chip->div2_cp_ilim_votable,
					CC_MODE_VOTER,
					get_effective_result(
...
static int smb1398_div2_cp_hw_init(struct smb1398_chip *chip)
{
	...
	rc = smb1398_masked_write(chip, MISC_CFG0_REG, SW_EN_SWITCHER_BIT,
				  SW_EN_SWITCHER_BIT);
	if (rc < 0) {
	...
	rc = smb1398_masked_write(chip, MISC_SL_SWITCH_EN_REG, EN_SWITCHER, 0);
	if (rc < 0) {
		dev_err(chip->dev, "write SWITCH_EN_REG failed, rc=%d\n", rc);
		return rc;
	}
	...
	rc = smb1398_masked_write(chip, NOLOCK_SPARE_REG, DIV2_WIN_UV_SEL_BIT,
				  0);
	if (rc < 0) {
...
static int smb1398_div2_cp_parse_dt(struct smb1398_chip *chip)
{
	...
	chip->die_temp_chan = devm_iio_channel_get(chip->dev, "die_temp");
	...
	chip->passthrough_mode_allowed = of_property_read_bool(
		chip->dev->of_node, "qcom,passthrough-mode-allowed");
...
static int smb1398_div2_cp_master_probe(struct smb1398_chip *chip)
{
	...
	if (rc < 0) {
		dev_err(chip->dev,
			"init wakeup failed for div2_cp_master device, rc=%d\n",
			rc);
		return rc;
	}
	...
	if (rc < 0) {
		dev_err(chip->dev,
			"smb1398_div2_cp_create_votables failed, rc=%d\n", rc);
		return rc;
	}
	...
	if (rc > 0) {
		dev_err(chip->dev,
			"smb1398_init_div2_cp_master_psy failed, rc=%d\n", rc);
	...
	INIT_DELAYED_WORK(&chip->high_power_monitor_work,
			  &smb1398_high_power_monitor_work);
				  ...
				  union power_supply_propval *pval)
{
	...
	default:
		dev_err(chip->dev,
			"read div2_cp_slave property %d is not supported\n",
			prop);
		return -EINVAL;
	}
				  ...
				  const union power_supply_propval *pval)
{
	...
	default:
		dev_err(chip->dev,
			"write div2_cp_slave property %d is not supported\n",
			prop);
		return -EINVAL;
	}
...
static int smb1398_init_div2_cp_slave_psy(struct smb1398_chip *chip)
{
	...
	chip->div2_cp_slave_psy = devm_power_supply_register(
		chip->dev, &div2_cps_psy_desc, &cps_cfg);
...
static int smb1398_div2_cp_slave_probe(struct smb1398_chip *chip)
{
	 ...
	 */
	rc = smb1398_masked_write(chip, PERPH0_CFG_SDCDC_REG, EN_WIN_UV_BIT,
				  EN_WIN_UV_BIT);
	if (rc < 0) {
...
static int smb1398_create_pre_regulator_votables(struct smb1398_chip *chip)
{
	chip->pre_regulator_iout_votable =
		create_votable("PRE_REGULATOR_IOUT", VOTE_MIN,
			       smb1398_pre_regulator_iout_vote_cb, chip);
	...
	chip->pre_regulator_vout_votable =
		create_votable("PRE_REGULATOR_VOUT", VOTE_MIN,
			       smb1398_pre_regulator_vout_vote_cb, chip);
				  ...
				  union power_supply_propval *pval)
{
	...
	default:
		dev_err(chip->dev,
			"read pre_regulator property %d is not supported\n",
			prop);
		return -EINVAL;
	}
				  ...
				  const union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_INPUT_CURRENT_MAX:
		...
		break;
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT_MAX:
		vote(chip->pre_regulator_iout_votable, CP_VOTER, true,
		     pval->intval);
		break;
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_VOLTAGE_MAX:
		vote(chip->pre_regulator_vout_votable, CP_VOTER, true,
		     pval->intval);
		break;
	default:
		dev_err(chip->dev,
			"write pre_regulator property %d is not supported\n",
			prop);
		return -EINVAL;
	}
...
static int smb1398_create_pre_regulator_psy(struct smb1398_chip *chip)
{
	...
	chip->pre_regulator_psy = devm_power_supply_register(
		chip->dev, &pre_regulator_psy_desc, &pre_regulator_psy_cfg);
...
static int smb1398_pre_regulator_probe(struct smb1398_chip *chip)
{
	...
	if (rc < 0) {
		dev_err(chip->dev,
			"Create votable for pre_regulator failed, rc=%d\n", rc);
		return rc;
	}
