...
static void rx1619_oob_set_ept_work(struct work_struct *work)
{
	struct rx1619_chg *chip =
		container_of(work, struct rx1619_chg, oob_set_ept_work.work);
...
{
	struct rx1619_chg *chip =
		container_of(work, struct rx1619_chg, oob_set_cep_work.work);
...
{
	struct rx1619_chg *chip =
		container_of(work, struct rx1619_chg, train_tx_work.work);
...
*/
	...
	case 0x0d:
		...
		if (chip->is_compatible_hwid) {
			...
			if (g_uuid_data[3] == 0x07 && g_uuid_data[1] == 0x1 &&
			    g_uuid_data[2] == 0x4 &&
			    ((g_uuid_data[0] == 0x9) ||
			     (g_uuid_data[0] == 0x1))) {
				chip->is_ble_tx = 1;
...
static int rx1619_parse_dt(struct rx1619_chg *chip)
{
	...
	chip->reverse_boost_enable_gpio =
		of_get_named_gpio(node, "rx,reverse-booset-enable", 0);
...
static int rx_set_reverse_boost_enable_gpio(struct rx1619_chg *chip, int enable)
{
	...
	if (gpio_is_valid(chip->reverse_boost_enable_gpio)) {
		...
		ret = gpio_direction_output(chip->reverse_boost_enable_gpio,
					    !!enable);
		if (ret) {
			dev_err(chip->dev,
			...
			const struct i2c_device_id *id)
{
	...
	if (alarmtimer_get_rtcdev()) {
		alarm_init(&chip->reverse_test_ready_alarm, ALARM_BOOTTIME,
