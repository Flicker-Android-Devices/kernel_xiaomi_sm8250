...
static bool is_input_present(struct step_chg_info *chip)
{
	...
	if (chip->usb_psy) {
		rc = power_supply_get_property(
			chip->usb_psy, POWER_SUPPLY_PROP_PRESENT, &pval);
		if (rc < 0)
	...
	if (chip->dc_psy) {
		rc = power_supply_get_property(
			chip->dc_psy, POWER_SUPPLY_PROP_PRESENT, &pval);
		if (rc < 0)
...
int read_range_data_from_node(struct device_node *node, const char *prop_str,
			      struct range_data *ranges, int max_threshold,
			      u32 max_value)
{
...
static int get_step_chg_jeita_setting_from_profile(struct step_chg_info *chip)
{
	...
	handle = of_get_property(chip->dev->of_node, "qcom,battery-data", NULL);
	if (!handle) {
		pr_debug(
			"ignore getting sw-jeita/step charging settings from profile\n");
		return 0;
	}
	...
	profile_node = of_batterydata_get_best_profile(
		batt_node, batt_id_ohms / 1000, NULL);
	...
	pr_debug(
		"battery: %s detected, getting sw-jeita/step charging settings\n",
		batt_type_str);
	...
	chip->vbat_avg_based_step_chg = of_property_read_bool(
		profile_node, "qcom,vbat-avg-based-step-chg");
	...
	rc = read_range_data_from_node(profile_node, "qcom,soc-fcc-ranges",
				       chip->soc_fcc_config->fcc_cfg,
				       BATT_CAP_SOC_MAX, max_fcc_ma * 1000);
	if (rc < 0) {
		pr_debug(
			"Read qcom,soc-fcc-ranges failed from battery profile, rc=%d\n",
			rc);
	...
	if (rc < 0) {
		pr_debug(
			"Read qcom,-fv-ranges failed from battery profile, rc=%d\n",
			rc);
	...
	rc = read_range_data_from_node(
		profile_node, "qcom,step-chg-ranges",
		chip->step_chg_config->fcc_cfg,
		chip->soc_based_step_chg ? 100 : max_fv_uv, max_fcc_ma * 1000);
	if (rc < 0) {
		pr_debug(
			"Read qcom,step-chg-ranges failed from battery profile, rc=%d\n",
			rc);
		chip->step_chg_cfg_valid = false;
	}

	chip->taper_fcc_rate_valid = true;
	rc = read_range_data_from_node(profile_node, "qcom,taper-fcc-rate",
				       chip->taper_fcc_rate->fcc_cfg,
				       max_fcc_ma * 1000, 100);
	if (rc < 0) {
		pr_debug(
			"Read qcom,taper-fcc-rate failed from battery profile, rc=%d\n",
			rc);
		chip->taper_fcc_rate_valid = false;
	}

	chip->sw_jeita_cfg_valid = true;
	rc = read_range_data_from_node(profile_node, "qcom,jeita-fcc-ranges",
				       chip->jeita_fcc_config->fcc_cfg,
				       BATT_HOT_DECIDEGREE_MAX,
				       max_fcc_ma * 1000);
	if (rc < 0) {
		pr_debug(
			"Read qcom,jeita-fcc-ranges failed from battery profile, rc=%d\n",
			rc);
		chip->sw_jeita_cfg_valid = false;
	}

	rc = read_range_data_from_node(profile_node, "qcom,jeita-fv-ranges",
				       chip->jeita_fv_config->fv_cfg,
				       BATT_HOT_DECIDEGREE_MAX, max_fv_uv);
	if (rc < 0) {
		pr_debug(
			"Read qcom,jeita-fv-ranges failed from battery profile, rc=%d\n",
			rc);
		chip->sw_jeita_cfg_valid = false;
	}
...
static void get_config_work(struct work_struct *work)
{
	struct step_chg_info *chip =
		container_of(work, struct step_chg_info, get_config_work.work);
	int i, rc;
		   ...
		   int threshold, int *new_index, int *val)
{
	...
	*new_index = -EINVAL;
	...
	for (i = 0; i < MAX_STEP_CHG_ENTRIES; i++) {
		...
		if (is_between(range[i].low_threshold, range[i].high_threshold,
			       threshold)) {
			*new_index = i;
			*val = range[i].value;
			break;
			       ...
			       int current_voltage)
{
	...
	rc = get_val(chip->taper_fcc_rate->fcc_cfg, 0, chip->rate_index,
		     current_fcc, &chip->rate_index, &rate);
	if (rc < 0) {
	...
	if (index == 0) {
		...
		if (current_fcc > target_fcc) {
			...
			if (step_ua < TAPERED_STEP_CHG_FCC_REDUCTION_STEP_UA)
				step_ua =
					TAPERED_STEP_CHG_FCC_REDUCTION_STEP_UA;
		}
		vote(chip->fcc_votable, STEP_CHG_VOTER, true,
		     max(target_fcc, current_fcc - step_ua));
	} else if ((current_fcc >
		...
		if (current_fcc >
		    chip->step_chg_config->fcc_cfg[index - 1].value) {
			step_ua = (current_fcc -
				   chip->step_chg_config->fcc_cfg[index - 1]
					   .value) *
				  rate / 100;
			if (step_ua < TAPERED_STEP_CHG_FCC_REDUCTION_STEP_UA)
				step_ua =
	...
	pr_info("cur:%d, target:%d, rate:%d, vol:%d cfg[index-1].value:%d, low:%d, high:%d, hysteresis:%d",
		current_fcc, target_fcc, rate, current_voltage,
		chip->step_chg_config->fcc_cfg[index - 1].value,
		chip->step_chg_config->fcc_cfg[index - 1].low_threshold,
		chip->step_chg_config->fcc_cfg[index - 1].high_threshold,
...
static int handle_soc_fcc_config(struct step_chg_info *chip)
{
	...
	rc = power_supply_get_property(chip->usb_psy, POWER_SUPPLY_PROP_PRESENT,
				       &pval);
	if (rc < 0) {
	...
	rc = power_supply_get_property(
		chip->batt_psy, chip->soc_fcc_config->param.psy_prop, &pval);
	if (rc < 0) {
	...
	rc = get_val(chip->soc_fcc_config->fcc_cfg,
		     chip->soc_fcc_config->param.hysteresis, chip->soc_index,
		     pval.intval, &chip->soc_index, &fcc_ua);
	if (rc < 0) {
		if (chip->fcc_votable)
...
static int handle_step_chg_config(struct step_chg_info *chip)
{
	...
	if (elapsed_us < STEP_CHG_HYSTERISIS_DELAY_US && !update_now &&
	    !chip->fcc_taper_start)
		return 0;

	rc = power_supply_get_property(
		chip->batt_psy, POWER_SUPPLY_PROP_STEP_CHARGING_ENABLED, &pval);
	if (rc < 0)
	...
	if (chip->step_chg_config->param.use_bms)
		rc = power_supply_get_property(
			chip->bms_psy, chip->step_chg_config->param.psy_prop,
			&pval);
	else
		rc = power_supply_get_property(
			chip->batt_psy, chip->step_chg_config->param.psy_prop,
			&pval);
	...
	rc = get_val(chip->step_chg_config->fcc_cfg,
		     chip->step_chg_config->param.hysteresis, chip->step_index,
		     pval.intval, &chip->step_index, &fcc_ua);
	if (rc < 0) {
...
static int handle_fast_charge_mode(struct step_chg_info *chip, int temp)
{
	...
	if (!chip->ffc_mode_dis_votable || !chip->fcc_votable ||
	    !chip->fv_votable)
		return -EINVAL;
	...
	rc = power_supply_get_property(
		chip->bms_psy, POWER_SUPPLY_PROP_FASTCHARGE_MODE, &pval);
	...
	if (ffc_enable) {
		...
		rc = power_supply_get_property(
			chip->batt_psy, POWER_SUPPLY_PROP_VOLTAGE_MAX, &pval);
		fv_max = pval.intval;
		...
		if (curr_vbat_uv > (fv_max - FV_REDUCTION_UV)) {
			if ((count++) / 10) {
				vote(chip->fv_votable, STEP_BMS_CHG_VOTER, true,
				     fv_max - FV_REDUCTION_UV);
				count = 0;
			}
			if (curr_fcc > FCC_REDUCTION_LIMIT_UA)
				vote(chip->fcc_votable, STEP_BMS_CHG_VOTER,
				     true, curr_fcc - FCC_REDUCTION_UA);
		} else if ((curr_vbat_uv < (fv_max - 2 * FV_REDUCTION_UV)) &&
			   is_client_vote_enabled(chip->fv_votable,
						  STEP_BMS_CHG_VOTER))
			vote(chip->fv_votable, STEP_BMS_CHG_VOTER, false, 0);
...
static int handle_jeita(struct step_chg_info *chip)
{
	...
	rc = power_supply_get_property(
		chip->batt_psy, POWER_SUPPLY_PROP_SW_JEITA_ENABLED, &pval);
	if (rc < 0)
	...
	if (!chip->sw_jeita_enable || !chip->sw_jeita_cfg_valid) {
		...
		if (chip->input_suspend_votable)
			vote(chip->input_suspend_votable, JEITA_VOTER, false,
			     0);
		return 0;
	}
	...
	if (chip->jeita_fcc_config->param.use_bms)
		rc = power_supply_get_property(
			chip->bms_psy, chip->jeita_fcc_config->param.psy_prop,
			&pval);
	else
		rc = power_supply_get_property(
			chip->batt_psy, chip->jeita_fcc_config->param.psy_prop,
			&pval);
	...
	if (chip->jeita_hot_th != -EINVAL && chip->jeita_cold_th != -EINVAL) {
		if (temp >= chip->jeita_hot_th || temp <= chip->jeita_cold_th) {
			pr_info("sw-jeita: temp is :%d, stop charing\n", temp);
			vote(chip->chg_disable_votable, JEITA_VOTER, true, 0);
			vote_override(chip->fcc_main_votable, CC_MODE_VOTER,
				      false, 0);
			vote_override(chip->input_suspend_votable,
				      CC_MODE_VOTER, false, 0);
	...
	if (!chip->use_bq_pump) {
		if (temp < chip->jeita_cool_th || temp > chip->jeita_warm_th) {
			...
			vote_override(chip->input_suspend_votable,
				      CC_MODE_VOTER, false, 0);
		} else if (temp >= chip->jeita_cool_th +
					   chip->jeita_fcc_config->param
						   .hysteresis &&
			   temp <= chip->jeita_warm_th -
					   chip->jeita_fcc_config->param
						   .hysteresis) {
	...
	rc = get_val(chip->jeita_fcc_config->fcc_cfg,
		     ...
		     chip->jeita_fcc_index, temp, &chip->jeita_fcc_index,
		     &fcc_ua);
	if (rc < 0)
		fcc_ua = 0;
	...
	if (chip->cold_step_chg_cfg_valid == true) {
		rc = get_val(chip->cold_step_chg_config->fcc_cfg,
			     ...
			     chip->jeita_cold_fcc_index, volt_now,
			     &chip->jeita_cold_fcc_index, &cold_fcc_ua);
		if (rc < 0)
			cold_fcc_ua = 0;
	}
	...
	if (chip->cold_step_chg_cfg_valid) {
		if (chip->jeita_fcc_index == 0 && temp < 0) {
			...
			cold_fcc_ua =
				chip->cold_step_chg_config
					->fcc_cfg[chip->jeita_cold_fcc_index]
					.value;
			vote(chip->fcc_votable, JEITA_VOTER,
			     cold_fcc_ua ? true : false, cold_fcc_ua);
		} else {
			if (!chip->jeita_taper_start) {
				if (last_index <= BATT_COOL_INDEX &&
				    chip->jeita_fcc_index > BATT_COOL_INDEX) {
					chip->jeita_current_fcc =
						JEITA_TAPER_DEFUALT_MA;
					chip->jeita_target_fcc = fcc_ua;
					schedule_delayed_work(
						&chip->jeita_taper_work, 0);
				} else
					vote(chip->fcc_votable, JEITA_VOTER,
					     fcc_ua ? true : false, fcc_ua);
			}
			chip->jeita_cold_fcc_index = 0;
		}
	} else {
		vote(chip->fcc_votable, JEITA_VOTER, fcc_ua ? true : false,
		     fcc_ua);
	}
	...
	rc = get_val(chip->jeita_fv_config->fv_cfg,
		     chip->jeita_fv_config->param.hysteresis,
		     chip->jeita_fv_index, temp, &chip->jeita_fv_index, &fv_uv);
	if (rc < 0)
		fv_uv = 0;
	...
	if (rc || (pval.intval == fv_uv)) {
		...
		vote(chip->fv_votable, JEITA_VOTER, fv_uv ? true : false,
		     fv_uv);
		goto update_time;
	}
	...
	if (fv_uv > 0) {
		...
		if ((curr_vbat_uv > fv_uv) && ((cell_vbat_uv > fv_uv)) &&
		    (temp >= chip->jeita_warm_th))
...
{
	struct step_chg_info *chip =
		container_of(work, struct step_chg_info, jeita_work.work);
	int rc = 0;
	union power_supply_propval pval = {
		0,
	};
...
{
	struct step_chg_info *chip =
		container_of(work, struct step_chg_info, fcc_taper_work.work);
...
{
	struct step_chg_info *chip =
		container_of(work, struct step_chg_info, jeita_taper_work.work);
	int target_fcc, rc;
	union power_supply_propval pval = {
		0,
	};
	...
	rc = power_supply_get_property(
		chip->bms_psy, chip->jeita_fcc_config->param.psy_prop, &pval);
	...
	pr_info("handle_jeita curr:%d, target:%d\n", chip->jeita_current_fcc,
		chip->jeita_target_fcc);
	chip->jeita_current_fcc = min(
		(chip->jeita_current_fcc + JEITA_TAPER_STEP_MA), target_fcc);
...
{
	struct step_chg_info *chip = container_of(work, struct step_chg_info,
	...
	power_supply_get_property(chip->usb_psy, POWER_SUPPLY_PROP_PD_ACTIVE,
				  &prop);
...
static int step_chg_notifier_call(struct notifier_block *nb, unsigned long ev,
				  void *v)
{
...
int qcom_step_chg_init(struct device *dev, bool step_chg_enable,
		       bool sw_jeita_enable, bool jeita_arb_en)
{
	int rc;
	struct step_chg_info *chip;
	...
	chip->step_chg_config =
		devm_kzalloc(dev, sizeof(struct step_chg_cfg), GFP_KERNEL);
	...
	chip->cold_step_chg_config =
		devm_kzalloc(dev, sizeof(struct cold_step_chg_cfg), GFP_KERNEL);
	chip->cold_step_chg_config->param.psy_prop =
		POWER_SUPPLY_PROP_VOLTAGE_NOW;
	...
	chip->soc_fcc_config =
		devm_kzalloc(dev, sizeof(struct soc_fcc_cfg), GFP_KERNEL);
	...
	chip->jeita_fcc_config =
		devm_kzalloc(dev, sizeof(struct jeita_fcc_cfg), GFP_KERNEL);
	chip->jeita_fv_config =
		devm_kzalloc(dev, sizeof(struct jeita_fv_cfg), GFP_KERNEL);
