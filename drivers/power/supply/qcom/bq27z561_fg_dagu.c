...
static int fg_write_read_block(struct bq_fg_chip *bq, u8 reg, u8 *data, u8 len,
			       u8 lens)
{
	int ret;
...
static int fg_mac_read_block(struct bq_fg_chip *bq, u16 cmd, u8 *buf, u8 len)
{
	...
	ret = fg_write_read_block(bq, bq->regs[BQ_FG_REG_ALT_MAC], t_buf, 2,
				  36);
	...
	if (cmd == FG_MAC_CMD_LIFETIME1) {
		strbuf = fg_print_buf(
			"mac_read_block FG_MAC_CMD_LIFETIME1: ", t_buf, 36);
		//bq_dbg(PR_OEM, "LTP: %s\n", strbuf);
	...
	if (cmd == FG_MAC_CMD_LIFETIME3) {
		strbuf = fg_print_buf(
			"mac_read_block FG_MAC_CMD_LIFETIME3: ", t_buf, 36);
...
static int fg_get_temp_max_fac(struct bq_fg_chip *bq)
{
	...
	while (retry < 2) {
		ret = fg_mac_read_block(bq, FG_MAC_CMD_LIFETIME1,
					data_limetime1, sizeof(data_limetime1));
		retry++;
		if (ret == 2) {
			bq_dbg(PR_OEM,
...
static int fg_get_time_ot(struct bq_fg_chip *bq)
{
	...
	while (retry < 2) {
		ret = fg_mac_read_block(bq, FG_MAC_CMD_LIFETIME3,
					data_limetime3, sizeof(data_limetime3));
		retry++;
		if (ret == 2) {
			bq_dbg(PR_OEM,
			       "the address FG_MAC_CMD_LIFETIME3 is error, retry:%d.\n",
			       retry);
			continue;
		}
		break;
