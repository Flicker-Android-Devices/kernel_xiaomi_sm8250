...
static int smb1398_div2_cp_forge_status1(struct smb1398_chip *chip, u8 *status)
{
	int rc = 0;
	u8 val;
...
static int smb1398_div2_cp_forge_status2(struct smb1398_chip *chip, u8 *status)
{
	int rc = 0;
	u8 val;
...
static int smb1398_div2_cp_forge_irq_status(struct smb1398_chip *chip,
					    u8 *status)
{
	int rc = 0;
	u8 val;
...
static int smb1398_div2_cp_en_switcher(struct smb1398_chip *chip, bool en)
{
	...
	mask = EN_SWITCHER;
	val = EN_SWITCHER;
	rc = smb1398_masked_write(chip, MISC_SL_SWITCH_EN_REG, mask,
				  en ? val : 0);
	if (rc < 0) {
...
static int smb1398_get_1s_vout_mv(struct smb1398_chip *chip, int *vout_mv)
{
	...
	max = (val & VOUT_DAC_VALUE_MASK) * VOUT_1S_PER_BIT_MV + VOUT_1S_MIN_MV;
...
static void smb1398_status_change_work(struct work_struct *work)
{
	...
	if (pval.intval == POWER_SUPPLY_CP_WIRELESS) {
		...
		rc = power_supply_get_property(
			chip->dc_psy, POWER_SUPPLY_PROP_CURRENT_MAX, &pval);
		if (rc < 0)
			dev_err(chip->dev, "Get DC CURRENT_MAX failed, rc=%d\n",
				rc);
		else
			vote(chip->div2_cp_ilim_votable, WIRELESS_VOTER, true,
			     pval.intval);
	} else {
...
static void smb1398_taper_work(struct work_struct *work)
{
	...
	while (true) {
		...
		if (fv_uV > chip->taper_entry_fv) {
			dev_dbg(chip->dev,
				"Float voltage increased (%d-->%d)uV, exit!\n",
				chip->taper_entry_fv, fv_uV);
			vote(chip->div2_cp_disable_votable, TAPER_VOTER, false,
			     0);
			goto out;
		} else {
			chip->taper_entry_fv = fv_uV;
...
static int smb1398_div2_cp_hw_init(struct smb1398_chip *chip)
{
	...
	rc = smb1398_masked_write(chip, MISC_CFG0_REG, CFG_EN_SOURCE_BIT,
				  CFG_EN_SOURCE_BIT);
	if (rc < 0) {
...
static int smb1398_pre_regulator_probe(struct smb1398_chip *chip)
{
	...
	if (rc < 0) {
		dev_err(chip->dev,
			"Read pre-regulator MODE_STATUS_REG failed, rc=%d\n",
			rc);
		return rc;
	}
