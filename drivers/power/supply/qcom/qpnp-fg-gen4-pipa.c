...
static ssize_t profile_dump_show(struct device *dev,
...
static struct fg_sram_param pm8150b_v1_sram_params[] = {
	...
	PARAM(VBATT_LOW, VBATT_LOW_WORD, VBATT_LOW_OFFSET, 1, 1000, 15625,
	      -2000, fg_encode_voltage, NULL),
	PARAM(VBATT_FULL, VBATT_FULL_WORD, VBATT_FULL_OFFSET, 2, 1000, 244141,
	      0, fg_encode_voltage, fg_decode_voltage_15b),
	PARAM(CUTOFF_CURR, CUTOFF_CURR_WORD, CUTOFF_CURR_OFFSET, 2, 100000,
	      48828, 0, fg_encode_current, NULL),
	...
	PARAM(KI_COEFF_CUTOFF, KI_COEFF_CUTOFF_WORD, KI_COEFF_CUTOFF_OFFSET, 1,
	...
	PARAM(KI_COEFF_LOW_DISCHG, KI_COEFF_LOW_DISCHG_WORD,
	      KI_COEFF_LOW_DISCHG_OFFSET, 1, 1000, 61035, 0, fg_encode_default,
	      NULL),
	PARAM(KI_COEFF_MED_DISCHG, KI_COEFF_MED_DISCHG_WORD,
	      KI_COEFF_MED_DISCHG_OFFSET, 1, 1000, 61035, 0, fg_encode_default,
	      NULL),
	PARAM(KI_COEFF_HI_DISCHG, KI_COEFF_HI_DISCHG_WORD,
	      KI_COEFF_HI_DISCHG_OFFSET, 1, 1000, 61035, 0, fg_encode_default,
	      NULL),
	...
	PARAM(BATT_TEMP_HOT, BATT_TEMP_CONFIG_WORD, BATT_TEMP_HOT_OFFSET, 1, 1,
	      1, 0, fg_encode_default, NULL),
	PARAM(ESR_CAL_SOC_MIN, BATT_TEMP_CONFIG2_WORD, ESR_CAL_SOC_MIN_OFFSET,
	      1, 1, 1, 0, fg_encode_default, NULL),
	PARAM(ESR_CAL_SOC_MAX, ESR_CAL_THRESH_WORD, ESR_CAL_SOC_MAX_OFFSET, 1,
	      1, 1, 0, fg_encode_default, NULL),
	PARAM(ESR_CAL_TEMP_MIN, ESR_CAL_THRESH_WORD, ESR_CAL_TEMP_MIN_OFFSET, 1,
	      1, 1, 0, fg_encode_default, NULL),
...
static struct fg_sram_param pm8150b_v2_sram_params[] = {
	...
	PARAM(ACT_BATT_CAP, ACT_BATT_CAP_v2_WORD, ACT_BATT_CAP_v2_OFFSET, 2, 1,
...
static int fg_get_batt_id_adc(struct fg_gen4_chip *chip, u32 *batt_id_ohms)
{
	...
	*batt_id_ohms = div64_u64(
		chip->dt.batt_id_pullup_kohms * 1000 * 1000 + denom / 2, denom);
...
static int fg_gen4_get_battery_temp(struct fg_dev *fg, int *val)
{
	...
	rc = fg_sram_read(fg, BATT_TEMP_WORD, BATT_TEMP_OFFSET, (u8 *)&buf, 2,
			  FG_IMA_DEFAULT);
	if (rc < 0) {
	...
	*val = sign_extend32(buf, 9) * 100 / 40;
	...
	if (fg->usb_psy)
		rc = power_supply_get_property(
			fg->usb_psy, POWER_SUPPLY_PROP_BATTERY_SLAVE_TEMP,
			&prop);
	if (rc < 0) {
...
static bool is_debug_batt_id(struct fg_dev *fg)
{
	...
	if (is_between(debug_batt_id[0], debug_batt_id[1], fg->batt_id_ohms)) {
...
static int fg_gen4_get_prop_soc_decimal_rate(struct fg_gen4_chip *chip,
					     int *val)
{
	struct fg_dev *fg = &chip->fg;
	int i, soc, rc = 0;
...
static int fg_gen4_get_ttf_param(void *data, enum ttf_param param, int *val)
{
	...
	case TTF_FCC:
		...
		full_soc = DIV_ROUND_CLOSEST(
			((u16)full_soc >> 8) * FULL_CAPACITY, FULL_SOC_RAW);
...
static int fg_gen4_prime_cc_soc_sw(void *data, u32 batt_soc)
{
	...
	rc = fg_sram_write(fg, fg->sp[FG_SRAM_CC_SOC_SW].addr_word,
			   ...
			   (u8 *)&cc_soc_sw, fg->sp[FG_SRAM_CC_SOC_SW].len,
			   FG_IMA_ATOMIC);
	if (rc < 0)
					    ...
					    int batt_temp)
{
	...
	rc = fg_sram_write(fg, KI_COEFF_FULL_SOC_NORM_WORD,
			   KI_COEFF_FULL_SOC_NORM_OFFSET, &val, 1,
			   FG_IMA_DEFAULT);
	if (rc < 0) {
	...
	rc = fg_sram_write(fg, KI_COEFF_LOW_DISCHG_WORD,
			   KI_COEFF_FULL_SOC_LOW_OFFSET, &val, 1,
			   FG_IMA_DEFAULT);
	if (rc < 0) {
				       ...
				       int ki_coeff_med, int ki_coeff_hi)
{
	...
	if (ki_coeff_low >= 0) {
		fg_encode(fg->sp, FG_SRAM_KI_COEFF_LOW_DISCHG, ki_coeff_low,
			  &val);
		rc = fg_sram_write(
			fg, fg->sp[FG_SRAM_KI_COEFF_LOW_DISCHG].addr_word,
	...
	if (ki_coeff_med >= 0) {
		fg_encode(fg->sp, FG_SRAM_KI_COEFF_MED_DISCHG, ki_coeff_med,
			  &val);
		rc = fg_sram_write(
			fg, fg->sp[FG_SRAM_KI_COEFF_MED_DISCHG].addr_word,
	...
	if (ki_coeff_hi >= 0) {
		...
		rc = fg_sram_write(fg,
				   ...
				   fg->sp[FG_SRAM_KI_COEFF_HI_DISCHG].addr_byte,
				   &val, fg->sp[FG_SRAM_KI_COEFF_HI_DISCHG].len,
				   FG_IMA_DEFAULT);
		if (rc < 0) {
...
static int fg_gen4_adjust_ki_coeff_dischg(struct fg_dev *fg)
{
	...
	rc = fg_gen4_set_ki_coeff_dischg(fg, ki_coeff_low, ki_coeff_med,
					 ki_coeff_hi);
	if (rc < 0)
		return rc;
...
static int fg_gen4_rapid_soc_config(struct fg_gen4_chip *chip, bool en)
{
	...
	fg_dbg(fg, FG_STATUS,
	       "Configured slope limit coeff %d cutoff current %d mA\n",
	       slope_limit_coeff, cutoff_curr_ma);
	return 0;
}

static int
qpnp_fg_gen4_get_step_charging_params(struct fg_gen4_chip *chip,
				      struct device_node *profile_node)
{
	...
	if (!of_property_read_bool(profile_node, "qcom,soc-based-step-chg") &&
		...
		chip->ttf->step_chg_cfg = kcalloc(
			len, sizeof(*chip->ttf->step_chg_cfg), GFP_KERNEL);
		...
		if (chip->ttf->step_chg_cfg_valid) {
			for (i = 0; i < tuple_len; i++)
				pr_debug(
					"Vbatt_low: %d Vbatt_high: %d FCC: %d\n",
					chip->ttf->step_chg_cfg[i].low_threshold,
					chip->ttf->step_chg_cfg[i]
						.high_threshold,
					     ...
					     struct device_node *profile_node)
{
	...
	rc = of_property_read_u32(profile_node,
				  "qcom,fg-ffc-cc-cv-threshold-mv",
	...
	if (of_find_property(profile_node, "qcom,therm-coefficients", &len)) {
		...
		rc = of_property_read_u32_array(profile_node,
						"qcom,therm-coefficients",
						fg->bp.therm_coeffs, len);
		if (rc < 0) {
		...
		rc = of_property_read_u32(profile_node,
					  "qcom,therm-center-offset",
					  &fg->bp.therm_ctr_offset);
		if (rc < 0) {
	...
	if (of_find_property(profile_node, "qcom,rslow-normal-coeffs", NULL) &&
		...
		if (!fg->bp.rslow_normal_coeffs) {
			fg->bp.rslow_normal_coeffs =
				devm_kcalloc(fg->dev, RSLOW_NUM_COEFFS,
					     sizeof(u32), GFP_KERNEL);
			if (!fg->bp.rslow_normal_coeffs)
				return -ENOMEM;
		}
...
static int fg_gen4_get_batt_profile(struct fg_dev *fg)
{
	...
	if (chip->dt.multi_profile_load) {
		profile_node = of_batterydata_get_best_aged_profile(
			batt_node, fg->batt_id_ohms / 1000,
		...
		if ((chip->ds_romid[0] == 0x9F) &&
		    ((chip->ds_romid[5] & 0xF0) == 0xF0) &&
		    (chip->ds_romid[6] == 0x04) && !fg->profile_already_find) {
			pr_err("get romid success");
			if ((chip->ds_page0[0] == 'L') ||
			    (chip->ds_page0[0] == 'X')) {
...
#else
		profile_node = of_batterydata_get_best_profile(
			batt_node, fg->batt_id_ohms / 1000, NULL);
#endif
	}
	...
	if (chip->dt.multi_profile_load) {
		if (chip->batt_age_level != avail_age_level) {
			fg_dbg(fg, FG_STATUS,
			       "Batt_age_level %d doesn't exist, using %d\n",
			       chip->batt_age_level, avail_age_level);
			chip->batt_age_level = avail_age_level;
		}
...
static void clear_battery_profile(struct fg_dev *fg)
{
	...
	rc = fg_sram_write(fg, PROFILE_INTEGRITY_WORD, PROFILE_INTEGRITY_OFFSET,
...
static bool is_profile_load_required(struct fg_gen4_chip *chip)
{
	...
	rc = fg_sram_read(fg, PROFILE_INTEGRITY_WORD, PROFILE_INTEGRITY_OFFSET,
			  &val, 1, FG_IMA_DEFAULT);
	if (rc < 0) {
	...
	if (val & PROFILE_LOAD_BIT) {
		...
		profiles_same =
			memcmp(chip->batt_profile, buf, PROFILE_COMP_LEN) == 0;
		if (profiles_same) {
			fg_dbg(fg, FG_STATUS,
			       "Battery profile is same, not loading it\n");
		...
		fg_dbg(fg, FG_STATUS,
		       "Profiles are different, loading the correct one\n");
	} else {
...
static int qpnp_fg_gen4_load_profile(struct fg_gen4_chip *chip)
{
	...
	normal_profile_load =
		...
		(chip->dt.multi_profile_load && chip->batt_age_level == 0);
...
static bool is_sdam_cookie_set(struct fg_gen4_chip *chip)
{
	...
	rc = nvmem_device_read(chip->fg_nvmem, SDAM_COOKIE_OFFSET, 1, &cookie);
	if (rc < 0) {
...
static void fg_gen4_post_profile_load(struct fg_gen4_chip *chip)
{
	...
	if (chip->fg_nvmem && !is_sdam_cookie_set(chip)) {
		...
		rc = fg_sram_read(&chip->fg, CYCLE_COUNT_WORD,
				  CYCLE_COUNT_OFFSET, buf, 16, FG_IMA_DEFAULT);
		if (rc < 0) {
			pr_err("Error in reading cycle counters from SRAM rc=%d\n",
			       rc);
		} else {
			rc = nvmem_device_write(chip->fg_nvmem,
						SDAM_CYCLE_COUNT_OFFSET, 16,
						(u8 *)buf);
			if (rc < 0)
...
{
	struct fg_dev *fg =
		container_of(work, struct fg_dev, profile_load_work.work);
	struct fg_gen4_chip *chip = container_of(fg, struct fg_gen4_chip, fg);
...
static void get_batt_psy_props(struct fg_dev *fg)
{
	...
	if (!chip->recharge_soc_thr) {
		rc = power_supply_get_property(
			fg->batt_psy, POWER_SUPPLY_PROP_RECHARGE_SOC, &prop);
		if (rc < 0) {
...
static int fg_gen4_update_maint_soc(struct fg_dev *fg)
{
	...
	fg_dbg(fg, FG_STATUS,
	       ...
	       fg->last_msoc, fg->maint_soc, fg->delta_soc);
	fg->last_msoc = msoc;
out:
	...
	if (is_input_present(fg)) {
		if (fg->charge_done) {
			if (!fg->recharge_soc_adjusted) {
				...
				new_recharge_soc =
					msoc - (FULL_CAPACITY - recharge_soc);
...
static int fg_gen4_charge_full_update(struct fg_dev *fg)
{
	...
	recharge_soc =
		DIV_ROUND_CLOSEST(recharge_soc * FULL_SOC_RAW, FULL_CAPACITY);
	...
	fg_dbg(fg, FG_STATUS,
	       "msoc: %d bsoc: %x health: %d status: %d full: %d\n", msoc, bsoc,
	       fg->health, fg->charge_status, fg->charge_full);
	if (fg->charge_done && !fg->charge_full) {
		...
		fg_dbg(fg, FG_STATUS,
		       "msoc_raw = %d bsoc: %d recharge_soc: %d delta_soc: %d\n",
...
static int fg_gen4_esr_fcc_config(struct fg_gen4_chip *chip)
{
	...
	if (is_parallel_charger_available(fg)) {
		rc = power_supply_get_property(
			fg->parallel_psy, POWER_SUPPLY_PROP_CHARGING_ENABLED,
			&prop);
		if (rc < 0)
			pr_err_ratelimited(
				"Error in reading charging_enabled from parallel_psy, rc=%d\n",
				rc);
		else
	...
	fg_dbg(fg, FG_POWER_SUPPLY,
	       "chg_sts: %d par_en: %d cp_en: %d qnov_en: %d esr_fcc_ctrl_en: %d\n",
					  ...
					  int temp_max)
{
	...
	rc = fg_sram_write(fg, fg->sp[FG_SRAM_ESR_CAL_TEMP_MIN].addr_word,
			   ...
			   fg->sp[FG_SRAM_ESR_CAL_TEMP_MIN].len,
			   FG_IMA_DEFAULT);
	if (rc < 0) {
	...
	rc = fg_sram_write(fg, fg->sp[FG_SRAM_ESR_CAL_TEMP_MAX].addr_word,
			   ...
			   fg->sp[FG_SRAM_ESR_CAL_TEMP_MAX].len,
			   FG_IMA_DEFAULT);
	if (rc < 0) {
...
static int fg_gen4_esr_fast_calib_config(struct fg_gen4_chip *chip, bool en)
{
	...
	if (en) {
		...
		vote(chip->delta_esr_irq_en_votable, DELTA_ESR_IRQ_VOTER, true,
		     0);
		...
		vote(chip->delta_esr_irq_en_votable, DELTA_ESR_IRQ_VOTER, false,
		     0);
	}
	...
	mask = BIT(6) | BIT(7);
	rc = fg_sram_masked_write(fg, SYS_CONFIG_WORD, SYS_CONFIG_OFFSET, mask,
				  val, FG_IMA_DEFAULT);
	if (rc < 0) {
	...
	if (en) {
		...
		if (chip->dt.esr_timer_chg_fast > 0 &&
			...
			esr_fast_cal_ms =
				3 * chip->dt.delta_esr_disable_count *
...
static int fg_gen4_set_vbatt_tau(struct fg_gen4_chip *chip, u8 vbatt_tau)
{
	...
	rc = fg_sram_read(fg, fg->sp[FG_SRAM_VBAT_TAU].addr_word,
			  fg->sp[FG_SRAM_VBAT_TAU].addr_byte, &buf,
			  fg->sp[FG_SRAM_VBAT_TAU].len, FG_IMA_DEFAULT);
	if (rc < 0) {
	...
	buf &= IBATT_TAU_MASK;
	buf |= vbatt_tau << 4;
	rc = fg_sram_write(fg, fg->sp[FG_SRAM_VBAT_TAU].addr_word,
			   fg->sp[FG_SRAM_VBAT_TAU].addr_byte, &buf,
			   fg->sp[FG_SRAM_VBAT_TAU].len, FG_IMA_DEFAULT);
	if (rc < 0)
...
static int fg_gen4_enter_soc_scale(struct fg_gen4_chip *chip)
{
	...
	chip->soc_scale_slope =
		DIV_ROUND_CLOSEST(chip->vbatt_res, chip->soc_scale_msoc);
...
static void fg_gen4_write_scale_msoc(struct fg_gen4_chip *chip)
{
	...
	if (!fg->charge_full) {
		soc_raw = DIV_ROUND_CLOSEST(chip->soc_scale_msoc * 0xFFFF, 100);
		rc = fg_sram_write(fg, fg->sp[FG_SRAM_MONOTONIC_SOC].addr_word,
...
static int fg_gen4_validate_soc_scale_mode(struct fg_gen4_chip *chip)
{
	...
	if (!chip->soc_scale_mode &&
	    fg->charge_status == POWER_SUPPLY_STATUS_DISCHARGING &&
	    chip->current_now > 0 && chip->vbatt_avg < vbatt_scale_mv) {
...
static int fg_gen4_set_vbatt_low(struct fg_gen4_chip *chip)
{
	...
	if (chip->soc_scale_mode) {
		rc = fg_get_sram_prop(fg, FG_SRAM_VBAT_FLT, &vbatt_flt);
		if (rc < 0) {
			pr_err("failed to get filtered battery voltage, rc=%d\n",
			       rc);
			/*
...
static irqreturn_t fg_vbatt_low_irq_handler(int irq, void *data)
{
	...
	if (vbatt_mv < chip->dt.cutoff_volt_mv) {
		if (chip->dt.rapid_soc_dec_en) {
			...
			if (chip->vbat_critical_low_count <
				    EMPTY_DEBOUNCE_TIME_COUNT_MAX &&
...
static irqreturn_t fg_delta_msoc_irq_handler(int irq, void *data)
{
	...
	if (rc < 0) {
		...
		if (chip->cl->active) {
			batt_soc_cp =
				div64_u64((u64)(u32)batt_soc * CENTI_FULL_SOC,
					  BATT_SOC_32BIT);
						     ...
						     ktime_t time)
{
	struct fg_gen4_chip *chip =
		container_of(alarm, struct fg_gen4_chip, esr_fast_cal_timer);
	struct fg_dev *fg = &chip->fg;
...
{
	struct fg_gen4_chip *chip =
		container_of(work, struct fg_gen4_chip, esr_calib_work);
	struct fg_dev *fg = &chip->fg;
	...
	if (chip->delta_esr_count > chip->dt.delta_esr_disable_count ||
		...
		fg_dbg(fg, FG_STATUS,
		       "delta_esr_count: %d esr_fast_calib_done:%d\n",
	...
	rc = fg_sram_read(fg, ESR_WORD, ESR_OFFSET, buf, 2, FG_IMA_DEFAULT);
	if (rc < 0) {
	...
	fg_dbg(fg, FG_STATUS,
	       "esr_raw: 0x%x esr_char_raw: 0x%x esr_meas_diff: 0x%x esr_delta: 0x%x\n",
	...
	fg_dbg(fg, FG_STATUS,
	       "fg_esr_meas_diff: 0x%x esr_filt: 0x%x esr_delta_new: 0x%x\n",
	...
	rc = fg_sram_write(fg, ESR_DELTA_CHG_WORD, ESR_DELTA_CHG_OFFSET, buf, 2,
			   FG_IMA_DEFAULT);
	if (rc < 0) {
		pr_err("Error in writing ESR_DELTA_CHG, rc=%d\n", rc);
		goto out;
...
{
	struct fg_gen4_chip *chip =
		container_of(alarm, struct fg_gen4_chip, soc_scale_alarm_timer);
...
static int calculate_average_current(struct fg_gen4_chip *chip)
{
	...
	fg->param.samples_index =
		(fg->param.samples_index + 1) % BATT_MA_AVG_SAMPLES;
	...
	if (fg->param.samples_num) {
		...
		for (i = 0; i < fg->param.samples_num; i++) {
			pr_debug("iavg_samples_ma[%d] = %d\n", i,
				 fg->param.batt_ma_avg_samples[i]);
			iavg_ma += fg->param.batt_ma_avg_samples[i];
		}
		fg->param.batt_ma_avg =
			DIV_ROUND_CLOSEST(iavg_ma, fg->param.samples_num);
	}
...
{
	struct fg_gen4_chip *chip =
		container_of(work, struct fg_gen4_chip, soc_scale_work);
	struct fg_dev *fg = &chip->fg;
	...
	mutex_lock(&chip->soc_scale_lock);
	soc = DIV_ROUND_CLOSEST(chip->vbatt_res, chip->soc_scale_slope);
	...
	if ((chip->prev_soc_scale_msoc - chip->msoc_actual) > soc_thr_percent) {
		 ...
		 */
		chip->soc_scale_msoc =
			chip->prev_soc_scale_msoc - soc_thr_percent;
		 ...
		 * of SOC.
		 */
		calculate_delta_time(&last_change_time,
		...
		fg_dbg(fg, FG_FVSS,
		       "get delta_time = %d, soc_changed =%d, time_since_last_change_sec = %d\n",
	...
	fg_dbg(fg, FG_FVSS,
	       "Calculated SOC=%d SOC reported=%d timer resolution=%d\n", soc,
	       chip->soc_scale_msoc, chip->scale_timer);
	alarm_start_relative(&chip->soc_scale_alarm_timer,
			     ms_to_ktime(chip->scale_timer));
}
...
{
	struct fg_gen4_chip *chip =
		container_of(work, struct fg_gen4_chip, pl_current_en_work);
	struct fg_dev *fg = &chip->fg;
...
{
	struct fg_gen4_chip *chip =
		container_of(work, struct fg_gen4_chip, pl_enable_work.work);
	struct fg_dev *fg = &chip->fg;
...
static void battery_authentic_work(struct work_struct *work)
{
	...
	rc = power_supply_get_property(fg->fg_psy, POWER_SUPPLY_PROP_AUTHENTIC,
				       &pval);
	if (pval.intval != true) {
		retry_battery_authentic_result++;
		if (retry_battery_authentic_result <
		    BATTERY_AUTHENTIC_COUNT_MAX) {
			pr_err("battery authentic work begin to restart.\n");
			schedule_delayed_work(
				&chip->battery_authentic_work,
...
static void ds_romid_work(struct work_struct *work)
{
	...
	struct fg_gen4_chip *chip =
		container_of(work, struct fg_gen4_chip, ds_romid_work.work);
	struct fg_dev *fg = &chip->fg;

	rc = power_supply_get_property(fg->fg_psy, POWER_SUPPLY_PROP_ROMID,
				       &pval);
	if (rc < 0) {
		retry_ds_romid++;
		...
		if (retry_ds_romid == DS_ROMID_COUNT_MAX) {
			pr_err("FG: romid prop is %02x %02x %02x %02x %02x %02x %02x %02x\n",
			       pval.arrayval[0], pval.arrayval[1],
			       pval.arrayval[2], pval.arrayval[3],
			       pval.arrayval[4], pval.arrayval[5],
			       pval.arrayval[6], pval.arrayval[7]);
		}
	} else {
		pr_err("FG: romid prop is %02x %02x %02x %02x %02x %02x %02x %02x\n",
		       pval.arrayval[0], pval.arrayval[1], pval.arrayval[2],
		       pval.arrayval[3], pval.arrayval[4], pval.arrayval[5],
		       pval.arrayval[6], pval.arrayval[7]);
	}
}
...
static void ds_status_work(struct work_struct *work)
{
	...
	struct fg_gen4_chip *chip =
		container_of(work, struct fg_gen4_chip, ds_status_work.work);
	struct fg_dev *fg = &chip->fg;

	rc = power_supply_get_property(fg->fg_psy, POWER_SUPPLY_PROP_DS_STATUS,
				       &pval);
	if (rc < 0) {
		retry_ds_status++;
...
static void ds_page0_work(struct work_struct *work)
{
	...
	struct fg_gen4_chip *chip =
		container_of(work, struct fg_gen4_chip, ds_page0_work.work);
	struct fg_dev *fg = &chip->fg;

	rc = power_supply_get_property(fg->fg_psy, POWER_SUPPLY_PROP_PAGE0_DATA,
				       &pval);
	if (rc < 0) {
		retry_ds_page0++;
		...
		if (retry_ds_page0 == DS_PAGE0_COUNT_MAX) {
			pr_err("FG: ds_page0 prop is %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x\n",
			       ...
			       pval.arrayval[4], pval.arrayval[5],
			       pval.arrayval[6], pval.arrayval[7],
			       pval.arrayval[8], pval.arrayval[9],
			       pval.arrayval[10], pval.arrayval[11],
			       pval.arrayval[12], pval.arrayval[13],
			       pval.arrayval[14], pval.arrayval[15]);
		}
	} else {
		pr_err("FG: ds_page0 prop is %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x\n",
		       pval.arrayval[0], pval.arrayval[1], pval.arrayval[2],
		       pval.arrayval[3], pval.arrayval[4], pval.arrayval[5],
		       pval.arrayval[6], pval.arrayval[7], pval.arrayval[8],
		       pval.arrayval[9], pval.arrayval[10], pval.arrayval[11],
		       pval.arrayval[12], pval.arrayval[13], pval.arrayval[14],
		       pval.arrayval[15]);
	}
}
...
{
	struct fg_dev *fg =
		container_of(work, struct fg_dev, status_change_work);
	struct fg_gen4_chip *chip = container_of(fg, struct fg_gen4_chip, fg);
	int rc, batt_soc, batt_temp, msoc_raw;
	...
	batt_soc_cp =
		div64_u64((u64)(u32)batt_soc * CENTI_FULL_SOC, BATT_SOC_32BIT);
	cap_learning_update(chip->cl, batt_temp, batt_soc_cp, fg->charge_status,
			    fg->charge_done, input_present, qnovo_en);
...
out:
	fg_dbg(fg, FG_STATUS,
	       "charge_status:%d charge_type:%d charge_done:%d\n",
...
{
	struct fg_dev *fg =
		container_of(work, struct fg_dev, sram_dump_work.work);
	u8 *buf;
	int rc;
...
static ssize_t sram_dump_en_store(struct device *dev,
				  struct device_attribute *attr,
				  const char *buf, size_t count)
{
	int rc;
...
static ssize_t sram_dump_en_show(struct device *dev,
				 struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%c\n", fg_sram_dump ? 'Y' : 'N');
}
...
static ssize_t restart_show(struct device *dev, struct device_attribute *attr,
			    char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d\n", fg_restart_mp);
}
...
static ssize_t esr_fast_cal_en_store(struct device *dev,
				     struct device_attribute *attr,
				     const char *buf, size_t count)
{
	int rc;
...
static ssize_t esr_fast_cal_en_show(struct device *dev,
				    struct device_attribute *attr, char *buf)
{
...
static struct attribute *fg_attrs[] = {
	&dev_attr_profile_dump.attr,	&dev_attr_sram_dump_period_ms.attr,
	&dev_attr_sram_dump_en.attr,	&dev_attr_restart.attr,
	&dev_attr_esr_fast_cal_en.attr, NULL,
};
ATTRIBUTE_GROUPS(fg);
...
static int fg_gen4_set_ki_coeff_curr(struct fg_dev *fg, bool enable_ffc)
{
	...
	fg_encode(fg->sp, FG_SRAM_KI_COEFF_LO_MED_CHG_THR, lo_med_curr, &val);
	rc = fg_sram_write(fg,
	...
	fg_encode(fg->sp, FG_SRAM_KI_COEFF_MED_HI_CHG_THR, med_hi_curr, &val);
	rc = fg_sram_write(fg,
			   fg->sp[FG_SRAM_KI_COEFF_MED_HI_CHG_THR].addr_word,
			   fg->sp[FG_SRAM_KI_COEFF_MED_HI_CHG_THR].addr_byte,
			   &val, fg->sp[FG_SRAM_KI_COEFF_MED_HI_CHG_THR].len,
			   FG_IMA_DEFAULT);
	if (rc < 0) {
			       ...
			       union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_CHIP_OK:
		...
		rc = power_supply_get_property(
			fg->max_verify_psy, POWER_SUPPLY_PROP_CHIP_OK, &b_val);
	...
	case POWER_SUPPLY_PROP_CAPACITY:
		...
		if (chip->dt.fg_increase_100soc_time) {
			if (fg->param.smooth_batt_soc >= 0 &&
			    !chip->rapid_soc_dec_en && !chip->soc_scale_mode)
...
static int fg_notifier_cb(struct notifier_block *nb, unsigned long event,
			  void *data)
{
...
static int fg_alg_init(struct fg_gen4_chip *chip)
{
	...
	if (rc < 0) {
		dev_err(fg->dev,
			"Error in initializing capacity learning, rc:%d\n", rc);
		counter->data = NULL;
		cl->data = NULL;
		devm_kfree(fg->dev, counter);
...
static int fg_gen4_esr_calib_config(struct fg_gen4_chip *chip)
{
	...
	if (chip->esr_fast_calib) {
		...
		if (chip->dt.esr_timer_chg_slow[TIMER_RETRY] >= 0 &&
			...
			rc = fg_set_esr_timer(
				fg, chip->dt.esr_timer_chg_slow[TIMER_RETRY],
		...
		if (chip->dt.esr_timer_dischg_slow[TIMER_RETRY] >= 0 &&
			...
			rc = fg_set_esr_timer(
				fg, chip->dt.esr_timer_dischg_slow[TIMER_RETRY],
				chip->dt.esr_timer_dischg_slow[TIMER_MAX],
				false, FG_IMA_DEFAULT);
			if (rc < 0) {
				pr_err("Error in setting ESR discharge timer, rc=%d\n",
	...
	fg_encode(fg->sp, FG_SRAM_DELTA_ESR_THR,
		  ...
		  buf);
	rc = fg_sram_write(fg, fg->sp[FG_SRAM_DELTA_ESR_THR].addr_word,
...
static int fg_gen4_batt_temp_config(struct fg_gen4_chip *chip)
{
	...
	if (chip->dt.batt_temp_cold_thresh != -EINVAL) {
		...
		rc = fg_sram_write(fg, fg->sp[FG_SRAM_BATT_TEMP_COLD].addr_word,
				   fg->sp[FG_SRAM_BATT_TEMP_COLD].addr_byte,
				   &buf, fg->sp[FG_SRAM_BATT_TEMP_COLD].len,
				   FG_IMA_DEFAULT);
		if (rc < 0) {
	...
	if (chip->dt.batt_temp_hot_thresh != -EINVAL) {
		...
		rc = fg_sram_write(fg, fg->sp[FG_SRAM_BATT_TEMP_HOT].addr_word,
				   fg->sp[FG_SRAM_BATT_TEMP_HOT].addr_byte,
				   &buf, fg->sp[FG_SRAM_BATT_TEMP_HOT].len,
				   FG_IMA_DEFAULT);
		if (rc < 0) {
	...
	if (chip->dt.batt_temp_hyst != -EINVAL) {
		...
		rc = fg_sram_masked_write(fg, BATT_TEMP_CONFIG2_WORD,
					  BATT_TEMP_HYST_DELTA_OFFSET, mask,
					  val, FG_IMA_DEFAULT);
		if (rc < 0) {
...
static int fg_gen4_init_ki_coeffts(struct fg_gen4_chip *chip)
{
	...
	if (chip->dt.ki_coeff_low_chg != -EINVAL) {
		...
		rc = fg_sram_write(fg,
				   ...
				   fg->sp[FG_SRAM_KI_COEFF_LOW_CHG].addr_byte,
				   &val, fg->sp[FG_SRAM_KI_COEFF_LOW_CHG].len,
				   FG_IMA_DEFAULT);
		if (rc < 0) {
	...
	if (chip->dt.ki_coeff_med_chg != -EINVAL) {
		...
		rc = fg_sram_write(fg,
				   ...
				   fg->sp[FG_SRAM_KI_COEFF_MED_CHG].addr_byte,
				   &val, fg->sp[FG_SRAM_KI_COEFF_MED_CHG].len,
				   FG_IMA_DEFAULT);
		if (rc < 0) {
	...
	if (chip->dt.ki_coeff_lo_med_chg_thr_ma != -EINVAL) {
		...
		rc = fg_sram_write(
			fg, fg->sp[FG_SRAM_KI_COEFF_LO_MED_CHG_THR].addr_word,
			fg->sp[FG_SRAM_KI_COEFF_LO_MED_CHG_THR].addr_byte, &val,
			fg->sp[FG_SRAM_KI_COEFF_LO_MED_CHG_THR].len,
			FG_IMA_DEFAULT);
		if (rc < 0) {
			pr_err("Error in writing ki_coeff_lo_med_chg_thr_ma, rc=%d\n",
			       rc);
	...
	if (chip->dt.ki_coeff_med_hi_chg_thr_ma != -EINVAL) {
		...
		rc = fg_sram_write(
			fg, fg->sp[FG_SRAM_KI_COEFF_MED_HI_CHG_THR].addr_word,
	...
	if (chip->dt.ki_coeff_lo_med_dchg_thr_ma != -EINVAL) {
		...
		rc = fg_sram_write(
			fg, fg->sp[FG_SRAM_KI_COEFF_LO_MED_DCHG_THR].addr_word,
	...
	if (chip->dt.ki_coeff_med_hi_dchg_thr_ma != -EINVAL) {
		...
		rc = fg_sram_write(
			fg, fg->sp[FG_SRAM_KI_COEFF_MED_HI_DCHG_THR].addr_word,
	...
	if (chip->dt.ki_coeff_cutoff_gain != -EINVAL) {
		...
		rc = fg_sram_write(fg,
				   ...
				   fg->sp[FG_SRAM_KI_COEFF_CUTOFF].addr_byte,
				   &val, fg->sp[FG_SRAM_KI_COEFF_CUTOFF].len,
				   FG_IMA_DEFAULT);
		if (rc < 0) {
...
static int fg_gen4_hw_init(struct fg_gen4_chip *chip)
{
	...
	if (chip->dt.empty_volt_mv > 0) {
		fg_encode(fg->sp, FG_SRAM_VBATT_LOW, chip->dt.empty_volt_mv,
			  buf);
	...
	fg_encode(fg->sp, FG_SRAM_DELTA_MSOC_THR, chip->dt.delta_soc_thr, buf);
	rc = fg_sram_write(fg, fg->sp[FG_SRAM_DELTA_MSOC_THR].addr_word,
			   fg->sp[FG_SRAM_DELTA_MSOC_THR].addr_byte, buf,
			   fg->sp[FG_SRAM_DELTA_MSOC_THR].len, FG_IMA_DEFAULT);
	if (rc < 0) {
		pr_err("Error in writing delta_msoc_thr, rc=%d\n", rc);
		return rc;
	}

	fg_encode(fg->sp, FG_SRAM_DELTA_BSOC_THR, chip->dt.delta_soc_thr, buf);
	rc = fg_sram_write(fg, fg->sp[FG_SRAM_DELTA_BSOC_THR].addr_word,
			   fg->sp[FG_SRAM_DELTA_BSOC_THR].addr_byte, buf,
			   fg->sp[FG_SRAM_DELTA_BSOC_THR].len, FG_IMA_DEFAULT);
	if (rc < 0) {
	...
	rc = fg_sram_write(fg, fg->sp[FG_SRAM_ESR_PULSE_THRESH].addr_word,
			   ...
			   fg->sp[FG_SRAM_ESR_PULSE_THRESH].len,
			   FG_IMA_DEFAULT);
	if (rc < 0) {
...
static int fg_parse_ki_coefficients(struct fg_dev *fg)
{
	...
	if (of_find_property(node, "qcom,ki-coeff-full-dischg", NULL)) {
		rc = fg_parse_dt_property_u32_array(
			node, "qcom,ki-coeff-full-dischg",
...
static int fg_parse_esr_cal_params(struct fg_dev *fg)
{
	...
	if (chip->dt.esr_timer_dischg_slow[TIMER_RETRY] >= 0 &&
		...
		chip->dt.esr_calib_dischg =
			of_property_read_bool(node, "qcom,fg-esr-calib-dischg");
		if (chip->dt.esr_calib_dischg)
			return 0;
	}
	...
	rc = of_property_read_u32(node, "qcom,fg-esr-filter-factor", &temp);
	if (!rc)
...
static int fg_gen4_parse_child_nodes_dt(struct fg_gen4_chip *chip)
{
	...
	for_each_available_child_of_node (node, child) {
		...
		if (rc < 0) {
			dev_err(fg->dev,
				"reg not specified in node %s, rc=%d\n",
				child->full_name, rc);
			return rc;
		}
		...
		if (rc < 0) {
			dev_err(fg->dev,
				"Couldn't read subtype for base %d, rc=%d\n",
				base, rc);
			return rc;
		}
...
static int fg_gen4_parse_dt(struct fg_gen4_chip *chip)
{
	...
	if (size) {
		chip->dt.dec_rate_seq = devm_kzalloc(fg->dev, size, GFP_KERNEL);
	...
	rc = fg_parse_dt_property_u32_array(node,
					    ...
					    chip->dt.esr_timer_dischg_slow,
					    NUM_ESR_TIMERS);
	if (rc < 0)
		return rc;

	chip->dt.force_load_profile =
		of_property_read_bool(node, "qcom,fg-force-load-profile");

	chip->dt.shutdown_delay_enable =
		of_property_read_bool(node, "qcom,shutdown-delay-enable");
	chip->dt.cutoff_voltage_adjust_enable = of_property_read_bool(
		node, "qcom,cutoff-voltage-adjust-enable");

	fg_gen4_parse_cl_params_dt(chip);
	fg_gen4_parse_batt_temp_dt(chip);

	chip->dt.hold_soc_while_full =
		of_property_read_bool(node, "qcom,hold-soc-while-full");

	chip->dt.linearize_soc =
		of_property_read_bool(node, "qcom,linearize-soc");

	chip->dt.soc_scale_mode =
		of_property_read_bool(node, "qcom,soc-scale-mode-en");
	if (chip->dt.soc_scale_mode) {
	...
	chip->dt.rapid_soc_dec_en =
		of_property_read_bool(node, "qcom,rapid-soc-dec-en");

	chip->dt.five_pin_battery =
		of_property_read_bool(node, "qcom,five-pin-battery");
	chip->dt.multi_profile_load =
		of_property_read_bool(node, "qcom,multi-profile-load");
	chip->dt.soc_hi_res = of_property_read_bool(node, "qcom,soc-hi-res");

	chip->dt.fg_increase_100soc_time =
		of_property_read_bool(node, "qcom,fg-increase-100soc-time");
	fg->param.smooth_batt_flag =
		of_property_read_bool(node, "qcom,fg-increase-100soc-time-2");
...
static void fg_battery_soc_smooth_tracking(struct fg_gen4_chip *chip)
{
	...
	if (last_batt_soc >= 0) {
		if (last_batt_soc != 100 && fg->param.batt_raw_soc >= 95 &&
		    fg->charge_status == POWER_SUPPLY_STATUS_FULL)
			// Unlikely status
	...
	if (fg->param.batt_soc != last_batt_soc) {
		...
		if (chip->dt.fg_increase_100soc_time) {
			if ((fg->charge_status ==
			     POWER_SUPPLY_STATUS_CHARGING) &&
			    fg->param.batt_soc > 96 &&
			    scale_count < SCALE_SOC) {
				fg->param.smooth_batt_soc -= DETAL_SOC;
				fg->param.smooth_full_soc =
					fg->param.smooth_batt_soc;
				scale_count++;
			} else {
				scale_count = 0;
	...
	if (chip->dt.fg_increase_100soc_time) {
		...
		if ((fg->charge_status == POWER_SUPPLY_STATUS_CHARGING ||
		     fg->param.batt_ma < 0) &&
		    (last_batt_soc < 3) && off_charge_flag &&
		    (fg->param.smooth_low_batt_soc <
		     fg->param.smooth_batt_soc)) {
			pr_info("smooth_low_batt_soc%d",
				fg->param.smooth_low_batt_soc);
		...
		if (last_smooth_batt_soc > 0) {
			...
			if ((fg->param.smooth_batt_soc - last_smooth_batt_soc) >
			    1) {
				fg->param.smooth_batt_soc =
					last_smooth_batt_soc + DETAL_SOC;
				pr_info("batt_soc:++\n");
			} else if ((last_smooth_batt_soc -
				    fg->param.smooth_batt_soc) > 1) {
				fg->param.smooth_batt_soc =
					last_smooth_batt_soc - DETAL_SOC;
				pr_info("batt_soc:--\n");
			}
			pr_info("last_smooth_batt_soc:%d, smooth_batt_soc:%d\n",
				last_smooth_batt_soc,
				fg->param.smooth_batt_soc);
		...
		if ((last_smooth_batt_soc == 0) && !off_charge_flag &&
		    (last_batt_soc >= 2) && (last_batt_soc <= 5)) {
...
{
	struct fg_dev *fg =
		container_of(work, struct fg_dev, empty_restart_fg_work.work);
	union power_supply_propval prop = {
		0,
	};
	int usb_present = 0;
	...
	if (usb_psy_initialized(fg)) {
		rc = power_supply_get_property(
			fg->usb_psy, POWER_SUPPLY_PROP_PRESENT, &prop);
		if (rc < 0) {
	...
	if (!usb_present) {
		if (fg->profile_load_status == PROFILE_LOADED) {
			...
			schedule_delayed_work(
				&fg->soc_monitor_work,
				msecs_to_jiffies(
					RESTART_FG_MONITOR_SOC_WAIT_PER_MS));
		} else {
			schedule_delayed_work(
...
static int fg_dynamic_set_cutoff_voltage(struct fg_dev *fg, int cut_off_mv)
{
	int rc;
	u8 buf[4];
...
static void soc_monitor_work(struct work_struct *work)
{
	int rc;
	struct fg_dev *fg =
		container_of(work, struct fg_dev, soc_monitor_work.work);
	struct fg_gen4_chip *chip = container_of(fg, struct fg_gen4_chip, fg);
	...
	pr_info("soc:%d, raw_soc:%d, c:%d, s:%d\n", fg->param.batt_soc,
		fg->param.batt_raw_soc, fg->param.batt_ma, fg->charge_status);
...
static int fg_gen4_probe(struct platform_device *pdev)
{
...
#ifdef CONFIG_BATT_VERIFY_BY_DS28E16_PIPA
	INIT_DELAYED_WORK(&chip->battery_authentic_work,
			  battery_authentic_work);
	...
	fg->awake_votable =
		create_votable("FG_WS", VOTE_SET_ANY, fg_awake_cb, fg);
	...
	fg->delta_bsoc_irq_en_votable = create_votable(
		"FG_DELTA_BSOC_IRQ", VOTE_SET_ANY, fg_delta_bsoc_irq_en_cb, fg);
	...
	chip->delta_esr_irq_en_votable =
		create_votable("FG_DELTA_ESR_IRQ", VOTE_SET_ANY,
			       fg_gen4_delta_esr_irq_en_cb, chip);
	...
	chip->mem_attn_irq_en_votable =
		create_votable("FG_MEM_ATTN_IRQ", VOTE_SET_ANY,
	...
	chip->parallel_current_en_votable = create_votable(
		"FG_SMB_MEAS_EN", VOTE_SET_ANY, fg_parallel_current_en_cb, fg);
	...
	if (chip->esr_fast_calib) {
		if (alarmtimer_get_rtcdev()) {
			...
			dev_err(fg->dev,
				"Failed to initialize esr_fast_cal timer\n");
			rc = -EPROBE_DEFER;
			goto exit;
		}
	}
	...
	if (chip->dt.soc_scale_mode) {
		if (alarmtimer_get_rtcdev()) {
			alarm_init(&chip->soc_scale_alarm_timer, ALARM_BOOTTIME,
				   fg_soc_scale_timer);
		} else {
			dev_err(fg->dev,
				"Failed to initialize SOC scale timer\n");
			rc = -EPROBE_DEFER;
			goto exit;
		}
	}
	...
	fg->fg_psy =
		devm_power_supply_register(fg->dev, &fg_psy_desc, &fg_psy_cfg);
	if (IS_ERR(fg->fg_psy)) {
	...
	if (IS_ERR_OR_NULL(fg->tz_dev)) {
		...
		fg->tz_dev = NULL;
		dev_dbg(fg->dev,
			"Couldn't register with thermal framework rc:%d\n", rc);
	}
...
early_param("androidboot.profile_vendor_id",
	    early_parse_batt_profile_vendor_id);
