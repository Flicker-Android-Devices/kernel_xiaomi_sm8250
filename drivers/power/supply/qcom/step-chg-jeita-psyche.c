...
static int get_step_chg_jeita_setting_from_profile(struct step_chg_info *chip)
{
	...
	power_supply_get_property(chip->bms_psy, POWER_SUPPLY_PROP_CYCLE_COUNT,
				  &prop);
	...
	if (cycle_count_num < 100)
		...
		rc = read_range_data_from_node(
			profile_node, "qcom,step-chg-high-cc-ranges",
			chip->step_chg_config->fcc_cfg,
			chip->soc_based_step_chg ? 100 : max_fv_uv,
			max_fcc_ma * 1000);
	if (rc < 0) {
...
static void handle_chg_iterm(struct step_chg_info *chip, int temp)
{
	...
	rc = power_supply_get_property(
		chip->bms_psy, POWER_SUPPLY_PROP_FFC_TERMINATION_CURRENT,
		&pval);

	if ((temp < (FFC_ITERM_TEMP - FFC_ITERM_TEMP_HYST)) && ffc_temp_high) {
		ffc_temp_high = false;
		rc = power_supply_set_property(
			chip->usb_psy, POWER_SUPPLY_PROP_FFC_ITERM, &pval);
		pr_info("ffc iterm current [%d], battery temp [%d]\n",
			pval.intval, temp);
	} else if ((temp > (FFC_ITERM_TEMP + FFC_ITERM_TEMP_HYST)) &&
		   !ffc_temp_high) {
		ffc_temp_high = true;
		rc = power_supply_set_property(
			chip->usb_psy, POWER_SUPPLY_PROP_FFC_ITERM, &pval);
