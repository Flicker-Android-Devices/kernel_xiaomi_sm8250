...
static bool max28200_set_watchdog_enable(struct max28200_chip *max, bool enable)
{
	...
	if (max->cp_en_gpio) {
		...
		rc = regmap_update_bits(max->regmap, MAX28200_BB_REG,
					MAX28200_P00_MAKS, val);
		if (rc) {
...
static int max28200_psy_register(struct max28200_chip *max)
{
	...
	max->max28200_psy = devm_power_supply_register(
		max->dev, &max->max28200_psy_d, &max28200_psy_cfg);
...
static int max28200_notifier_call(struct notifier_block *nb, unsigned long ev,
				  void *v)
{
	...
	if ((strcmp(psy->desc->name, "usb") == 0)) {
		rc = power_supply_get_property(psy, POWER_SUPPLY_PROP_PRESENT,
					       &pval);
		if (rc < 0) {
			return NOTIFY_OK;
		}
...
static void max28200_monitor_workfunc(struct work_struct *work)
{
	struct max28200_chip *max =
		container_of(work, struct max28200_chip, monitor_work.work);
...
static int max_parse_dt(struct max28200_chip *max)
{
	...
	if ((!gpio_is_valid(max->cp_en_gpio))) {
		...
		if (ret) {
			max_dbg(PR_OEM,
				"%s: unable to request max cp en gpio [%d]\n",
				__func__, max->cp_en_gpio);
		}
		ret = gpio_direction_output(max->cp_en_gpio, 0);
		if (ret) {
			max_dbg(PR_OEM,
				"%s: unable to set direction cp en gpio[%d]\n",
				__func__, max->cp_en_gpio);
		}
