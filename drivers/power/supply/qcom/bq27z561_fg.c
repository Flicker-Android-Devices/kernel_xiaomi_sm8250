...
static int fg_get_temp_max_fac(struct bq_fg_chip *bq)
{
	...
	ret = fg_mac_read_block(bq, FG_MAC_CMD_LIFETIME1, data_limetime1,
...
static int fg_get_time_ot(struct bq_fg_chip *bq)
{
	...
	ret = fg_mac_read_block(bq, FG_MAC_CMD_LIFETIME3, data_limetime3,
	...
	if (data[2] == 'C') //TI
	{
		ret = fg_mac_read_block(bq, FG_MAC_CMD_FW_VER, data,
					sizeof(data));
		if (ret)
			   ...
			   union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_MAX:
		...
		if (val->intval == BQ_MAXIUM_VOLTAGE_FOR_CELL) {
			...
			if (bq->batt_volt >
			    BQ_MAXIUM_VOLTAGE_FOR_CELL + VOLTAGE_FOR_CELL_HYS) {
#else
			if (bq->batt_volt >
			...
			val->intval = BQ_PACK_MAXIUM_VOLTAGE_FOR_PMIC -
				      bq->cell_ov_check * 10;
					  ...
					  int batt_ma)
{
	...
	bq_dbg(PR_DEBUG,
	       ...
	       optimiz_soc[bq->fg_index]);
	bq_dbg(PR_DEBUG,
	       "delta_time:%d, change_delta:%d, unit_time:%d"
	       " soc_changed:%d, bq->update_now:%d, bq->ffc_smooth",
	       delta_time, change_delta, unit_time, soc_changed, bq->update_now,
	       bq->ffc_smooth);
