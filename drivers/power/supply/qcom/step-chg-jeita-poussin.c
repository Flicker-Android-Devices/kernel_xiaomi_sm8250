			       ...
			       int current_voltage)
{
	...
	power_supply_get_property(chip->usb_psy, POWER_SUPPLY_PROP_APDO_MAX,
				  &pval);
	pps_max_watts = pval.intval;
	power_supply_get_property(chip->bms_psy, POWER_SUPPLY_PROP_CAPACITY,
				  &pval);
	...
	if (pps_max_watts >= 40) { // for 67w pd
		if (pval.intval >= TAPERED_STEP_SOC_90 &&
		    target_fcc >= TAPERED_STEP_SOC_FCC_90_67W)
			target_fcc = TAPERED_STEP_SOC_FCC_90_67W;
		else if (pval.intval >= TAPERED_STEP_SOC_80 &&
			 target_fcc >= TAPERED_STEP_SOC_FCC_80_67W)
			target_fcc = TAPERED_STEP_SOC_FCC_80_67W;
		else if (pval.intval >= TAPERED_STEP_SOC_70 &&
			 target_fcc >= TAPERED_STEP_SOC_FCC_70_67W)
			target_fcc = TAPERED_STEP_SOC_FCC_70_67W;
		else if (pval.intval >= TAPERED_STEP_SOC_60 &&
...
static int handle_step_chg_config(struct step_chg_info *chip)
{
	...
	if (chip->use_bq_gauge) {
		rc = power_supply_get_property(
			chip->bms_psy, POWER_SUPPLY_PROP_VOLTAGE_MAX, &pval);
		...
		if (cv_count >= CV_RISE_VOLT_DELAY && bms_uv < FFC_MAX_VOLT) {
			...
			pr_err("cv_fv_state = %d-%d,bms_uv=%d\n", cv_fv_state,
			       cv_count, bms_uv);
		} else if (cv_count >= CV_RISE_VOLT_DELAY &&
			   bms_uv >= FFC_MAX_VOLT) {
			cv_fv_state = 0;
			cv_count = 0;
		}
		...
		rc = power_supply_get_property(
			chip->bms_psy, POWER_SUPPLY_PROP_CURRENT_MAX, &pval);
		if (rc >= 0 && chip->fcc_votable && pval.intval > 0)
			vote(chip->fcc_votable, STEP_BMS_CHG_VOTER, false,
			     pval.intval);
		fcc_ua = pval.intval;
...
static int handle_fast_charge(struct step_chg_info *chip, int temp)
{
	...
	if (is_dc_wls_available(chip)) {
		...
		rc = power_supply_get_property(
			chip->wls_psy, POWER_SUPPLY_PROP_WIRELESS_CP_EN, &pval);
		if (rc < 0)
	...
	rc = power_supply_get_property(
		chip->usb_psy, POWER_SUPPLY_PROP_PD_AUTHENTICATION, &pval);
	if (rc < 0) {
...
static int handle_jeita(struct step_chg_info *chip)
{
	...
	pr_info("%s = %d FCC = %duA FV = %duV %s = %d max voltage= %duv battery warm = %d battery cool = %d\n",
		...
		chip->jeita_fcc_config->param.prop_name, temp, pval.intval,
	...
	if (fv_uv > 0) {
		...
		if (!chip->six_pin_battery) {
			if ((curr_vbat_uv > fv_uv) &&
			    (temp >= chip->jeita_warm_th))
				vote(chip->usb_icl_votable, JEITA_VOTER, true,
				     0);
			else if (curr_vbat_uv < (fv_uv - JEITA_SUSPEND_HYST_UV))
				vote(chip->usb_icl_votable, JEITA_VOTER, false,
				     0);
		} else {
			curr_vfloat_uv = get_effective_result(chip->fv_votable);

			rc = power_supply_get_property(
				chip->batt_psy, POWER_SUPPLY_PROP_CHARGE_TYPE,
				&pval);
			if (rc < 0) {
			...
			if (curr_vfloat_uv != WARM_VFLOAT_UV) {
				if (curr_vbat_uv >
				    fv_uv + JEITA_SIX_PIN_BATT_HYST_UV) {
					if (pval.intval ==
						    POWER_SUPPLY_CHARGE_TYPE_TAPER &&
					    fv_uv == WARM_VFLOAT_UV)
						vote(chip->usb_icl_votable,
						     JEITA_VOTER, true, 0);
				} else if (curr_vbat_uv <
					   (fv_uv - JEITA_SUSPEND_HYST_UV)) {
