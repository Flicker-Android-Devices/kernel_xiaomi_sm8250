...
#define smblib_dbg(chg, reason, fmt, ...)                                      \
	do {                                                                   \
		if (*chg->debug_mask & (reason))                               \
			pr_info("%s: %s: " fmt, chg->name, __func__,           \
				##__VA_ARGS__);                                \
		else                                                           \
...
int smblib_set_fastcharge_mode(struct smb_charger *chg, bool enable)
{
	...
	rc = power_supply_get_property(chg->bms_psy, POWER_SUPPLY_PROP_CAPACITY,
				       &pval);
	if (rc < 0) {
	...
	if (enable && pval.intval >= 95) {
		smblib_dbg(chg, PR_MISC,
			   "soc:%d is more than 95do not setfastcharge mode\n",
			   pval.intval);
		enable = false;
	}
	...
	if (enable && (pval.intval >= chg->chg_warm_threshold ||
		...
		smblib_dbg(chg, PR_MISC,
			   "temp:%d is abortdo not setfastcharge mode\n",
			   pval.intval);
		enable = false;
	}
...
set_term:
	...
	if (batt_temp < chg->chg_warm_threshold &&
		...
		rc = vote(chg->fcc_votable, PD_VERIFED_VOTER, !enable,
	...
	if (chg->support_ffc) {
		...
		if (chg->smartBatVal) {
			...
			pr_info("Now fastcharge mode effective FV: %d\n",
				effective_fv);
			if (enable)
				vote(chg->fv_votable, SMART_BATTERY_FV, true,
				     chg->batt_profile_fv_uv -
					     chg->smartBatVal * 1000);
			else
				vote(chg->fv_votable, SMART_BATTERY_FV, true,
				     chg->non_fcc_batt_profile_fv_uv -
					     chg->smartBatVal * 1000);
...
static void smblib_dc_chg_q1_enable(struct smb_charger *chg, bool enable)
{
	...
	if (enable) {
		...
		if (rc)
			smblib_err(chg,
				   "unable to set dc_chg_en to high rc = %d\n",
				   rc);
	} else {
	...
	if (chg->batt_verify_psy && chg->bms_psy) {
		...
		if (!authen_result) {
			...
			if (rc)
				smblib_err(
					chg,
					"set batt_verify authentic prop failed: %d\n",
					rc);
		}
	...
	if (!strcmp(psy->desc->name, "bms")) {
		...
		if (ev == PSY_EVENT_PROP_CHANGED) {
			...
			if (rc < 0)
				smblib_err(
					chg,
					"Couldn't get batt verify status rc=%d\n",
					rc);
...
int smblib_get_prop_battery_input_suspend(struct smb_charger *chg,
					  union power_supply_propval *val)
{
	val->intval = (get_client_vote(chg->fcc_votable, USER_VOTER) == 0);
	return 0;
}
				     ...
				     union power_supply_propval *val)
{
	...
	if (!chg_en && chg->power_good_en && val->intval) {
		smblib_dbg(chg, PR_OEM,
			   "force recharge when charge done error\n");
		val->intval = 0;
		pval.intval = true;
		rc = power_supply_set_property(
			chg->batt_psy, POWER_SUPPLY_PROP_FORCE_RECHARGE, &pval);
		if (rc < 0) {
			smblib_dbg(chg, PR_OEM,
				   "could not set force recharging!\n");
			return rc;
		}
		return rc;
	}
...
int smblib_set_prop_batt_capacity(struct smb_charger *chg,
				  const union power_supply_propval *val)
{
	union power_supply_propval shutdown_delay_en = {
		0,
	};
	...
	case POWER_SUPPLY_TYPE_USB_HVDCP_3:
		if (get_client_vote_locked(chg->usb_icl_votable,
			...
			if (rc < 0)
				smblib_err(
					chg,
					"Couldn't disable USB thermal ICL vote rc=%d\n",
					rc);
		}
	...
	if (chg->system_temp_level == 0) {
		...
		if (thermal_icl_ua > 0) {
			...
			if (rc < 0)
				smblib_err(
					chg,
					"Couldn't enable USB thermal ICL vote rc=%d\n",
					rc);
		}
		if (thermal_fcc_ua > 0) {
			rc = vote(chg->fcc_votable, THERMAL_DAEMON_VOTER, true,
				      ...
				      const union power_supply_propval *val)
{
	...
	smblib_dbg(
		chg, PR_OEM,
		"thermal level:%d, thermal_levels:%d chg->system_temp_level:%d, charger_type:%d\n",
					  ...
					  const union power_supply_propval *val)
{
	...
	smblib_dbg(
		chg, PR_OEM,
		"Before process fv:%d, charging status:%d, Down value: %d.\n",
			       ...
			       union power_supply_propval *val)
{
	...
	if (battery_input_suspend && (!chg->night_chg_flag || capacity <= 75)) {
		pval.intval = 0;
...
static void smblib_reg_work(struct work_struct *work)
{
	...
	int charger_type, typec_mode, typec_orientation, esr_uohms_nominal,
		esr_uohms_actual, resistance;
	...
	if (usb_present) {
		...
		power_supply_get_property(chg->bms_psy,
					  POWER_SUPPLY_PROP_ESR_NOMINAL, &val);
		esr_uohms_nominal = val.intval;

		power_supply_get_property(chg->bms_psy,
					  POWER_SUPPLY_PROP_ESR_ACTUAL, &val);
		...
		smblib_dbg(
			...
			icl_settle, usb_cur_in, usb_vol_in, resistance,
			esr_uohms_nominal, esr_uohms_actual);
		if (!chg->usb_main_psy) {
	...
	case POWER_SUPPLY_DP_DM_DP_PULSE:
		...
		if (chg->real_charger_type == POWER_SUPPLY_TYPE_USB_HVDCP_3 &&
			...
			if (chg->pulse_cnt > MAX_QC3P5_PLUSE_COUNT_ALLOWED)
				return rc;
			else if (chg->pulse_cnt >=
				 QC3P5_DP_RAPIDLY_TUNE_ALLOWED)
		...
		if (chg->qc3p5_dp_tune_rapidly) {
			for (qc3p5_dp_cnt = 1;
			     qc3p5_dp_cnt <= QC3P5_DP_RAPIDLY_TUNE_PULSE;
			     qc3p5_dp_cnt++) {
				rc = smblib_dp_pulse(chg);
				if (rc < 0) {
					smblib_err(
						chg,
						"Couldn't increase pulse count rc=%d\n",
						rc);
					chg->pulse_cnt--;
				}
				smblib_dbg(
					chg, PR_PARALLEL,
					"DP_DM_DP_PULSE rapidly rc=%d cnt=%d\n",
					rc,
					chg->pulse_cnt -
						QC3P5_DP_RAPIDLY_TUNE_PULSE +
						qc3p5_dp_cnt);
...
int smblib_get_prop_batt_slave_temp(struct smb_charger *chg)
{
	...
	if (rc < 0) {
		while (retry < TEMP_READ_RETRY) {
			rc = smblib_read_iio_channel(
				chg, chg->iio.batt_slave_temp_chan,
		...
		if (retry >= TEMP_READ_RETRY) {
			smblib_err(chg,
				   "Couldn't read battery temp slave, rc=%d\n",
				   rc);
			return rc;
		}
	}
...
static void smblib_conn_therm_work(struct work_struct *work)
{
	...
	if (thermal_status != chg->thermal_status) {
		...
		if (thermal_status == TEMP_ABOVE_RANGE) {
			smblib_dbg(
				chg, PR_OEM,
				"connect temp is too hot, disable vbus, sink_src_mode[%d]\n",
				chg->sink_src_mode);
			...
			smblib_dbg(
				chg, PR_OEM,
				"connect temp normal recovery vbus, sink_src_mode[%d]\n",
				chg->sink_src_mode);
	...
	if (!usb_present && thermal_status == TEMP_BELOW_RANGE &&
		...
		if (get_client_vote(chg->usb_icl_votable, USB_PSY_VOTER) ==
		    500000)
			vote(chg->usb_icl_votable, USB_PSY_VOTER, false, 0);
		return;
	}
				   ...
				   union power_supply_propval *val)
{
	...
	if (chg->wls_psy) {
		rc = power_supply_get_property(
			chg->wls_psy, POWER_SUPPLY_PROP_VOLTAGE_MAX, val);
		if (rc < 0) {
			       ...
			       union power_supply_propval *val)
{
	...
	if (chg->ext_bbc) {
		rc = smblib_get_prop_from_bbc(chg, POWER_SUPPLY_PROP_PRESENT,
					      &pval);
		if (rc < 0)
			val->intval = false;
		val->intval = pval.intval;
				 ...
				 union power_supply_propval *val)
{
	...
	if (chg->iio.temp_chan) {
		...
		if (rc < 0) {
			smblib_err(chg,
				   "Error in reading temp channel, rc=%d\n",
				   rc);
			return rc;
		}
		val->intval = temp / 100;
				    ...
				    union power_supply_propval *val)
{
	...
	if (chg->iio.usbin_i_chan) {
		...
		if (rc < 0) {
			smblib_err(chg,
				   "Error in reading USBIN_I channel, rc=%d\n",
				   rc);
			return rc;
		}
...
static int smblib_handle_usb_current(struct smb_charger *chg, int usb_current)
{
	...
	if (chg->real_charger_type == POWER_SUPPLY_TYPE_USB_FLOAT) {
		...
		if (rc < 0) {
			smblib_err(chg,
				   "Couldn't remove SW_ICL_MAX vote rc=%d\n",
				   rc);
			return rc;
		}
	}
	...
	struct smb_charger *chg, const union power_supply_propval *val)
{
	...
	if (val->intval == 1 && !chg->power_good_en) {
		smblib_err(
			chg,
			"Type-C snk disconnect and no wireless charging, set bq_en q1 gpio to low now\n");
				    ...
				    const union power_supply_propval *val)
{
	...
	if (val->intval < TYPEC_SRC_RP_MAX_ELEMENTS) {
		...
		rc = smblib_masked_write(chg, TYPE_C_CURRSRC_CFG_REG,
					 TYPEC_SRC_RP_SEL_MASK, val->intval);
		if (rc < 0)
			smblib_err(
				chg,
			      ...
			      const union power_supply_propval *val)
{
	...
	if (chg->real_charger_type == POWER_SUPPLY_TYPE_USB && !chg->ok_to_pd) {
		typec_mode = smblib_get_prop_typec_mode(chg);
				     ...
				     int therm_overheat)
{
	...
	if (chg->cp_disable_votable) {
		vote(chg->cp_disable_votable, OVERHEAT_LIMIT_VOTER,
		     therm_overheat, 0);
		vote(chg->usb_icl_votable, OVERHEAT_LIMIT_VOTER, therm_overheat,
		     icl_ua);
	}
	...
	if (chg->wireless_bq) {
		if (dc_power_on) {
			...
			if (chg->power_good_en) {
				...
				smblib_err(chg,
					   "dc_power_on without power good\n");
			}
			vote(chg->awake_votable, DC_AWAKE_VOTER, true, 0);
		} else {
...
static void smblib_eval_chg_termination(struct smb_charger *chg, u8 batt_status)
{
	...
	if (chg->ext_fg)
		rc = smblib_get_prop_from_bms(
			chg, POWER_SUPPLY_PROP_CAPACITY_RAW, &pval);
	else
		rc = smblib_get_prop_from_bms(
			chg, POWER_SUPPLY_PROP_REAL_CAPACITY, &pval);
	if (rc < 0) {
		smblib_err(chg, "Couldn't read SOC value, rc=%d\n", rc);
		return;
	}
...
irqreturn_t usbin_uv_irq_handler(int irq, void *data)
{
	...
	if (!chg->qc2_unsupported_voltage && !chg->qc2_unsupported &&
		...
		if (rc < 0)
			smblib_err(chg,
				   "Failed to set adapter allowance to 5V\n");
		rc = smblib_set_opt_switcher_freq(chg, chg->chg_freq.freq_5V);
		if (rc < 0)
...
static int smblib_role_switch_failure(struct smb_charger *chg)
{
	...
	if (pval.intval) {
		smblib_dbg(
			chg, PR_MISC,
			" Role reversal failed, notifying device mode to usb driver.\n");
...
static int typec_partner_register(struct smb_charger *chg)
{
	...
	if (!chg->typec_partner) {
		...
		chg->typec_partner = typec_register_partner(
			chg->typec_port, &chg->typec_partner_desc);
		if (IS_ERR(chg->typec_partner)) {
			rc = PTR_ERR(chg->typec_partner);
			smblib_err(chg,
				   "failed to register typec_partner rc=%d\n",
				   rc);
			goto unlock;
		}
	}
...
{
	struct smb_charger *chg =
		container_of(work, struct smb_charger, check_init_boot.work);
...
{
	struct smb_charger *chg =
		container_of(work, struct smb_charger, check_sc8551_work.work);
	...
	if (master_en) {
		...
		power_supply_get_property(
			chg->cp_psy, POWER_SUPPLY_PROP_TI_VBUS_PRESENT, &val);
		master_vbus_present = val.intval;
		power_supply_get_property(
			chg->cp_psy, POWER_SUPPLY_PROP_TI_BUS_VOLTAGE, &val);
		master_vbus = val.intval;
		power_supply_get_property(
			chg->cp_psy, POWER_SUPPLY_PROP_TI_BUS_CURRENT, &val);
		...
		smblib_err(
			chg,
			"SC8551 Master: vbus_present:%d, Vbus:%d, Ibus:%d, Vbat:%d, Ibat:%d.\n",
			master_vbus_present, master_vbus, master_ibus,
			master_vbat, master_ibat);
	}
	if (slave_en) {
		...
		smblib_err(
			chg,
			"SC8551 Slave: vbus_present:%d, Vbus:%d, Ibus:%d, Vbat:%d, Ibat:%d.\n",
			slave_vbus_present, slave_vbus, slave_ibus, slave_vbat,
			slave_ibat);
	}

	if (master_en) {
		if (master_vbus_present && (master_vbus > 3000) &&
		    (master_ibus < 200)) {
			smblib_err(chg,
				   "Ibus low, close master charge pump.\n");
			val.intval = 0;
			power_supply_set_property(
				chg->cp_psy, POWER_SUPPLY_PROP_CHARGING_ENABLED,
				&val);
		}
	}
	if (slave_en) {
		if (slave_vbus_present && (slave_vbus > 3000) &&
		    (slave_ibus < 200)) {
...
void smblib_usb_plugin_hard_reset_locked(struct smb_charger *chg)
{
	...
	if (vbus_rising) {
		...
		if (chg->cp_psy)
			power_supply_get_property(
				chg->cp_psy, POWER_SUPPLY_PROP_TI_RESET_CHECK,
				&val);
		if (chg->cp_sec_psy)
			power_supply_get_property(
				chg->cp_sec_psy,
...
void smblib_usb_plugin_locked(struct smb_charger *chg)
{
	...
	if (chg->vbus_rising) {
		...
		if (!first_boot_flag)
			schedule_delayed_work(&chg->check_init_boot,
...
static bool qc3p5_vbus_timeout_check(struct smb_charger *chg, int timeout_ms,
				     int vbus_lo_bound, int vbus_hi_bound,
				     int *vbus_uv)
{
	union power_supply_propval pval;
	...
	if ((pval.intval <= vbus_lo_bound) || (pval.intval >= vbus_hi_bound) ||
	    (ktime_to_ms(delta_kt) > timeout_ms)) {
	...
	skip:
		...
		if (chg->is_qc_class_a)
			...
			if (!is_client_vote_enabled(chg->fcc_votable,
						    PD_VERIFED_VOTER))
				vote(chg->fcc_votable, PD_VERIFED_VOTER, false,
				     0);
...
static void update_sw_icl_max(struct smb_charger *chg, int pst)
{
	...
	if (pst == POWER_SUPPLY_TYPE_USB_HVDCP ||
		...
		if (pst == POWER_SUPPLY_TYPE_USB_HVDCP ||
			...
			pr_info("HVDCP update sw_icl_max effective FV: %d, chg->smartBatVal: %d",
				get_effective_result(chg->fv_votable),
				chg->smartBatVal);
			vote(chg->fv_votable, SMART_BATTERY_FV,
			     (chg->smartBatVal > 0) ? true : false,
	...
	pr_info("update sw_icl_max effective FV: %d, chg->smartBatVal: %d",
		get_effective_result(chg->fv_votable), chg->smartBatVal);
	vote(chg->fv_votable, SMART_BATTERY_FV,
	     (chg->smartBatVal > 0) ? true : false,
	     chg->non_fcc_batt_profile_fv_uv - chg->smartBatVal * 1000);
...
static const char *const dr_mode_text[] = { "ufp", "dfp", "none" };
...
{
	struct smb_charger *chg =
		container_of(cap, struct smb_charger, typec_caps);
	int rc = 0;
	...
	if ((chg->pr_swap_in_progress) || (type == TYPEC_PORT_DRP)) {
		smblib_dbg(
			chg, PR_MISC,
			"Ignoring port type request type = %d swap_in_progress = %d\n",
...
static void typec_src_removal(struct smb_charger *chg)
{
	...
	if (chg->sdam_base) {
		rc = smblib_write(chg, chg->sdam_base + SDAM_QC_DET_STATUS_REG,
				  0);
		if (rc < 0)
			smblib_err(chg, "Couldn't clear SDAM QC status rc=%d\n",
				   rc);

		rc = smblib_batch_write(
			chg, chg->sdam_base + SDAM_QC_ADC_LSB_REG, val, 2);
		if (rc < 0)
			smblib_err(chg,
				   "Couldn't clear SDAM ADC status rc=%d\n",
				   rc);
	}
...
irqreturn_t typec_state_change_irq_handler(int irq, void *data)
{
	...
	smblib_dbg(chg, PR_MISC,
		   "IRQ: cc-state-change; Type-C %s detected (%d)\n",
				...
				const union power_supply_propval *val)
{
	...
	if (val->intval) {
		...
		rc = vote(chg->dc_suspend_votable, SW_DISABLE_DC_VOTER, true,
			  0);
		...
		if (rc < 0) {
			dev_err(chg->dev,
				"Couldn't configure dc_en override rc=%d\n",
				rc);
			return rc;
		}
		...
		rc = smblib_masked_write(
			chg, DCIN_CMD_PULLDOWN_REG,
			DCIN_PULLDOWN_EN_BIT | DCIN_MID_PULLDOWN_BIT,
			DCIN_PULLDOWN_EN_BIT | DCIN_MID_PULLDOWN_BIT);
		if (rc < 0) {
			dev_err(chg->dev,
				"Couldn't enable dcin_pulldown rc=%d\n", rc);
			return rc;
		}
		...
		rc = vote(chg->dc_suspend_votable, SW_DISABLE_DC_VOTER, false,
			  0);
	} else {
		...
		if (rc < 0) {
			dev_err(chg->dev,
				"Couldn't disable dcin_pulldown rc=%d\n", rc);
			return rc;
		}
		/* wait 10ms to pull mid_chg lower than Vsys+Vrevi */
		usleep_range(10000, 11000);
...
static int
smblib_set_dc_pach_safe_when_power_good_enable(struct smb_charger *chg)
{
	int rc = 0;
	...
	rc = smblib_masked_write(chg, DCIN_CMD_IL_REG,
				 DCIN_EN_OVERRIDE_BIT | DCIN_EN_BIT, 0);
	if (rc < 0) {
		dev_err(chg->dev, "Couldn't configure dc_en override rc=%d\n",
			rc);
		return rc;
					 ...
					 const union power_supply_propval *val)
{
	...
	if (chg->power_good_en) {
		...
		if (rc < 0)
			smblib_err(
				chg,
				"Failed to set adapter allowance to 5V or 9v to 12v\n");
		...
		if (rc < 0)
			smblib_err(
				chg,
				"Couldn't set USBIN_ADAPTER_ALLOW_CFG_REG rc=%d\n",
				rc);
...
{
	struct smb_charger *chg =
		container_of(alarm, struct smb_charger, dcin_aicl_alarm);
...
irqreturn_t dc_plugin_irq_handler(int irq, void *data)
{
	...
	if (dcin_present && !vbus_present) {
		...
		if (chg->sec_cp_present) {
			...
			if (rc < 0)
				smblib_err(chg,
					   "Couldn't read batt status rc=%d\n",
					   rc);
...
static void smblib_six_pin_batt_step_chg_work(struct work_struct *work)
{
	...
	if (chg->chg_enable_k11a) {
		...
		rc = power_supply_get_property(
			chg->cp_psy, POWER_SUPPLY_PROP_TI_BATTERY_VOLTAGE,
			&pval);
		if (rc < 0)
			smblib_err(
				chg,
				"Error in getting TI_BATTERY_VOLTAGE, rc=%d\n",
				rc);
	...
	if (chg->start_step_vbat >= VBAT_FOR_STEP_MIN_UV) {
		smblib_err(
			chg,
			"start step vbat is too high, no need do step charge\n");
		return;
	}
	...
	if (chg->support_second_ffc_term_current) {
		...
		if (main_charge_type == POWER_SUPPLY_CHARGE_TYPE_TAPER &&
		    ...
		    pval.intval > SECOND_FFC_TERM_TEMP && capacity == 100) {
	...
	if (main_charge_type == POWER_SUPPLY_CHARGE_TYPE_TAPER &&
		...
		smblib_err(chg,
			   "taper from main charger, reducing FCC to %duA\n",
			   fcc_ua);
...
out:
	...
	if (chg->chg_enable_k11a) {
		pr_info("fv_uv:%d, fcc_ua:%d, index_vfloat:%d.\n", fv_uv,
			fcc_ua, chg->index_vfloat);
		if (batt_temp < chg->chg_cool_threshold &&
		    ti_battery_voltage > (fv_uv / 1000) && (fv_uv == 4450000)) {
			fv_uv -= 10000;
			vote(chg->fv_votable, NON_FFC_VFLOAT_VOTER, true,
			     fv_uv);
			smblib_err(
				chg,
				"temp:%d, ti_battery_voltage > NON_FFC_VFLOAT_VOTER, reduce uv:%d.\n",
				batt_temp, fv_uv);
		}

		if (chg->index_vfloat > 0) {
			if ((fv_uv == chg->six_pin_step_cfg[chg->index_vfloat]
					      .vfloat_step_uv) &&
				...
				smblib_err(
					chg,
					"ti_battery_voltage > vfloat_step_uv, reduce uv:%d.\n",
					fv_uv);
			}
			if ((fcc_ua == chg->six_pin_step_cfg[chg->index_vfloat]
					       .fcc_step_ua) &&
			    ibat_ua > fcc_ua) {
				fcc_ua -= 150000;
				vote(chg->fcc_votable, SIX_PIN_VFLOAT_VOTER,
...
static int smblib_dynamic_recharge_vbat(struct smb_charger *chg)
{
	...
	if (temp <= BATT_COOL_THRESHOLD) {
		...
		rc = smblib_get_prop_from_bms(
			chg, POWER_SUPPLY_PROP_RECHARGE_VBAT, &val);
		if (rc < 0) {
			smblib_dbg(chg, PR_REGISTER,
				   "Couldn't get recharge vbat rc=%d\n", rc);
...
static void smblib_chg_termination_work(struct work_struct *work)
{
	...
	if (chg->ext_fg) {
		...
		rc = smblib_get_prop_from_bms(
			chg, POWER_SUPPLY_PROP_VOLTAGE_MAX, &pval);
		if (rc < 0)
			goto out;
	...
	if (chg->ext_fg) {
		...
		if (pval.intval <
		    DIV_ROUND_CLOSEST(chg->cc_soc_ref * 10050, 10000)) {
			vote(chg->usb_icl_votable, CHG_TERMINATION_VOTER, false,
			     0);
			vote(chg->dc_suspend_votable, CHG_TERMINATION_VOTER,
			     false, 0);
			delay = CHG_TERM_WA_ENTRY_DELAY_MS;
			    ...
			    DIV_ROUND_CLOSEST(chg->cc_soc_ref * 10075,
					      10000)) &&
...
{
	struct smb_charger *chg =
		container_of(tm, struct smb_charger, apsd_timer);
...
int smblib_init(struct smb_charger *chg)
{
	...
	INIT_DELAYED_WORK(&chg->step_charge_notify_work,
			  smblib_step_charge_notify_work);
	INIT_DELAYED_WORK(&chg->cc_un_compliant_charge_work,
			  smblib_cc_un_compliant_charge_work);
	INIT_DELAYED_WORK(&chg->clean_cp_to_sw_work,
			  smblib_clean_cp_to_sw_work);
