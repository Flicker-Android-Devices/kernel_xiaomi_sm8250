...
int Dual_Fg_Check_Chg_Fg_Status_And_Disable_Chg_Path(struct dual_fg_chip *bq)
{
	...
	effective_fv_client =
		strcmp(get_effective_client(bq->fv_votable), SMART_BATTERY_FV);
	...
	if (!global_dual_fg_info->fg1_batt_ctl_enabled &&
		...
		if (fg_master_soc >= fg_slave_soc) {
			if ((effective_fv_client || bq->fast_mode) &&
			    fg_master_soc == FG_RAW_FULL &&
			    fg_master_volt >= 4470000) {
				rc = fg_master_set_disable(true);
				global_dual_fg_info->fg1_batt_ctl_enabled =
					true;
			}
...
static void fg_check_dual_current(struct dual_fg_chip *bq)
{
	...
	if (curr_max >= thres) {
		oc_count++;
		offset_Curr = (offset_Curr > (curr_max - step_curr / 2)) ?
				      offset_Curr :
				      (curr_max - step_curr / 2);
	}
	...
	if (oc_count >= 4) {
		...
		bq_dbg(PR_OEM,
		       "need down offset:%d, curr max:%d, div:%d, the diff current:%d\n",
...
static int fg_read_system_soc(struct dual_fg_chip *bq)
{
	...
	if (!global_dual_fg_info->fg1_batt_ctl_enabled) {
		...
		} else if (last_batt_soc_master < FG_FULL &&
			   bq->batt_temp < 0) {
			batt_soc_master = last_batt_soc_master;
		} else {
...
static int bq_battery_soc_smooth_tracking_new(struct dual_fg_chip *bq, int soc)
{
	...
	if (bq->raw_soc > HW_REPORT_FULL_SOC)
		...
		system_soc =
			((bq->raw_soc + SOC_PROPORTION_C) / SOC_PROPORTION);
		if (system_soc > 99)
			system_soc = 99;
	}
	...
	if ((status == POWER_SUPPLY_STATUS_DISCHARGING ||
		...
		bq_dbg(PR_OEM,
		       "enter low temperature smooth unit_time=%d batt_ma_now=%d\n",
		       unit_time, batt_ma);
	}
	...
	if (soc_delta >= 1 || (bq->batt_volt < 3300 && system_soc > 0) ||
		...
		bq_dbg(PR_OEM,
		       "system soc=%d,last system soc: %d,delta time: %d,soc_changed:%d,unit_time:%d,soc_delta:%d\n",
		       system_soc, last_system_soc, delta_time, soc_changed,
		       unit_time, soc_delta);
	}
...
static int fg_get_VoltageMax(struct dual_fg_chip *bq)
{
	...
	bq_dbg(PR_OEM, "dual_gauge voltage max:%d,effective_fv:%d\n", volt_max,
	       get_effective_result(bq->fv_votable));
	return volt_max;
}
		...
		if (bq->shutdown_delay_enable) {
			if (val->intval == 0) {
				...
				if (vbat_mv > SHUTDOWN_DELAY_VOL &&
					...
					bq->shutdown_delay = false;
					bq_dbg(PR_OEM,
					       "shutdown_delay_cancel:%d, shutdown vbat_uv_2::%d\n",
...
static void fg_update_status(struct dual_fg_chip *bq)
{
	...
	bq_dbg(PR_OEM,
	       "SOC:%d,Volt:%d,Cur:%d,Temp:%d,RM:%d,FC:%d,FAST:%d,Raw_soc:%d,Charging status:%d",
	       bq->batt_soc, bq->batt_volt, bq->batt_curr, bq->batt_temp,
	       bq->batt_rm, bq->batt_fcc, bq->fast_mode, bq->raw_soc,
	       bq->batt_st);

	bq_dbg(PR_OEM,
	       "Print master info. Volt_m:%d, Curr_m:%d, Temp_m:%d, Raw_soc_m:%d\n",
	       bq->batt_volt_m, bq->batt_curr_m, bq->batt_temp_m,
	       bq->raw_soc_m);

	bq_dbg(PR_OEM,
	       "Print slave info. Volt_s:%d, Curr_s:%d, Temp_s:%d, Raw_soc_s:%d\n",
	       bq->batt_volt_s, bq->batt_curr_s, bq->batt_temp_s,
	       bq->raw_soc_s);
