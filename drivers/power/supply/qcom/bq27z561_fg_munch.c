...
static int fg_read_system_soc(struct bq_fg_chip *bq)
{
	...
	if (bq->last_soc != soc) {
		if (abs(soc - bq->last_soc) > 1) {
			...
			bq_dbg(PR_OEM,
			       "avoid jump soc = %d last_soc = %d soc_change = %d state = %d ,delta_time = %d\n",
			       soc, bq->last_soc, soc_changed, status,
			       change_delta);
	...
	case POWER_SUPPLY_PROP_CAPACITY:
		...
		if (bq->shutdown_delay_enable) {
			if (val->intval == 0) {
				...
				bq_dbg(PR_OEM,
				       "vbat:%d,status:%d,shutdown_delay:%d",
				       vbat_mv, status, bq->shutdown_delay);
				if (vbat_mv > SHUTDOWN_DELAY_VOL &&
					...
					shutdown_delay_cancel[bq->fg_index] =
						true;
					val->intval = 1;
				} else {
					bq->shutdown_delay = false;
			...
			if (last_shutdown_delay[bq->fg_index] !=
			    bq->shutdown_delay) {
				last_shutdown_delay[bq->fg_index] =
					bq->shutdown_delay;
				if (bq->fg_psy)
...
*/
	...
	case POWER_SUPPLY_PROP_CHIP_OK:
		...
		bq->chip_ok = fg_read_word(bq, bq->regs[BQ_FG_REG_BATT_STATUS],
					   &flags);
		ret = bq->chip_ok;
		if (ret < 0)
			val->intval = 0;
			   ...
			   const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_AUTHENTIC:
		...
		vote(bq->fcc_votable, BMS_VERIFY_VOTER,
...
static DEVICE_ATTR(max_charge_current, S_IRUGO, fg_attr_show_max_charge_current,
		   NULL);
static DEVICE_ATTR(max_discharge_current, S_IRUGO,
		   fg_attr_show_max_discharge_current, NULL);
...
static DEVICE_ATTR(total_fw_runtime, S_IRUGO, fg_attr_show_total_fw_runtime,
		   NULL);
static DEVICE_ATTR(time_spent_in_lt, S_IRUGO, fg_attr_show_time_spent_in_lt,
		   NULL);
static DEVICE_ATTR(time_spent_in_ht, S_IRUGO, fg_attr_show_time_spent_in_ht,
		   NULL);
static DEVICE_ATTR(time_spent_in_ot, S_IRUGO, fg_attr_show_time_spent_in_ot,
		   NULL);
					  ...
					  int batt_ma)
{
	...
	if (bq->fast_mode && raw_soc >= FG_REPORT_FULL_SOC &&
		...
		if (raw_soc > last_raw_soc[bq->fg_index] && soc_changed) {
			...
			last_optimiz_time[bq->fg_index] = ktime_get();
			bq_dbg(PR_DEBUG,
			       "optimiz_soc:%d, last_optimiz_time%ld\n",
			       optimiz_soc[bq->fg_index],
			       last_optimiz_time[bq->fg_index]);
	...
	if (batt_temp > BATT_COOL_THRESHOLD && !cold_smooth[bq->fg_index] &&
	    batt_soc != 0) {
		if (bq->ffc_smooth &&
		    (status == POWER_SUPPLY_STATUS_DISCHARGING ||
	...
	if (last_batt_soc[bq->fg_index] < batt_soc && batt_ma < 0)
		 ...
		 */
		last_batt_soc[bq->fg_index] =
			bq->update_now ?
				batt_soc :
				last_batt_soc[bq->fg_index] + soc_changed;
	...
	if (abs(delta_time - last_log_time) >= 1) {
		...
		bq_dbg(PR_OEM,
		       "batt_ma_avg:%d, batt_ma:%d, cold_smooth:%d, optimiz_soc:%d",
		       batt_ma_avg, batt_ma, cold_smooth[bq->fg_index],
		       optimiz_soc[bq->fg_index]);
		bq_dbg(PR_OEM,
		       "unit_time:%d soc_changed:%d, bq->update_now:%d, bq->ffc_smooth",
		       unit_time, soc_changed, bq->update_now, bq->ffc_smooth);
		bq_dbg(PR_OEM,
		       "raw_soc:%d batt_soc:%d,last_batt_soc:%d,system_soc:%d"
		       " bq->fast_mode:%d",
		       raw_soc, batt_soc, last_batt_soc[bq->fg_index],
		       system_soc[bq->fg_index], bq->fast_mode);
	}
...
static int fg_check_full_status(struct bq_fg_chip *bq)
{
	...
	if (!bq->fast_mode && prop.intval &&
	    bq->raw_soc >= BQ_REPORT_FULL_SOC) {
		if (bq->batt_soc < 100)
...
static int bq_parse_dt(struct bq_fg_chip *bq)
{
	...
	bq->force_soc_enable =
		of_property_read_bool(node, "bq,force-soc-enable");
...
static int fg_check_device(struct bq_fg_chip *bq)
{
	...
	for (i = 0; i <= 3; i++) {
		...
		if (ret == 1)
			bq_dbg(PR_OEM,
			       "failed to get FG_MAC_CMD_DEVICE_NAME with ret:%d and retry!\n",
			       ret);
		else if (ret < 0)
			bq_dbg(PR_OEM,
			       "failed to get FG_MAC_CMD_DEVICE_NAME with ret:%d!\n",
			       ret);
		else
			break;
	}
