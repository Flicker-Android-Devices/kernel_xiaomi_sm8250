...
static void pen_charge_notifier_work(struct work_struct *work)
{
	struct idtp9220_device_info *di = container_of(
		work, struct idtp9220_device_info, pen_notifier_work.work);
...
void pen_charge_state_notifier_call_chain(unsigned long val, void *v)
{
	struct idtp9220_device_info *di =
		container_of(pen_notifier_work, struct idtp9220_device_info,
			     pen_notifier_work);
	di->pen_val = val;
	di->pen_v = v;
...
static void idtp9220_hall3_irq_work(struct work_struct *work)
{
	...
	if (di->fw_update) {
		dev_info(di->dev,
			 "[hall3] fw updating, don't enable reverse chg\n");
		return;
	}
...
static void idtp9220_hall4_irq_work(struct work_struct *work)
{
	...
	if (di->fw_update) {
		dev_info(di->dev,
			 "[hall4] fw updating, don't enable reverse chg\n");
		return;
	}
