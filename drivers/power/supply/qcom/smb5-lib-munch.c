...
static const struct apsd_result *smblib_get_apsd_result(struct smb_charger *chg)
{
	...
	if (chg->hvdcp_det_lock == true) {
		...
		smblib_dbg(chg, PR_OEM,
			   "hold apsd result to DCP for bootup pd check!\n");
		return result;
	}
...
static const struct apsd_result *smblib_update_usb_type(struct smb_charger *chg)
{
	...
	smblib_dbg(chg, PR_OEM, "APSD=%s PD=%d QC3P5=%d\n", apsd_result->name,
		   chg->pd_active, chg->qc3p5_detected);
	return apsd_result;
}
				      ...
				      const union power_supply_propval *val)
{
	...
	smblib_dbg(chg, PR_OEM,
		   "thermal_remove:%d, thermal level:%d, thermal_levels:%d "
	...
	if (thermal_status != chg->thermal_status) {
		...
		if (thermal_status == TEMP_ABOVE_RANGE) {
			...
			while (retry_count) {
				...
				smblib_err(
					chg,
					"connect temp is too hot,cp_enable:%d,cp_sec_enable:%d,etry_count:%d\n",
...
{
	struct smb_charger *chg =
		container_of(work, struct smb_charger, slow_pd_wa_work.work);
...
static void smblib_handle_apsd_done(struct smb_charger *chg, bool rising)
{
	...
	if (time.tv_sec < QC_WAIT_PD_CONN_TIME_S &&
	    apsd_result->pst == POWER_SUPPLY_TYPE_USB_DCP &&
	    chg->hvdcp_det_lock == false) {
		if (!chg->slow_pd_wa_wakelock.active) {
			__pm_stay_awake(&chg->slow_pd_wa_wakelock);
			chg->hvdcp_det_lock = true;
			smblib_err(chg,
				   "boot_time %ld, wait pd update status\n",
				   time.tv_sec);
			smblib_hvdcp_detect_enable(chg, false);
			recheck_time =
				(QC_WAIT_PD_CONN_TIME_S - time.tv_sec) * 1000;
		}
	}
	...
	if (chg->hvdcp_det_lock) {
		schedule_delayed_work(&chg->slow_pd_wa_work,
