...
EXPORT_SYMBOL(write_byte_slave);

int onewire_gpio_get_status_slave(void)
{
	return onewire_gpio_detected;
}
EXPORT_SYMBOL(onewire_gpio_get_status_slave);
					  ...
					  const char *buf, size_t count)
{
	...
	if (buf_int == 0) {
		...
		if (result)
			ow_dbg("ow_reset_slave: no device.result = %02x",
			       result);
		else
			ow_dbg("ow_reset_slave: device exist.result = %02x",
			       result);
	} else if (buf_int == 5) {
		result = read_bit();
...
static int __init onewire_gpio_init(void)
{
	...
	onewire_class = class_create(THIS_MODULE, "onewire_slave");
...
static void __exit onewire_gpio_exit(void)
{
	...
	unregister_chrdev(onewire_major, "onewireslavectrl");
	class_destroy(onewire_class);
}
