...
static int usbpd_pm_get_vbus_error(struct usbpd_pm *pdpm)
{
	...
	ret = power_supply_get_property(
		pdpm->cp_psy, POWER_SUPPLY_PROP_TI_BUS_ERROR_STATUS, &val);
	if (!ret)
...
static int usbpd_pm_update_cp_charge_mode(struct usbpd_pm *pdpm)
{
	...
	if (fcc <= BYPASS_FCC_ENTER_MAX_THRES &&
	    fcc >= BYPASS_FCC_ENTER_MIN_THRES &&
	    pdpm->apdo_max_curr >= BYPASS_FCC_ENTER_MAX_THRES &&
	    (chg_mode == SC8551_CHARGE_MODE_DIV2 ||
	     chg_mode == SC8551_CHARGE_MODE_STANDBY)) {
		...
		chg_mode = SC8551_CHARGE_MODE_DIV2;
		   ...
		   fcc > BYPASS_FCC_ENTER_MAX_THRES &&
		   chg_mode == SC8551_CHARGE_MODE_STANDBY) {
		chg_mode = SC8551_CHARGE_MODE_DIV2;
	}
	pr_info("chg_mode:%d\n", chg_mode);
...
static void usbpd_pm_evaluate_src_caps(struct usbpd_pm *pdpm)
{
	...
	if (pdpm->pps_supported) {
		...
		if (pdpm->apdo_min_volt >= 5000 && pdpm->usb_psy) {
			...
			ret = power_supply_set_property(
				pdpm->usb_psy,
				POWER_SUPPLY_PROP_ENABLE_BYPASS_MODE, &pval);
			if (ret < 0)
...
static int chg_mode, ibus_upgrade, ibus_upgrade_bypass, last_curr_ibus_limit,
	last_curr_ibus_limit_bypass;
static int usbpd_pm_fc2_charge_algo(struct usbpd_pm *pdpm)
{
	...
	if (is_fastcharge_mode) {
		...
		pm_config.bat_volt_lp_lmt =
			pdpm->non_ffc_bat_volt_max - pval.intval;
	...
	if (!pdpm->use_qcom_gauge && is_fastcharge_mode) {
		if (pdpm->cp.bms_vbat_mv >
		    pdpm->cell_vol_high_threshold_mv - pval.intval) {
...
reg[%d-%d-%d-%d-%d],step[%d-%d-%d-%d-%d-%d-%d-%d],pmconfig[%d-%d-%d,%d-%d-%d-%d][%d-%d-%d-%d-%d]\n",
		       ...
		       pdpm->cp.sc8551_charge_mode, pdpm->enable_bypass,
	...
	if (pdpm->enable_bypass == 1) {
		...
		if (pdpm->cp.sc8551_charge_mode != chg_mode &&
		    chg_mode != SC8551_CHARGE_MODE_STANDBY) {
			if (chg_mode == SC8551_CHARGE_MODE_DIV2 ||
	...
	if (pdpm->cp.sc8551_charge_mode != SC8551_CHARGE_MODE_BYPASS) {
		...
		pdpm->request_current =
			min(pdpm->apdo_max_curr, curr_ibus_limit);
...
static int usbpd_pm_sm(struct usbpd_pm *pdpm)
{
	...
	if (pdpm->usb_psy) {
		rc = power_supply_get_property(
			pdpm->usb_psy, POWER_SUPPLY_PROP_ENABLE_BYPASS_MODE,
			&val);
		if (rc < 0)
			pr_err("can not get enable_bypass with rc:%d\n", rc);
	...
	case PD_PM_STATE_ENTRY:
		...
		} else if (effective_fcc_val < BYPASS_FCC_ENTER_MIN_THRES &&
			   chg_mode != SC8551_CHARGE_MODE_BYPASS) {
	...
	case PD_PM_STATE_FC2_ENTRY_1:
		...
		if (pdpm->enable_bypass == 1) {
			...
			if (pdpm->cp.sc8551_charge_mode != chg_mode) {
				if (pdpm->cp_model == CHARGEPUMP_LN8000)
					...
					usbpd_pm_switch_cp_charge_mode(
						pdpm, chg_mode);
		...
		if (pdpm->cp.sc8551_charge_mode == 1) {
			...
			pr_err("%s ENTRY_1  request_voltage [%d-%d]\n",
			       __func__, pdpm->request_voltage,
			       pdpm->cp.vbat_volt);
			pdpm->request_current =
				min(pdpm->apdo_max_curr, curr_ibus_lmt);
			pr_err("%s ENTRY_1 request_current [%d-%d-%d]\n",
			       __func__, pdpm->apdo_max_curr, curr_ibus_lmt,
			       MAX_BYPASS_CURRENT_MA);
		} else {
			if (pdpm->cp.sc8551_charge_mode == -1)
		...
		if (pdpm->sw_psy) {
			...
			rc = power_supply_set_property(
				pdpm->sw_psy, POWER_SUPPLY_PROP_CP_TO_SW_STATUS,
				&val);
			if (rc < 0)
	...
	case PD_PM_STATE_FC2_ENTRY_2:
		...
		if (((pdpm->cp.vbus_volt <
			...
			else if (pdpm->cp.vbus_error_status ==
					 CP_VBUS_ERROR_LOW ||
				 pdpm->cp_model == CHARGEPUMP_LN8000)
				pdpm->request_voltage += STEP_MV_INIT_VBUS;
			pr_err("%s [ENTRY_2low] %d -%d-%d-%d-%d\n", __func__,
			       tune_vbus_retry, pdpm->request_voltage,
			       pdpm->request_current, pdpm->cp.vbus_volt,
			       pdpm->cp.vbat_volt);
			usbpd_select_pdo(pdpm->pd, pdpm->apdo_selected_pdo,
					 pdpm->request_voltage * 1000,
					 pdpm->request_current * 1000);
		} else if ((pdpm->cp.vbus_volt > (pdpm->cp.vbat_volt * 2 +
	...
	case PD_PM_STATE_FC2_ENTRY_3:
		...
		pr_err("%s PD_PM_STATE_FC2_ENTRY_3 :%d-%d-%d-%d\n", __func__,
		       pdpm->cp_sec.charge_enabled, pm_config.cp_sec_enable,
		...
		if (chg_mode != SC8551_CHARGE_MODE_BYPASS)
			if (pdpm->fcc_votable) {
				vote(pdpm->fcc_votable, BQ_TAPER_FCC_VOTER,
				     false, 0);
				vote(pdpm->fcc_votable,
				     BQ_TAPER_CELL_HGIH_FCC_VOTER, false, 0);
			}
