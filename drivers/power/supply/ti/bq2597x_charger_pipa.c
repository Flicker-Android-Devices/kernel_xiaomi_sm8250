...
static int bq2597x_get_adc_data(struct bq2597x *bq, int channel, int *result)
{
	...
	if (bq->chip_vendor == NU2105) {
		...
		*result = t;
		...
		if (channel == ADC_VBAT) {
			t = t *
			    (NU2105_SCALE_FACTOR + NU2105_CALIBRATION_FACTOR) /
			    NU2105_SCALE_FACTOR;
			*result = t;
		}
	} else {
		...
		*result = t;
		...
		if (bq->chip_vendor == SC8551) {
			*result = (u64)t * (u64)sc8551_adc_lsb[channel] /
				  10000000;
		}
	}
...
static int bq2597x_get_dev_role(struct i2c_client *client)
{
	...
	dev_info(&client->dev, "%s: matched to %s, dev_role: %d.\n", __func__,
		 of_id->compatible, (int)(uintptr_t)of_id->data);
...
static ssize_t diff_ti_bus_current_show(struct device *dev,
...
static int try_to_find_i2c_regess(struct bq2597x *bq)
{
	...
	for (i = 0; i < 2; i++) {
		...
		ret = i2c_smbus_read_byte_data(bq->client,
					       SC8551_PART_INFO_REG);
